/** @typedef {import('../runtime.js').LocalizedString} LocalizedString */
/** @typedef {{}} Acceptinviteerrorbody3Inputs */
/** @typedef {{}} Acceptinviteerrortitle3Inputs */
/** @typedef {{}} Acceptinvitetitle2Inputs */
/** @typedef {{}} Aiapikeylabel3Inputs */
/** @typedef {{}} Aiconnectbutton2Inputs */
/** @typedef {{}} Aiconnectdialogdescription3Inputs */
/** @typedef {{}} Aiconnectdialogtitle3Inputs */
/** @typedef {{}} Aiconnectguidetitle3Inputs */
/** @typedef {{}} Aiconnectsubmit2Inputs */
/** @typedef {{}} Aiconnectioncolumnactions3Inputs */
/** @typedef {{}} Aiconnectioncolumnmodel3Inputs */
/** @typedef {{}} Aiconnectioncolumnprovider3Inputs */
/** @typedef {{}} Aiconnectioncolumnstatus3Inputs */
/** @typedef {{}} Aiconnectiondefaultbadge3Inputs */
/** @typedef {{}} Aiconnectionstatusactive3Inputs */
/** @typedef {{}} Aiconnectionstatusinvalid3Inputs */
/** @typedef {{}} Aiconnectionsdescription2Inputs */
/** @typedef {{}} Aiconnectionsemptytitle3Inputs */
/** @typedef {{}} Aiconnectionsloaderrortitle4Inputs */
/** @typedef {{}} Aiconnectionstitle2Inputs */
/** @typedef {{}} Aicreditbalancelabel3Inputs */
/** @typedef {{}} Aicreditbalancelowwarning4Inputs */
/** @typedef {{}} Aidefaultmodellabel3Inputs */
/** @typedef {{}} Aifetchmodelsempty3Inputs */
/** @typedef {{}} Aifetchmodelserror3Inputs */
/** @typedef {{}} Aifetchmodelswaitingforkey5Inputs */
/** @typedef {{}} Aiinvalidapikeyerror4Inputs */
/** @typedef {{}} Aiproviderdescriptionanthropic3Inputs */
/** @typedef {{}} Aiproviderdescriptiongroq3Inputs */
/** @typedef {{}} Aiproviderdescriptionnvidia3Inputs */
/** @typedef {{}} Aiproviderdescriptionopenai3Inputs */
/** @typedef {{}} Aiproviderdescriptionopenrouter3Inputs */
/** @typedef {{}} Aiproviderlabel2Inputs */
/** @typedef {{}} Airemoveconfirmbody3Inputs */
/** @typedef {{}} Airemoveconfirmtitle3Inputs */
/** @typedef {{}} Airemoveconnectionaction3Inputs */
/** @typedef {{}} Airemoveconnectionerrortoast4Inputs */
/** @typedef {{}} Aisetdefaultaction3Inputs */
/** @typedef {{}} Aisetdefaulterrortoast4Inputs */
/** @typedef {{}} Aitrialremaininglabel3Inputs */
/** @typedef {{}} Aitrialremaininglowwarning4Inputs */
/** @typedef {{}} Aiusagecolumncost3Inputs */
/** @typedef {{}} Aiusagecolumndate3Inputs */
/** @typedef {{}} Aiusagecolumnmodel3Inputs */
/** @typedef {{}} Aiusagecolumntokens3Inputs */
/** @typedef {{}} Aiusagedescription2Inputs */
/** @typedef {{}} Aiusageemptytitle3Inputs */
/** @typedef {{}} Aiusageloaderrortitle4Inputs */
/** @typedef {{}} Aiusagetitle2Inputs */
/** @typedef {{}} Appname1Inputs */
/** @typedef {{}} Apptagline1Inputs */
/** @typedef {{}} Auditlogcolumnaction3Inputs */
/** @typedef {{}} Auditlogcolumnactor3Inputs */
/** @typedef {{}} Auditlogcolumntarget3Inputs */
/** @typedef {{}} Auditlogcolumntime3Inputs */
/** @typedef {{}} Auditlogdescription2Inputs */
/** @typedef {{}} Auditlogemptytitle3Inputs */
/** @typedef {{}} Auditlogloaderrortitle4Inputs */
/** @typedef {{}} Auditlogtitle2Inputs */
/** @typedef {{}} Authsidedescription2Inputs */
/** @typedef {{}} Authsidetitle2Inputs */
/** @typedef {{}} Authvaluelead2Inputs */
/** @typedef {{}} Authvaluenext2Inputs */
/** @typedef {{}} Authvaluepayment2Inputs */
/** @typedef {{}} Campaignsaccountnamelabel3Inputs */
/** @typedef {{}} Campaignsaccountnumberlabel3Inputs */
/** @typedef {{}} Campaignsactionduplicate2Inputs */
/** @typedef {{}} Campaignsaddbutton2Inputs */
/** @typedef {{}} Campaignsadddialogtitle3Inputs */
/** @typedef {{}} Campaignsanalyticsaction2Inputs */
/** @typedef {{}} Campaignsanalyticsbysourceempty4Inputs */
/** @typedef {{}} Campaignsanalyticsbysourcetitle4Inputs */
/** @typedef {{}} Campaignsanalyticsconversion2Inputs */
/** @typedef {{}} Campaignsanalyticsdescription2Inputs */
/** @typedef {{}} Campaignsanalyticsloaderrortitle4Inputs */
/** @typedef {{}} Campaignsanalyticsorders2Inputs */
/** @typedef {{}} Campaignsanalyticsrevenue2Inputs */
/** @typedef {{}} Campaignsanalyticssourcecolumn3Inputs */
/** @typedef {{}} Campaignsanalyticssubmits2Inputs */
/** @typedef {{}} Campaignsanalyticstitle2Inputs */
/** @typedef {{}} Campaignsanalyticsviews2Inputs */
/** @typedef {{}} Campaignsassignmentmodelabel3Inputs */
/** @typedef {{}} Campaignsassignmentmodemanual3Inputs */
/** @typedef {{}} Campaignsassignmentmoderoundrobin4Inputs */
/** @typedef {{}} Campaignsbankbinlabel3Inputs */
/** @typedef {{}} Campaignsbulkchangestatus3Inputs */
/** @typedef {{}} Campaignsbulkclearselection3Inputs */
/** @typedef {{}} Campaignsbulkdelete2Inputs */
/** @typedef {{}} Campaignsbulkdeleteconfirm3Inputs */
/** @typedef {{ count: NonNullable<unknown> }} Campaignsbulkdeleteconfirmdescription4Inputs */
/** @typedef {{}} Campaignsbulkdeleteconfirmtitle4Inputs */
/** @typedef {{}} Campaignsbulkdeleteerrortoast4Inputs */
/** @typedef {{ count: NonNullable<unknown> }} Campaignsbulkselectedcount3Inputs */
/** @typedef {{}} Campaignsbulkstatuserrortoast4Inputs */
/** @typedef {{}} Campaignscolumnactions2Inputs */
/** @typedef {{}} Campaignscolumnname2Inputs */
/** @typedef {{}} Campaignscolumnproducts2Inputs */
/** @typedef {{}} Campaignscolumnstatus2Inputs */
/** @typedef {{}} Campaignsdescription1Inputs */
/** @typedef {{}} Campaignsdialogdescription2Inputs */
/** @typedef {{}} Campaignsduplicateerrortoast3Inputs */
/** @typedef {{}} Campaignseditdialogtitle3Inputs */
/** @typedef {{}} Campaignsemptytitle2Inputs */
/** @typedef {{}} Campaignsendsatlabel3Inputs */
/** @typedef {{}} Campaignsformfieldaddbutton4Inputs */
/** @typedef {{}} Campaignsformfieldkeyplaceholder4Inputs */
/** @typedef {{}} Campaignsformfieldlabellabel4Inputs */
/** @typedef {{}} Campaignsformfieldoptionsplaceholder4Inputs */
/** @typedef {{}} Campaignsformfieldremovelabel4Inputs */
/** @typedef {{}} Campaignsformfieldrequiredlabel4Inputs */
/** @typedef {{}} Campaignsformfieldtypecheckbox4Inputs */
/** @typedef {{}} Campaignsformfieldtypeselect4Inputs */
/** @typedef {{}} Campaignsformfieldtypetext4Inputs */
/** @typedef {{}} Campaignsformfieldsempty3Inputs */
/** @typedef {{}} Campaignsformfieldssectiontitle4Inputs */
/** @typedef {{}} Campaignsgoallabel2Inputs */
/** @typedef {{}} Campaignsloaderrortitle3Inputs */
/** @typedef {{}} Campaignsnamelabel2Inputs */
/** @typedef {{}} Campaignsogimagecreatefirsthint5Inputs */
/** @typedef {{}} Campaignsogimagedescription3Inputs */
/** @typedef {{}} Campaignsogimagelabel3Inputs */
/** @typedef {{}} Campaignsogimagesectiontitle4Inputs */
/** @typedef {{}} Campaignspaymentenabledlabel3Inputs */
/** @typedef {{}} Campaignspaymentsectiontitle3Inputs */
/** @typedef {{}} Campaignspopupbodylabel3Inputs */
/** @typedef {{}} Campaignspopupmanualpendingtitle4Inputs */
/** @typedef {{}} Campaignspopuppaidtitle3Inputs */
/** @typedef {{}} Campaignspopupregisteredtitle3Inputs */
/** @typedef {{}} Campaignspopuptitlelabel3Inputs */
/** @typedef {{}} Campaignspopupssectiontitle3Inputs */
/** @typedef {{}} Campaignsproductsempty2Inputs */
/** @typedef {{}} Campaignsproductslabel2Inputs */
/** @typedef {{}} Campaignsremoveconfirmaction3Inputs */
/** @typedef {{}} Campaignsremoveconfirmbody3Inputs */
/** @typedef {{ name: NonNullable<unknown> }} Campaignsremoveconfirmtitle3Inputs */
/** @typedef {{}} Campaignsremoveerrortoast3Inputs */
/** @typedef {{}} Campaignssaveerrortoast3Inputs */
/** @typedef {{}} Campaignssavesubmit2Inputs */
/** @typedef {{}} Campaignssearchplaceholder2Inputs */
/** @typedef {{}} Campaignssepayautolabel3Inputs */
/** @typedef {{}} Campaignsstartsatlabel3Inputs */
/** @typedef {{}} Campaignsstatusactive2Inputs */
/** @typedef {{}} Campaignsstatusdraft2Inputs */
/** @typedef {{}} Campaignsstatusended2Inputs */
/** @typedef {{}} Campaignsstatuslabel2Inputs */
/** @typedef {{}} Campaignsstatuspaused2Inputs */
/** @typedef {{}} Campaignstitle1Inputs */
/** @typedef {{}} Campaignstransferprefixlabel3Inputs */
/** @typedef {{}} Campaignsutmdefaultaddbutton4Inputs */
/** @typedef {{}} Campaignsutmdefaultsempty3Inputs */
/** @typedef {{}} Campaignsutmdefaultssectiontitle4Inputs */
/** @typedef {{}} Campaignsutmkeyplaceholder3Inputs */
/** @typedef {{}} Campaignsutmremovelabel3Inputs */
/** @typedef {{}} Campaignsutmvalueplaceholder3Inputs */
/** @typedef {{}} Campaignszalourllabel3Inputs */
/** @typedef {{}} Commonall1Inputs */
/** @typedef {{}} Commonback1Inputs */
/** @typedef {{}} Commoncancel1Inputs */
/** @typedef {{}} Commoncontinue1Inputs */
/** @typedef {{}} Commoncopy1Inputs */
/** @typedef {{}} Commoncreate1Inputs */
/** @typedef {{}} Commonedit1Inputs */
/** @typedef {{}} Commonloadmore2Inputs */
/** @typedef {{}} Commonloading1Inputs */
/** @typedef {{}} Commonnext1Inputs */
/** @typedef {{ page: NonNullable<unknown>, total: NonNullable<unknown> }} Commonpaginationlabel2Inputs */
/** @typedef {{}} Commonprevious1Inputs */
/** @typedef {{}} Commonredo1Inputs */
/** @typedef {{}} Commonretry1Inputs */
/** @typedef {{}} Commonsave1Inputs */
/** @typedef {{}} Commonundo1Inputs */
/** @typedef {{}} Domainsaddbutton2Inputs */
/** @typedef {{}} Domainsadddialogdescription3Inputs */
/** @typedef {{}} Domainsadddialogtitle3Inputs */
/** @typedef {{}} Domainsadderrortoast3Inputs */
/** @typedef {{}} Domainsaddsubmit2Inputs */
/** @typedef {{}} Domainscnamehostlabel3Inputs */
/** @typedef {{}} Domainscnamevaluelabel3Inputs */
/** @typedef {{}} Domainscolumnactions2Inputs */
/** @typedef {{}} Domainscolumnhostname2Inputs */
/** @typedef {{}} Domainscolumnstatus2Inputs */
/** @typedef {{}} Domainsdescription1Inputs */
/** @typedef {{}} Domainsdnsinstructionsintro3Inputs */
/** @typedef {{}} Domainsemptytitle2Inputs */
/** @typedef {{}} Domainshostnamelabel2Inputs */
/** @typedef {{}} Domainslandingpagelabel3Inputs */
/** @typedef {{}} Domainslandingpageplaceholder3Inputs */
/** @typedef {{}} Domainsloaderrortitle3Inputs */
/** @typedef {{}} Domainsremoveaction2Inputs */
/** @typedef {{}} Domainsremoveconfirmbody3Inputs */
/** @typedef {{ hostname: NonNullable<unknown> }} Domainsremoveconfirmtitle3Inputs */
/** @typedef {{}} Domainsremoveerrortoast3Inputs */
/** @typedef {{}} Domainsstatusactive2Inputs */
/** @typedef {{}} Domainsstatusfailed2Inputs */
/** @typedef {{}} Domainsstatuspending2Inputs */
/** @typedef {{}} Domainstitle1Inputs */
/** @typedef {{}} Domainstxthostlabel3Inputs */
/** @typedef {{}} Domainstxtinstructionsintro3Inputs */
/** @typedef {{}} Domainstxtvaluelabel3Inputs */
/** @typedef {{}} Domainsupgraderequireddescription3Inputs */
/** @typedef {{}} Domainsupgraderequiredtitle3Inputs */
/** @typedef {{}} Domainsvaluecopiedtoast3Inputs */
/** @typedef {{}} Domainsverifybutton2Inputs */
/** @typedef {{}} Domainsverifyerrortoast3Inputs */
/** @typedef {{}} Entityimagedrophint3Inputs */
/** @typedef {{}} Entityimageremovelabel3Inputs */
/** @typedef {{}} Entityimageuploadfailed3Inputs */
/** @typedef {{}} Entityimageuploadlabel3Inputs */
/** @typedef {{}} Entityimageuploadinglabel3Inputs */
/** @typedef {{}} Featureupgradegenericdescription3Inputs */
/** @typedef {{}} Featureupgradegenerictitle3Inputs */
/** @typedef {{}} Forgotpasswordbody2Inputs */
/** @typedef {{}} Forgotpasswordemaillabel3Inputs */
/** @typedef {{}} Forgotpasswordsent2Inputs */
/** @typedef {{}} Forgotpasswordsubmit2Inputs */
/** @typedef {{}} Forgotpasswordtitle2Inputs */
/** @typedef {{}} Inboxdescription1Inputs */
/** @typedef {{}} Inboxtitle1Inputs */
/** @typedef {{}} Landingsactionassigntocampaign4Inputs */
/** @typedef {{}} Landingsactiondelete2Inputs */
/** @typedef {{}} Landingsactionduplicate2Inputs */
/** @typedef {{}} Landingsactionremovefromcampaign4Inputs */
/** @typedef {{}} Landingsactionrename2Inputs */
/** @typedef {{}} Landingsactionviewlive3Inputs */
/** @typedef {{}} Landingsassigntocampaignempty4Inputs */
/** @typedef {{}} Landingsbadgedraft2Inputs */
/** @typedef {{}} Landingsbadgepublished2Inputs */
/** @typedef {{}} Landingscardactionslabel3Inputs */
/** @typedef {{}} Landingscreateaibutton3Inputs */
/** @typedef {{}} Landingscreateaidescription3Inputs */
/** @typedef {{}} Landingscreateaititle3Inputs */
/** @typedef {{}} Landingscreateerrortoast3Inputs */
/** @typedef {{}} Landingscreateheading2Inputs */
/** @typedef {{}} Landingscreateimportdescription3Inputs */
/** @typedef {{}} Landingscreateimporttitle3Inputs */
/** @typedef {{}} Landingscreatemanualbutton3Inputs */
/** @typedef {{}} Landingscreatemanualdescription3Inputs */
/** @typedef {{}} Landingscreatemanualtitle3Inputs */
/** @typedef {{}} Landingsfilterall2Inputs */
/** @typedef {{}} Landingsfilterallcampaigns3Inputs */
/** @typedef {{}} Landingsfilterdraft2Inputs */
/** @typedef {{}} Landingsfilterpublished2Inputs */
/** @typedef {{}} Landingsimportbutton2Inputs */
/** @typedef {{}} Landingsimportdialogtitle3Inputs */
/** @typedef {{}} Landingsimporterrortoast3Inputs */
/** @typedef {{}} Landingsimporthtmlplaceholder3Inputs */
/** @typedef {{}} Landingsimportnamelabel3Inputs */
/** @typedef {{}} Landingsimportnameplaceholder3Inputs */
/** @typedef {{}} Landingsimportsubmitbutton3Inputs */
/** @typedef {{}} Landingsimportsubmitting2Inputs */
/** @typedef {{}} Landingsimporttabfile3Inputs */
/** @typedef {{}} Landingsimporttabhtml3Inputs */
/** @typedef {{}} Landingsimporttaburl3Inputs */
/** @typedef {{}} Landingsimporturlplaceholder3Inputs */
/** @typedef {{}} Landingsloaderrortitle3Inputs */
/** @typedef {{}} Landingsnoresultstitle3Inputs */
/** @typedef {{}} Landingsreuploadbutton2Inputs */
/** @typedef {{}} Landingsreuploaddialogtitle3Inputs */
/** @typedef {{}} Landingssearchplaceholder2Inputs */
/** @typedef {{}} Landingssortname2Inputs */
/** @typedef {{}} Landingssortupdated2Inputs */
/** @typedef {{ total: NonNullable<unknown>, published: NonNullable<unknown> }} Landingssummarycount2Inputs */
/** @typedef {{}} Landingsupdatedat2Inputs */
/** @typedef {{}} Leadsourcecsvimport3Inputs */
/** @typedef {{}} Leadsourcedirect2Inputs */
/** @typedef {{}} Leadsourcefacebook2Inputs */
/** @typedef {{}} Leadsourcegeneric2Inputs */
/** @typedef {{}} Leadsourcegoogleads3Inputs */
/** @typedef {{}} Leadsourcelandingpage3Inputs */
/** @typedef {{}} Leadsourcemanual2Inputs */
/** @typedef {{}} Leadsourcetiktok2Inputs */
/** @typedef {{}} Leadsourcezalooa3Inputs */
/** @typedef {{}} Leadsactioncall2Inputs */
/** @typedef {{}} Leadsactioncopiedtoast3Inputs */
/** @typedef {{}} Leadsactioncopyphone3Inputs */
/** @typedef {{}} Leadsactionzalo2Inputs */
/** @typedef {{}} Leadsactivitycall2Inputs */
/** @typedef {{}} Leadsactivitynote2Inputs */
/** @typedef {{}} Leadsactivityordercreated3Inputs */
/** @typedef {{}} Leadsactivitypayment2Inputs */
/** @typedef {{}} Leadsactivityresubmit2Inputs */
/** @typedef {{}} Leadsactivitystagechange3Inputs */
/** @typedef {{}} Leadsactivitysystem2Inputs */
/** @typedef {{}} Leadsaddnotelabel3Inputs */
/** @typedef {{}} Leadsaddnoteplaceholder3Inputs */
/** @typedef {{}} Leadsaddnotesubmit3Inputs */
/** @typedef {{ days: NonNullable<unknown> }} Leadsagedays2Inputs */
/** @typedef {{ hours: NonNullable<unknown> }} Leadsagehours2Inputs */
/** @typedef {{}} Leadsassignerrortoast3Inputs */
/** @typedef {{}} Leadsassigneelabel2Inputs */
/** @typedef {{}} Leadsassignmentaddrule3Inputs */
/** @typedef {{}} Leadsassignmentdeleteerrortoast4Inputs */
/** @typedef {{}} Leadsassignmenteditrule3Inputs */
/** @typedef {{}} Leadsassignmentemptytitle3Inputs */
/** @typedef {{}} Leadsassignmentfixedassigneelabel4Inputs */
/** @typedef {{}} Leadsassignmentpersonalabel3Inputs */
/** @typedef {{}} Leadsassignmentpersonaplaceholder3Inputs */
/** @typedef {{}} Leadsassignmentpoollabel3Inputs */
/** @typedef {{}} Leadsassignmentreordererrortoast4Inputs */
/** @typedef {{}} Leadsassignmentruledeletelabel4Inputs */
/** @typedef {{}} Leadsassignmentrulesdescription3Inputs */
/** @typedef {{}} Leadsassignmentrulesmanagebutton4Inputs */
/** @typedef {{}} Leadsassignmentrulestitle3Inputs */
/** @typedef {{}} Leadsassignmentsaveerrortoast4Inputs */
/** @typedef {{}} Leadsassignmentslabreachlabel4Inputs */
/** @typedef {{}} Leadsassignmentslabreachnone4Inputs */
/** @typedef {{}} Leadsassignmentslabreachnotify4Inputs */
/** @typedef {{}} Leadsassignmentslabreachreassign4Inputs */
/** @typedef {{}} Leadsassignmentslalabel3Inputs */
/** @typedef {{}} Leadsassignmentstrategyfixed3Inputs */
/** @typedef {{}} Leadsassignmentstrategylabel3Inputs */
/** @typedef {{}} Leadsassignmentstrategyleastactive4Inputs */
/** @typedef {{}} Leadsassignmentstrategyroundrobin4Inputs */
/** @typedef {{}} Leadsbulkassignerrortoast4Inputs */
/** @typedef {{}} Leadsbulkassignto3Inputs */
/** @typedef {{}} Leadsbulkchangestage3Inputs */
/** @typedef {{}} Leadsbulkclearselection3Inputs */
/** @typedef {{}} Leadsbulkdelete2Inputs */
/** @typedef {{}} Leadsbulkdeleteconfirm3Inputs */
/** @typedef {{ count: NonNullable<unknown> }} Leadsbulkdeleteconfirmdescription4Inputs */
/** @typedef {{}} Leadsbulkdeleteconfirmtitle4Inputs */
/** @typedef {{}} Leadsbulkdeleteerrortoast4Inputs */
/** @typedef {{}} Leadsbulkexport2Inputs */
/** @typedef {{ count: NonNullable<unknown> }} Leadsbulkselectedcount3Inputs */
/** @typedef {{}} Leadsbulkstageerrortoast4Inputs */
/** @typedef {{}} Leadscampaignlabel2Inputs */
/** @typedef {{}} Leadscolumnage2Inputs */
/** @typedef {{}} Leadscolumnassignee2Inputs */
/** @typedef {{}} Leadscolumncreatedat3Inputs */
/** @typedef {{}} Leadscolumnname2Inputs */
/** @typedef {{}} Leadscolumnphone2Inputs */
/** @typedef {{}} Leadscolumnstage2Inputs */
/** @typedef {{}} Leadsdescription1Inputs */
/** @typedef {{}} Leadsdsraddbutton3Inputs */
/** @typedef {{}} Leadsdsrcancel2Inputs */
/** @typedef {{}} Leadsdsrcompleteaction3Inputs */
/** @typedef {{}} Leadsdsrcompleteerrortoast4Inputs */
/** @typedef {{}} Leadsdsrcreateerrortoast4Inputs */
/** @typedef {{}} Leadsdsrdialogtitle3Inputs */
/** @typedef {{}} Leadsdsrduelabel3Inputs */
/** @typedef {{}} Leadsdsrempty2Inputs */
/** @typedef {{ count: NonNullable<unknown> }} Leadsdsrindicatorlabel3Inputs */
/** @typedef {{}} Leadsdsrnoteslabel3Inputs */
/** @typedef {{}} Leadsdsrnotesplaceholder3Inputs */
/** @typedef {{}} Leadsdsroverduebadge3Inputs */
/** @typedef {{}} Leadsdsrreceivedatlabel4Inputs */
/** @typedef {{}} Leadsdsrsectionlabel3Inputs */
/** @typedef {{}} Leadsdsrstatuscompleted3Inputs */
/** @typedef {{}} Leadsdsrstatuspending3Inputs */
/** @typedef {{}} Leadsdsrsubmit2Inputs */
/** @typedef {{}} Leadsdsrtypedelete3Inputs */
/** @typedef {{}} Leadsdsrtypeexport3Inputs */
/** @typedef {{}} Leadsdsrtypelabel3Inputs */
/** @typedef {{}} Leadsemptytitle2Inputs */
/** @typedef {{}} Leadsexportbutton2Inputs */
/** @typedef {{}} Leadsexporterrortoast3Inputs */
/** @typedef {{}} Leadsfilterassigneelabel3Inputs */
/** @typedef {{}} Leadsfiltercampaignlabel3Inputs */
/** @typedef {{}} Leadsfilterdatefromlabel4Inputs */
/** @typedef {{}} Leadsfilterdatetolabel4Inputs */
/** @typedef {{}} Leadsfilterpaidlabel3Inputs */
/** @typedef {{}} Leadsfilterpaidno3Inputs */
/** @typedef {{}} Leadsfilterpaidyes3Inputs */
/** @typedef {{}} Leadsfilterproductlabel3Inputs */
/** @typedef {{}} Leadsfilterrepeatcustomerlabel4Inputs */
/** @typedef {{}} Leadsfiltersearchlabel3Inputs */
/** @typedef {{}} Leadsfiltersearchplaceholder3Inputs */
/** @typedef {{}} Leadsfilterutmsourcelabel4Inputs */
/** @typedef {{}} Leadsfilterutmsourceplaceholder4Inputs */
/** @typedef {{}} Leadsimportbutton2Inputs */
/** @typedef {{}} Leadsimportdialogdescription3Inputs */
/** @typedef {{}} Leadsimportdialogtitle3Inputs */
/** @typedef {{}} Leadsimporterrortoast3Inputs */
/** @typedef {{}} Leadsimportmappingcustomfields4Inputs */
/** @typedef {{}} Leadsimportmappingemail3Inputs */
/** @typedef {{}} Leadsimportmappingfullname4Inputs */
/** @typedef {{}} Leadsimportmappingpersona3Inputs */
/** @typedef {{}} Leadsimportmappingphone3Inputs */
/** @typedef {{}} Leadsimportmappingskip3Inputs */
/** @typedef {{}} Leadsimportnorowserror4Inputs */
/** @typedef {{ rows: NonNullable<unknown>, shown: NonNullable<unknown> }} Leadsimportpreviewtitle3Inputs */
/** @typedef {{ count: NonNullable<unknown> }} Leadsimportskippedrowstoast4Inputs */
/** @typedef {{}} Leadsimportsubmit2Inputs */
/** @typedef {{ created: NonNullable<unknown>, merged: NonNullable<unknown> }} Leadsimportsuccesstoast3Inputs */
/** @typedef {{}} Leadsimportuploadlabel3Inputs */
/** @typedef {{}} Leadskanbanemptycolumn3Inputs */
/** @typedef {{}} Leadsloaderrortitle3Inputs */
/** @typedef {{}} Leadsnoteerrortoast3Inputs */
/** @typedef {{ count: NonNullable<unknown> }} Leadsnotificationsbelllabel3Inputs */
/** @typedef {{}} Leadsnotifychannelemailoption4Inputs */
/** @typedef {{}} Leadsnotifychannellabel3Inputs */
/** @typedef {{}} Leadsnotifychannelsmsoption4Inputs */
/** @typedef {{}} Leadsnotifychannelznsoption4Inputs */
/** @typedef {{}} Leadsnotifyesmsapikeylabel5Inputs */
/** @typedef {{}} Leadsnotifyesmsapikeyplaceholder5Inputs */
/** @typedef {{}} Leadsnotifyesmsbrandnamelabel4Inputs */
/** @typedef {{}} Leadsnotifyesmsbrandnameplaceholder4Inputs */
/** @typedef {{}} Leadsnotifyesmsdescription3Inputs */
/** @typedef {{}} Leadsnotifyesmssecretkeylabel5Inputs */
/** @typedef {{}} Leadsnotifyesmssecretkeyplaceholder5Inputs */
/** @typedef {{}} Leadsnotifyesmstitle3Inputs */
/** @typedef {{}} Leadsnotifyphonelabel3Inputs */
/** @typedef {{}} Leadsnotifyphoneplaceholder3Inputs */
/** @typedef {{}} Leadsnotifysettingsdescription3Inputs */
/** @typedef {{}} Leadsnotifysettingsmanagebutton4Inputs */
/** @typedef {{}} Leadsnotifysettingstitle3Inputs */
/** @typedef {{}} Leadsnotifyznsaccesstokenlabel5Inputs */
/** @typedef {{}} Leadsnotifyznsaccesstokenplaceholder5Inputs */
/** @typedef {{}} Leadsnotifyznsdescription3Inputs */
/** @typedef {{}} Leadsnotifyznstemplateidlabel5Inputs */
/** @typedef {{}} Leadsnotifyznstemplateidplaceholder5Inputs */
/** @typedef {{}} Leadsnotifyznstitle3Inputs */
/** @typedef {{}} Leadsorderawaitingconfirmation3Inputs */
/** @typedef {{}} Leadsordercancel2Inputs */
/** @typedef {{}} Leadsorderconfirmpayment3Inputs */
/** @typedef {{}} Leadsordercountlabel3Inputs */
/** @typedef {{}} Leadsordererrortoast3Inputs */
/** @typedef {{}} Leadsorderfulfill2Inputs */
/** @typedef {{}} Leadsorderreasoncancel3Inputs */
/** @typedef {{}} Leadsorderreasonconfirm3Inputs */
/** @typedef {{}} Leadsorderreasonplaceholder3Inputs */
/** @typedef {{}} Leadsorderreasontitle3Inputs */
/** @typedef {{}} Leadsorderslabel2Inputs */
/** @typedef {{}} Leadsrepeatcustomerbadge3Inputs */
/** @typedef {{}} Leadssavedviewdeletelabel4Inputs */
/** @typedef {{}} Leadssavedviewnamelabel4Inputs */
/** @typedef {{}} Leadssavedviewnameplaceholder4Inputs */
/** @typedef {{}} Leadssavedviewsavebutton4Inputs */
/** @typedef {{}} Leadssavedviewsaveconfirm4Inputs */
/** @typedef {{}} Leadssavedviewsaveerrortoast5Inputs */
/** @typedef {{}} Leadssavedviewsharelabel4Inputs */
/** @typedef {{}} Leadsstagechangeerrortoast4Inputs */
/** @typedef {{}} Leadsstagefilteractive3Inputs */
/** @typedef {{}} Leadsstagefilterclear3Inputs */
/** @typedef {{}} Leadssubnavassignmentrules4Inputs */
/** @typedef {{}} Leadssubnavnotifications3Inputs */
/** @typedef {{}} Leadssubnavoverview3Inputs */
/** @typedef {{}} Leadssubnavwebhooks3Inputs */
/** @typedef {{}} Leadstimelinelabel2Inputs */
/** @typedef {{}} Leadstitle1Inputs */
/** @typedef {{}} Leadstotalpaidlabel3Inputs */
/** @typedef {{}} Leadsunassigned1Inputs */
/** @typedef {{}} Leadsunreadindicatorlabel3Inputs */
/** @typedef {{}} Leadsviewkanban2Inputs */
/** @typedef {{}} Leadsviewlist2Inputs */
/** @typedef {{}} Leadswebhookcampaignlabel3Inputs */
/** @typedef {{}} Leadswebhookcampaignplaceholder3Inputs */
/** @typedef {{}} Leadswebhookcarddescription3Inputs */
/** @typedef {{}} Leadswebhookconfiguredbadge3Inputs */
/** @typedef {{}} Leadswebhookgenericdescription3Inputs */
/** @typedef {{}} Leadswebhookgenericgeneratebutton4Inputs */
/** @typedef {{}} Leadswebhookgenerickeyrevealedlabel5Inputs */
/** @typedef {{}} Leadswebhookgenerickeyrevealedwarning5Inputs */
/** @typedef {{}} Leadswebhookgenericrevokebutton4Inputs */
/** @typedef {{}} Leadswebhookgenericrotatebutton4Inputs */
/** @typedef {{}} Leadswebhookgenerictitle3Inputs */
/** @typedef {{}} Leadswebhookgoogleadsdescription4Inputs */
/** @typedef {{}} Leadswebhookgoogleadstitle4Inputs */
/** @typedef {{}} Leadswebhooklastusedlabel4Inputs */
/** @typedef {{}} Leadswebhookmissingbadge3Inputs */
/** @typedef {{}} Leadswebhookneverusedlabel4Inputs */
/** @typedef {{}} Leadswebhooknocampaignwarning4Inputs */
/** @typedef {{}} Leadswebhooknocampaignsempty4Inputs */
/** @typedef {{}} Leadswebhookpageaccesstokendescription5Inputs */
/** @typedef {{}} Leadswebhookpageaccesstokenlabel5Inputs */
/** @typedef {{}} Leadswebhookpageaccesstokenplaceholder5Inputs */
/** @typedef {{}} Leadswebhookremovebutton3Inputs */
/** @typedef {{}} Leadswebhookremovedtoast3Inputs */
/** @typedef {{}} Leadswebhooksaveerrortoast4Inputs */
/** @typedef {{}} Leadswebhooksavedtoast3Inputs */
/** @typedef {{}} Leadswebhooksecretlabel3Inputs */
/** @typedef {{}} Leadswebhooksecretplaceholder3Inputs */
/** @typedef {{}} Leadswebhooksecretrotateplaceholder4Inputs */
/** @typedef {{}} Leadswebhooksettingsdescription3Inputs */
/** @typedef {{}} Leadswebhooksettingsmanagebutton4Inputs */
/** @typedef {{}} Leadswebhooksettingstitle3Inputs */
/** @typedef {{}} Leadswebhooksharedsecretbadge4Inputs */
/** @typedef {{}} Leadswebhooktiktokadvertiserlabel4Inputs */
/** @typedef {{}} Leadswebhooktiktokconnectbutton4Inputs */
/** @typedef {{}} Leadswebhooktiktokdescription3Inputs */
/** @typedef {{}} Leadswebhooktiktokreconnectbutton4Inputs */
/** @typedef {{}} Leadswebhooktiktoktitle3Inputs */
/** @typedef {{}} Leadswebhookurllabel3Inputs */
/** @typedef {{}} Leadswebhookverifytokenlabel4Inputs */
/** @typedef {{}} Leadswebhookverifytokenplaceholder4Inputs */
/** @typedef {{}} Loginemaillabel2Inputs */
/** @typedef {{}} Loginerror1Inputs */
/** @typedef {{}} Loginforgotpassword2Inputs */
/** @typedef {{}} Loginnoaccount2Inputs */
/** @typedef {{}} Loginpasswordlabel2Inputs */
/** @typedef {{}} Loginsignuplink2Inputs */
/** @typedef {{}} Loginsubmit1Inputs */
/** @typedef {{}} Logintitle1Inputs */
/** @typedef {{}} Memberscancelinviteconfirmaction4Inputs */
/** @typedef {{}} Memberscancelinviteconfirmbody4Inputs */
/** @typedef {{ email: NonNullable<unknown> }} Memberscancelinviteconfirmtitle4Inputs */
/** @typedef {{ email: NonNullable<unknown> }} Memberscancelinvitesuccesstoast4Inputs */
/** @typedef {{}} Memberscolumnactions2Inputs */
/** @typedef {{}} Memberscolumnemail2Inputs */
/** @typedef {{}} Memberscolumnexpires2Inputs */
/** @typedef {{}} Memberscolumnmember2Inputs */
/** @typedef {{}} Memberscolumnrole2Inputs */
/** @typedef {{}} Memberscolumnseeallleads4Inputs */
/** @typedef {{}} Membersdescription1Inputs */
/** @typedef {{}} Membersemaillabel2Inputs */
/** @typedef {{}} Membersinvitationaccept2Inputs */
/** @typedef {{ org: NonNullable<unknown>, role: NonNullable<unknown> }} Membersinvitationbannertext3Inputs */
/** @typedef {{}} Membersinvitationexpiredbadge3Inputs */
/** @typedef {{}} Membersinvitationreject2Inputs */
/** @typedef {{}} Membersinvitationresponderrortoast4Inputs */
/** @typedef {{}} Membersinvitebutton2Inputs */
/** @typedef {{}} Membersinvitedialogdescription3Inputs */
/** @typedef {{}} Membersinvitedialogtitle3Inputs */
/** @typedef {{}} Membersinvitesubmit2Inputs */
/** @typedef {{ email: NonNullable<unknown> }} Membersinvitesuccesstoast3Inputs */
/** @typedef {{}} Membersloaderrortitle3Inputs */
/** @typedef {{}} Memberspendingdescription2Inputs */
/** @typedef {{}} Memberspendingemptytitle3Inputs */
/** @typedef {{}} Memberspendingtitle2Inputs */
/** @typedef {{}} Membersremoveconfirmaction3Inputs */
/** @typedef {{}} Membersremoveconfirmbody3Inputs */
/** @typedef {{ member: NonNullable<unknown> }} Membersremoveconfirmtitle3Inputs */
/** @typedef {{ member: NonNullable<unknown> }} Membersremovesuccesstoast3Inputs */
/** @typedef {{}} Membersrolelabel2Inputs */
/** @typedef {{}} Membersseeallleadserrortoast5Inputs */
/** @typedef {{}} Memberstitle1Inputs */
/** @typedef {{}} Offerworkspaceback2Inputs */
/** @typedef {{}} Offerworkspacecampaign2Inputs */
/** @typedef {{}} Offerworkspacecontentmissing3Inputs */
/** @typedef {{}} Offerworkspacecontentready3Inputs */
/** @typedef {{}} Offerworkspacecontenttitle3Inputs */
/** @typedef {{}} Offerworkspacedescription2Inputs */
/** @typedef {{}} Offerworkspacedraft2Inputs */
/** @typedef {{}} Offerworkspaceeditoraction3Inputs */
/** @typedef {{}} Offerworkspaceemptytitle3Inputs */
/** @typedef {{}} Offerworkspaceeyebrow2Inputs */
/** @typedef {{}} Offerworkspaceinboxaction3Inputs */
/** @typedef {{}} Offerworkspaceloaderrortitle4Inputs */
/** @typedef {{}} Offerworkspacenexttitle3Inputs */
/** @typedef {{}} Offerworkspacenocampaign3Inputs */
/** @typedef {{}} Offerworkspaceready2Inputs */
/** @typedef {{}} Offerworkspacestepcontent3Inputs */
/** @typedef {{}} Offerworkspacestepleadform4Inputs */
/** @typedef {{}} Offerworkspacesteppublish3Inputs */
/** @typedef {{}} Offerworkspaceversion2Inputs */
/** @typedef {{}} Offerssurfacenotice2Inputs */
/** @typedef {{}} Onboardingbody1Inputs */
/** @typedef {{}} Onboardingchecklistdismiss2Inputs */
/** @typedef {{ done: NonNullable<unknown>, total: NonNullable<unknown> }} Onboardingchecklistsubtitle2Inputs */
/** @typedef {{}} Onboardingchecklisttitle2Inputs */
/** @typedef {{}} Onboardingcreateerror2Inputs */
/** @typedef {{}} Onboardingdeliverylabel2Inputs */
/** @typedef {{}} Onboardingdeliverylink2Inputs */
/** @typedef {{}} Onboardingdeliverymanual2Inputs */
/** @typedef {{}} Onboardingdeliveryschedule2Inputs */
/** @typedef {{}} Onboardingdeliveryzalo2Inputs */
/** @typedef {{}} Onboardingdescriptionlabel2Inputs */
/** @typedef {{}} Onboardingdescriptionplaceholder2Inputs */
/** @typedef {{}} Onboardingorgnamehelp3Inputs */
/** @typedef {{}} Onboardingorgnamelabel3Inputs */
/** @typedef {{}} Onboardingorgnameplaceholder3Inputs */
/** @typedef {{}} Onboardingpricehelp2Inputs */
/** @typedef {{}} Onboardingpricelabel2Inputs */
/** @typedef {{}} Onboardingprivacynote2Inputs */
/** @typedef {{}} Onboardingproductnamelabel3Inputs */
/** @typedef {{}} Onboardingproductnameplaceholder3Inputs */
/** @typedef {{}} Onboardingproducttypelabel3Inputs */
/** @typedef {{}} Onboardingsetupdescription2Inputs */
/** @typedef {{}} Onboardingsetuptitle2Inputs */
/** @typedef {{}} Onboardingstepconnectai3Inputs */
/** @typedef {{}} Onboardingstepconnectpayment3Inputs */
/** @typedef {{}} Onboardingstepcreatelanding3Inputs */
/** @typedef {{}} Onboardingstepcreateproduct3Inputs */
/** @typedef {{}} Onboardingsteppublishlanding3Inputs */
/** @typedef {{}} Onboardingsubmit1Inputs */
/** @typedef {{}} Onboardingtitle1Inputs */
/** @typedef {{}} Orgswitchercreateorg3Inputs */
/** @typedef {{}} Orgswitchernoorg3Inputs */
/** @typedef {{}} Orgswitcherswitcherror3Inputs */
/** @typedef {{}} Paymentaccountnamelabel3Inputs */
/** @typedef {{}} Paymentaccountnumberlabel3Inputs */
/** @typedef {{}} Paymentapikeylabel3Inputs */
/** @typedef {{}} Paymentbankbinlabel3Inputs */
/** @typedef {{}} Paymentconnectbutton2Inputs */
/** @typedef {{}} Paymentconnectdialogdescription3Inputs */
/** @typedef {{}} Paymentconnectdialogtitle3Inputs */
/** @typedef {{}} Paymentconnecterrortoast3Inputs */
/** @typedef {{}} Paymentconnectsubmit2Inputs */
/** @typedef {{}} Paymentconnectioncolumnaccount3Inputs */
/** @typedef {{}} Paymentconnectioncolumnactions3Inputs */
/** @typedef {{}} Paymentconnectioncolumnbank3Inputs */
/** @typedef {{}} Paymentconnectioncolumnstatus3Inputs */
/** @typedef {{}} Paymentconnectionsdescription2Inputs */
/** @typedef {{}} Paymentconnectionsemptytitle3Inputs */
/** @typedef {{}} Paymentconnectionsloaderrortitle4Inputs */
/** @typedef {{}} Paymentconnectionstitle2Inputs */
/** @typedef {{}} Paymentguidedescription2Inputs */
/** @typedef {{}} Paymentguideloaderrortitle4Inputs */
/** @typedef {{}} Paymentguidetitle2Inputs */
/** @typedef {{}} Paymentremoveconfirmbody3Inputs */
/** @typedef {{}} Paymentremoveconfirmtitle3Inputs */
/** @typedef {{}} Paymentremoveconnectionaction3Inputs */
/** @typedef {{}} Paymentremoveconnectionerrortoast4Inputs */
/** @typedef {{}} Platformactiondonetoast3Inputs */
/** @typedef {{}} Platformactionerrortoast3Inputs */
/** @typedef {{}} Platformauditcolumnaction3Inputs */
/** @typedef {{}} Platformauditcolumnmeta3Inputs */
/** @typedef {{}} Platformauditcolumnstaff3Inputs */
/** @typedef {{}} Platformauditcolumntime3Inputs */
/** @typedef {{}} Platformauditdescription2Inputs */
/** @typedef {{}} Platformauditempty2Inputs */
/** @typedef {{}} Platformbacktoorgs3Inputs */
/** @typedef {{}} Platformcancel1Inputs */
/** @typedef {{}} Platformconfirm1Inputs */
/** @typedef {{}} Platformdisableorgaction3Inputs */
/** @typedef {{}} Platformdisableorgdescription3Inputs */
/** @typedef {{}} Platformenableorgaction3Inputs */
/** @typedef {{}} Platformenableorgdescription3Inputs */
/** @typedef {{}} Platformfeatureforceoff3Inputs */
/** @typedef {{}} Platformfeatureforceon3Inputs */
/** @typedef {{}} Platformfeatureplanexcluded3Inputs */
/** @typedef {{}} Platformfeatureplanincluded3Inputs */
/** @typedef {{}} Platformfeaturesempty2Inputs */
/** @typedef {{}} Platformfeatureshint2Inputs */
/** @typedef {{}} Platformfeatureslabel2Inputs */
/** @typedef {{}} Platformorgdetailloaderrortitle5Inputs */
/** @typedef {{}} Platformorgscolumnaicredit4Inputs */
/** @typedef {{}} Platformorgscolumncreated3Inputs */
/** @typedef {{}} Platformorgscolumnname3Inputs */
/** @typedef {{}} Platformorgscolumnowner3Inputs */
/** @typedef {{}} Platformorgscolumnplan3Inputs */
/** @typedef {{}} Platformorgscolumnstatus3Inputs */
/** @typedef {{}} Platformorgsdescription2Inputs */
/** @typedef {{}} Platformorgsemptytitle3Inputs */
/** @typedef {{}} Platformorgsloaderrortitle4Inputs */
/** @typedef {{}} Platformorgsloading2Inputs */
/** @typedef {{}} Platformorgssearchplaceholder3Inputs */
/** @typedef {{}} Platformorgsstatusactive3Inputs */
/** @typedef {{}} Platformorgsstatusall3Inputs */
/** @typedef {{}} Platformorgsstatusdisabled3Inputs */
/** @typedef {{}} Platformorgstitle2Inputs */
/** @typedef {{}} Platformplanlabel2Inputs */
/** @typedef {{}} Platformreasonlabel2Inputs */
/** @typedef {{}} Platformreasonplaceholder2Inputs */
/** @typedef {{}} Platformreasonsharedhint3Inputs */
/** @typedef {{}} Platformrefundassistaction3Inputs */
/** @typedef {{}} Platformrefundassistdescription3Inputs */
/** @typedef {{}} Platformrefundassistdonetoast4Inputs */
/** @typedef {{}} Platformrefundassisttitle3Inputs */
/** @typedef {{}} Platformrefundorderidplaceholder4Inputs */
/** @typedef {{}} Platformstaffaddaction3Inputs */
/** @typedef {{}} Platformstaffaddemailplaceholder4Inputs */
/** @typedef {{}} Platformstaffadderrortoast4Inputs */
/** @typedef {{ email: NonNullable<unknown> }} Platformstaffaddsuccesstoast4Inputs */
/** @typedef {{}} Platformstaffcolumnactions3Inputs */
/** @typedef {{}} Platformstaffcolumncreated3Inputs */
/** @typedef {{}} Platformstaffcolumnemail3Inputs */
/** @typedef {{}} Platformstaffcolumnrole3Inputs */
/** @typedef {{}} Platformstaffdescription2Inputs */
/** @typedef {{}} Platformstaffemptytitle3Inputs */
/** @typedef {{}} Platformstaffloaderrortitle4Inputs */
/** @typedef {{}} Platformstaffloading2Inputs */
/** @typedef {{}} Platformstaffremoveaction3Inputs */
/** @typedef {{}} Platformstaffremoveconfirmaction4Inputs */
/** @typedef {{}} Platformstaffremoveconfirmbody4Inputs */
/** @typedef {{ email: NonNullable<unknown> }} Platformstaffremoveconfirmtitle4Inputs */
/** @typedef {{}} Platformstaffremoveerrortoast4Inputs */
/** @typedef {{ email: NonNullable<unknown> }} Platformstaffremovesuccesstoast4Inputs */
/** @typedef {{}} Platformstataicreditbalance4Inputs */
/** @typedef {{}} Platformstataicreditspent4Inputs */
/** @typedef {{}} Platformstatcampaigns2Inputs */
/** @typedef {{}} Platformstatcreated2Inputs */
/** @typedef {{}} Platformstatleads2Inputs */
/** @typedef {{}} Platformstatmembers2Inputs */
/** @typedef {{}} Platformstattrialuses3Inputs */
/** @typedef {{}} Platformsubscriptiondescription2Inputs */
/** @typedef {{}} Platformsubscriptionsave2Inputs */
/** @typedef {{}} Platformsubscriptiontitle2Inputs */
/** @typedef {{}} Platformtabaudit2Inputs */
/** @typedef {{}} Platformtabbilling2Inputs */
/** @typedef {{}} Platformtaborgs2Inputs */
/** @typedef {{}} Platformtaboverview2Inputs */
/** @typedef {{}} Platformtabstaff2Inputs */
/** @typedef {{}} Productsaddbutton2Inputs */
/** @typedef {{}} Productsadddialogtitle3Inputs */
/** @typedef {{}} Productscolumnactions2Inputs */
/** @typedef {{}} Productscolumnname2Inputs */
/** @typedef {{}} Productscolumnprice2Inputs */
/** @typedef {{}} Productscolumntype2Inputs */
/** @typedef {{}} Productscourseactivationlabel3Inputs */
/** @typedef {{}} Productscourseclassschedulelabel4Inputs */
/** @typedef {{}} Productscoursefieldstitle3Inputs */
/** @typedef {{}} Productscoursestartsatlabel4Inputs */
/** @typedef {{}} Productscoursezalolabel3Inputs */
/** @typedef {{}} Productsdescription1Inputs */
/** @typedef {{}} Productsdescriptionlabel2Inputs */
/** @typedef {{}} Productsdialogdescription2Inputs */
/** @typedef {{}} Productseditdialogtitle3Inputs */
/** @typedef {{}} Productsemptytitle2Inputs */
/** @typedef {{}} Productsimagelabel2Inputs */
/** @typedef {{}} Productsloaderrortitle3Inputs */
/** @typedef {{}} Productsnamelabel2Inputs */
/** @typedef {{}} Productspricelabel2Inputs */
/** @typedef {{}} Productsremoveconfirmaction3Inputs */
/** @typedef {{}} Productsremoveconfirmbody3Inputs */
/** @typedef {{ name: NonNullable<unknown> }} Productsremoveconfirmtitle3Inputs */
/** @typedef {{}} Productsremoveerrortoast3Inputs */
/** @typedef {{}} Productssaveerrortoast3Inputs */
/** @typedef {{}} Productssavesubmit2Inputs */
/** @typedef {{}} Productstitle1Inputs */
/** @typedef {{}} Productstypecourse2Inputs */
/** @typedef {{}} Productstypelabel2Inputs */
/** @typedef {{}} Productstypeother2Inputs */
/** @typedef {{}} Productstypeproduct2Inputs */
/** @typedef {{}} Productstypeservice2Inputs */
/** @typedef {{}} Promptlibrarycopiedtoast3Inputs */
/** @typedef {{}} Promptlibrarydescription2Inputs */
/** @typedef {{}} Promptlibrarydevicedesktop3Inputs */
/** @typedef {{}} Promptlibrarydevicemobile3Inputs */
/** @typedef {{}} Promptlibrarydevicetablet3Inputs */
/** @typedef {{}} Promptlibraryemptytitle3Inputs */
/** @typedef {{}} Promptlibraryfullpromptsection4Inputs */
/** @typedef {{}} Promptlibrarygeneratecta3Inputs */
/** @typedef {{}} Promptlibrarygeneratedialogdescription4Inputs */
/** @typedef {{}} Promptlibrarygeneratedialogtitle4Inputs */
/** @typedef {{}} Promptlibrarygeneratenamelabel4Inputs */
/** @typedef {{}} Promptlibrarygeneratepromptlabel4Inputs */
/** @typedef {{}} Promptlibrarygeneratesubmit3Inputs */
/** @typedef {{}} Promptlibrarygeneratesubmitting3Inputs */
/** @typedef {{}} Promptlibraryimportcta3Inputs */
/** @typedef {{}} Promptlibraryloaderrortitle4Inputs */
/** @typedef {{}} Promptlibrarynopreview3Inputs */
/** @typedef {{}} Promptlibrarypreviewsection3Inputs */
/** @typedef {{}} Promptlibrarysearchplaceholder3Inputs */
/** @typedef {{}} Promptlibrarysummarysection3Inputs */
/** @typedef {{}} Promptlibrarytitle2Inputs */
/** @typedef {{}} Promptlibraryusetemplatecta4Inputs */
/** @typedef {{}} Promptlibraryusetemplatesubmitting4Inputs */
/** @typedef {{}} Prompttemplateaddsection3Inputs */
/** @typedef {{}} Prompttemplateaddvariable3Inputs */
/** @typedef {{}} Prompttemplatecolumnslug3Inputs */
/** @typedef {{}} Prompttemplatecolumntype3Inputs */
/** @typedef {{}} Prompttemplatecolumnversion3Inputs */
/** @typedef {{}} Prompttemplatecompareheading3Inputs */
/** @typedef {{}} Prompttemplatecompareselect3Inputs */
/** @typedef {{}} Prompttemplatecompilebutton3Inputs */
/** @typedef {{}} Prompttemplatecompileerrortoast4Inputs */
/** @typedef {{}} Prompttemplateconnectionlabel3Inputs */
/** @typedef {{}} Prompttemplateconnectionplatform3Inputs */
/** @typedef {{}} Prompttemplateconnectiontrial3Inputs */
/** @typedef {{}} Prompttemplatecreatebutton3Inputs */
/** @typedef {{}} Prompttemplatecreatedialogdescription4Inputs */
/** @typedef {{}} Prompttemplatecreatedialogtitle4Inputs */
/** @typedef {{}} Prompttemplatehistoryemptybody4Inputs */
/** @typedef {{}} Prompttemplatehistoryemptytitle4Inputs */
/** @typedef {{}} Prompttemplatehistoryheading3Inputs */
/** @typedef {{}} Prompttemplatelighthouseunavailable3Inputs */
/** @typedef {{}} Prompttemplatenotfoundtitle4Inputs */
/** @typedef {{}} Prompttemplateopenaction3Inputs */
/** @typedef {{}} Prompttemplateremoveaction3Inputs */
/** @typedef {{}} Prompttemplateremoveconfirmbody4Inputs */
/** @typedef {{}} Prompttemplateremoveconfirmtitle4Inputs */
/** @typedef {{}} Prompttemplateremoveerrortoast4Inputs */
/** @typedef {{}} Prompttemplateremovesection3Inputs */
/** @typedef {{}} Prompttemplateremovevariable3Inputs */
/** @typedef {{}} Prompttemplateruntestbutton4Inputs */
/** @typedef {{}} Prompttemplatesaveerrortoast4Inputs */
/** @typedef {{}} Prompttemplatesectionkeyplaceholder4Inputs */
/** @typedef {{}} Prompttemplatesectionslabel3Inputs */
/** @typedef {{}} Prompttemplatesluglabel3Inputs */
/** @typedef {{}} Prompttemplatetabcompile3Inputs */
/** @typedef {{}} Prompttemplatetabedit3Inputs */
/** @typedef {{}} Prompttemplatetabtestbench4Inputs */
/** @typedef {{}} Prompttemplatetestrunerrortoast5Inputs */
/** @typedef {{}} Prompttemplatetestrunheading4Inputs */
/** @typedef {{}} Prompttemplatevariablekeyplaceholder4Inputs */
/** @typedef {{}} Prompttemplatevariablelabelplaceholder4Inputs */
/** @typedef {{}} Prompttemplatevariablerequired3Inputs */
/** @typedef {{}} Prompttemplatevariableslabel3Inputs */
/** @typedef {{}} Prompttemplatesdescription2Inputs */
/** @typedef {{}} Prompttemplatesemptytitle3Inputs */
/** @typedef {{}} Prompttemplatesloaderrortitle4Inputs */
/** @typedef {{}} Prompttemplatestitle2Inputs */
/** @typedef {{}} Reconciliationcolumnamount2Inputs */
/** @typedef {{}} Reconciliationcolumncandidates2Inputs */
/** @typedef {{}} Reconciliationcolumnreason2Inputs */
/** @typedef {{}} Reconciliationcolumntransaction2Inputs */
/** @typedef {{}} Reconciliationdescription1Inputs */
/** @typedef {{}} Reconciliationdismissaction2Inputs */
/** @typedef {{}} Reconciliationdismissconfirmbody3Inputs */
/** @typedef {{}} Reconciliationdismissconfirmtitle3Inputs */
/** @typedef {{}} Reconciliationdismisserrortoast3Inputs */
/** @typedef {{}} Reconciliationemptytitle2Inputs */
/** @typedef {{}} Reconciliationloaderrortitle3Inputs */
/** @typedef {{}} Reconciliationreasonalreadypaid3Inputs */
/** @typedef {{}} Reconciliationreasonambiguous2Inputs */
/** @typedef {{}} Reconciliationreasonnocandidate3Inputs */
/** @typedef {{}} Reconciliationresolveerrortoast3Inputs */
/** @typedef {{}} Reconciliationsearchbutton2Inputs */
/** @typedef {{}} Reconciliationsearcherrortoast3Inputs */
/** @typedef {{}} Reconciliationsearchnoresults3Inputs */
/** @typedef {{}} Reconciliationsearchplaceholder2Inputs */
/** @typedef {{}} Reconciliationselectorderaction3Inputs */
/** @typedef {{}} Reconciliationsummaryalreadypaid3Inputs */
/** @typedef {{}} Reconciliationsummaryambiguous2Inputs */
/** @typedef {{}} Reconciliationsummarynocandidate3Inputs */
/** @typedef {{}} Reconciliationsummarytitle2Inputs */
/** @typedef {{}} Reconciliationtitle1Inputs */
/** @typedef {{}} Refundrequestscolumnamount3Inputs */
/** @typedef {{}} Refundrequestscolumnlead3Inputs */
/** @typedef {{}} Refundrequestscolumnorder3Inputs */
/** @typedef {{}} Refundrequestscolumnreason3Inputs */
/** @typedef {{}} Refundrequestscolumnstatus3Inputs */
/** @typedef {{}} Refundrequestscompleteerrortoast4Inputs */
/** @typedef {{}} Refundrequestsdescription2Inputs */
/** @typedef {{}} Refundrequestsduplicatebadge3Inputs */
/** @typedef {{}} Refundrequestsemptytitle3Inputs */
/** @typedef {{}} Refundrequestsevidencelabel3Inputs */
/** @typedef {{}} Refundrequestsfilterall3Inputs */
/** @typedef {{}} Refundrequestsloaderrortitle4Inputs */
/** @typedef {{}} Refundrequestsmarktransferredbutton4Inputs */
/** @typedef {{}} Refundrequestsreasoncustomer3Inputs */
/** @typedef {{}} Refundrequestsreasonduplicatepayment4Inputs */
/** @typedef {{}} Refundrequestsreasonother3Inputs */
/** @typedef {{}} Refundrequestsreasonwrongmatch4Inputs */
/** @typedef {{}} Refundrequestsrejectbutton3Inputs */
/** @typedef {{}} Refundrequestsrejectconfirmaction4Inputs */
/** @typedef {{}} Refundrequestsrejectconfirmbody4Inputs */
/** @typedef {{}} Refundrequestsrejectconfirmtitle4Inputs */
/** @typedef {{}} Refundrequestsrejecterrortoast4Inputs */
/** @typedef {{}} Refundrequestsremitterlabel3Inputs */
/** @typedef {{}} Refundrequestsstatuscompleted3Inputs */
/** @typedef {{}} Refundrequestsstatuspending3Inputs */
/** @typedef {{}} Refundrequestsstatusprocessing3Inputs */
/** @typedef {{}} Refundrequestsstatusrejected3Inputs */
/** @typedef {{}} Refundrequeststitle2Inputs */
/** @typedef {{}} Refundrequestsuploaderrortoast4Inputs */
/** @typedef {{}} Refundrequestsuploadevidencebutton4Inputs */
/** @typedef {{}} Resetpasswordinvalidlink3Inputs */
/** @typedef {{}} Resetpasswordnewlabel3Inputs */
/** @typedef {{}} Resetpasswordsubmit2Inputs */
/** @typedef {{}} Resetpasswordsuccess2Inputs */
/** @typedef {{}} Resetpasswordtitle2Inputs */
/** @typedef {{}} Roleadmin1Inputs */
/** @typedef {{}} Roleadmindescription2Inputs */
/** @typedef {{}} Roleeditor1Inputs */
/** @typedef {{}} Roleeditordescription2Inputs */
/** @typedef {{}} Roleowner1Inputs */
/** @typedef {{}} Roleownerdescription2Inputs */
/** @typedef {{}} Rolesales1Inputs */
/** @typedef {{}} Rolesalesdescription2Inputs */
/** @typedef {{}} Routeerrordescription2Inputs */
/** @typedef {{}} Routeerrortitle2Inputs */
/** @typedef {{}} Routenotfoundbackbutton4Inputs */
/** @typedef {{}} Routenotfounddescription3Inputs */
/** @typedef {{}} Routenotfoundtitle3Inputs */
/** @typedef {{}} Settingsbrandbodyfontlabel4Inputs */
/** @typedef {{}} Settingsbrandheadingfontlabel4Inputs */
/** @typedef {{}} Settingsbrandkitdescription3Inputs */
/** @typedef {{}} Settingsbrandkittitle3Inputs */
/** @typedef {{}} Settingsbrandprimarycolorlabel4Inputs */
/** @typedef {{}} Settingsbrandsecondarycolorlabel4Inputs */
/** @typedef {{}} Settingsdescription1Inputs */
/** @typedef {{}} Settingsforbiddentitle2Inputs */
/** @typedef {{}} Settingsleaddigestfrequencydaily4Inputs */
/** @typedef {{}} Settingsleaddigestfrequencydescription4Inputs */
/** @typedef {{}} Settingsleaddigestfrequencyhourly4Inputs */
/** @typedef {{}} Settingsleaddigestfrequencylabel4Inputs */
/** @typedef {{}} Settingsloaderrortitle3Inputs */
/** @typedef {{}} Settingslogodescription2Inputs */
/** @typedef {{}} Settingslogolabel2Inputs */
/** @typedef {{}} Settingssaveerrortoast3Inputs */
/** @typedef {{}} Settingstitle1Inputs */
/** @typedef {{}} Shellainav2Inputs */
/** @typedef {{}} Shellcampaignsnav2Inputs */
/** @typedef {{}} Shellcommandpaletteempty3Inputs */
/** @typedef {{}} Shellcommandpaletteinputplaceholder4Inputs */
/** @typedef {{}} Shelldomainsnav2Inputs */
/** @typedef {{}} Shellgroupadvanced2Inputs */
/** @typedef {{}} Shellgroupai2Inputs */
/** @typedef {{}} Shellgroupcontent2Inputs */
/** @typedef {{}} Shellgrouporganization2Inputs */
/** @typedef {{}} Shellgrouppayments2Inputs */
/** @typedef {{}} Shellgroupsales2Inputs */
/** @typedef {{}} Shellgroupsetup2Inputs */
/** @typedef {{}} Shellgroupworkspace2Inputs */
/** @typedef {{}} Shelllandingsnav2Inputs */
/** @typedef {{}} Shellleadsnav2Inputs */
/** @typedef {{}} Shelllogout1Inputs */
/** @typedef {{}} Shellmembersnav2Inputs */
/** @typedef {{}} Shelloffersnav2Inputs */
/** @typedef {{}} Shellordersnav2Inputs */
/** @typedef {{}} Shellpaymentsnav2Inputs */
/** @typedef {{}} Shellproductsnav2Inputs */
/** @typedef {{}} Shellpromptlibrarynav3Inputs */
/** @typedef {{}} Shellprompttemplatesnav3Inputs */
/** @typedef {{}} Shellreconciliationnav2Inputs */
/** @typedef {{}} Shellrefundrequestsnav3Inputs */
/** @typedef {{}} Shellsearchplaceholder2Inputs */
/** @typedef {{}} Shellsettingsnav2Inputs */
/** @typedef {{ email: NonNullable<unknown> }} Shellsignedinas3Inputs */
/** @typedef {{}} Shellskillsnav2Inputs */
/** @typedef {{}} Shellthemetoggletodark4Inputs */
/** @typedef {{}} Shellthemetoggletolight4Inputs */
/** @typedef {{}} Shelltodaynav2Inputs */
/** @typedef {{}} Signupemaillabel2Inputs */
/** @typedef {{}} Signuphasaccount2Inputs */
/** @typedef {{}} Signuploginlink2Inputs */
/** @typedef {{}} Signupnamelabel2Inputs */
/** @typedef {{}} Signuppasswordlabel2Inputs */
/** @typedef {{}} Signupsubmit1Inputs */
/** @typedef {{}} Signuptitle1Inputs */
/** @typedef {{}} Skillcolumnactions2Inputs */
/** @typedef {{}} Skillcolumndefault2Inputs */
/** @typedef {{}} Skillcolumnname2Inputs */
/** @typedef {{}} Skillcolumnslug2Inputs */
/** @typedef {{}} Skillcolumntype2Inputs */
/** @typedef {{}} Skillcontentlabel2Inputs */
/** @typedef {{}} Skillcreatebutton2Inputs */
/** @typedef {{}} Skillcreatedialogtitle3Inputs */
/** @typedef {{}} Skilldialogdescription2Inputs */
/** @typedef {{}} Skilleditdialogtitle3Inputs */
/** @typedef {{}} Skillnamelabel2Inputs */
/** @typedef {{}} Skillpreviewlabel2Inputs */
/** @typedef {{}} Skillremoveaction2Inputs */
/** @typedef {{}} Skillremoveconfirmbody3Inputs */
/** @typedef {{}} Skillremoveconfirmtitle3Inputs */
/** @typedef {{}} Skillremoveerrortoast3Inputs */
/** @typedef {{}} Skillsaveerrortoast3Inputs */
/** @typedef {{}} Skillsluglabel2Inputs */
/** @typedef {{}} Skilltoggledefaulterrortoast4Inputs */
/** @typedef {{}} Skilltypeplatform2Inputs */
/** @typedef {{}} Skilltypetenant2Inputs */
/** @typedef {{}} Skillsdescription1Inputs */
/** @typedef {{}} Skillsemptytitle2Inputs */
/** @typedef {{}} Skillsloaderrortitle3Inputs */
/** @typedef {{}} Skillstitle1Inputs */
/** @typedef {{ time: NonNullable<unknown> }} Studioautosavesaved2Inputs */
/** @typedef {{}} Studioautosavesaving2Inputs */
/** @typedef {{}} Studiobacktocustomimport4Inputs */
/** @typedef {{}} Studiochatemptydescription3Inputs */
/** @typedef {{}} Studiochatemptytitle3Inputs */
/** @typedef {{}} Studiochatplaceholder2Inputs */
/** @typedef {{}} Studiochatsuggestioncta3Inputs */
/** @typedef {{}} Studiochatsuggestionheadline3Inputs */
/** @typedef {{}} Studiochatsuggestionmobile3Inputs */
/** @typedef {{}} Studiochatviewversionlink4Inputs */
/** @typedef {{}} Studiocomingsoon2Inputs */
/** @typedef {{}} Studiocommentattachlabel3Inputs */
/** @typedef {{}} Studiocommentchatchip3Inputs */
/** @typedef {{}} Studiocommentplaceholder2Inputs */
/** @typedef {{}} Studiocommentqueuebutton3Inputs */
/** @typedef {{}} Studiocommentqueueempty3Inputs */
/** @typedef {{}} Studiocommentqueueerrortoast4Inputs */
/** @typedef {{}} Studiocommentqueuetitle3Inputs */
/** @typedef {{}} Studiocommentqueuedtoast3Inputs */
/** @typedef {{}} Studiocommentsendallbutton4Inputs */
/** @typedef {{}} Studiocommentsendallerrortoast5Inputs */
/** @typedef {{}} Studiocommentsendalltoast4Inputs */
/** @typedef {{}} Studiocommentsenderrortoast4Inputs */
/** @typedef {{}} Studiocommentsendtochatbutton5Inputs */
/** @typedef {{}} Studiocommentsenttoast3Inputs */
/** @typedef {{}} Studiodeleteconfirmaction3Inputs */
/** @typedef {{}} Studiodeleteconfirmcancel3Inputs */
/** @typedef {{}} Studiodeleteconfirmdescription3Inputs */
/** @typedef {{}} Studiodeleteconfirmtitle3Inputs */
/** @typedef {{}} Studiodeleteerrortoast3Inputs */
/** @typedef {{}} Studiodeployhistorytitle3Inputs */
/** @typedef {{}} Studiodeploystatussuperseded3Inputs */
/** @typedef {{}} Studiodeploystatusunpublished3Inputs */
/** @typedef {{}} Studiodesignfilesemptydescription4Inputs */
/** @typedef {{}} Studiodesignfilesemptytitle4Inputs */
/** @typedef {{}} Studiodownload1Inputs */
/** @typedef {{}} Studiodrawclear2Inputs */
/** @typedef {{}} Studiodrawcolorlabel3Inputs */
/** @typedef {{}} Studiodrawpromptplaceholder3Inputs */
/** @typedef {{}} Studiodrawsendbutton3Inputs */
/** @typedef {{}} Studiodrawsenderrortoast4Inputs */
/** @typedef {{}} Studiodrawsending2Inputs */
/** @typedef {{}} Studiodrawtoolarrow3Inputs */
/** @typedef {{}} Studiodrawtoolpen3Inputs */
/** @typedef {{}} Studiodrawtoolrect3Inputs */
/** @typedef {{}} Studiodrawundo2Inputs */
/** @typedef {{}} Studioduplicateerrortoast3Inputs */
/** @typedef {{}} Studioexport1Inputs */
/** @typedef {{}} Studioexporthtml2Inputs */
/** @typedef {{}} Studioexportpng2Inputs */
/** @typedef {{}} Studioexportzip2Inputs */
/** @typedef {{}} Studiofilesfolderassets3Inputs */
/** @typedef {{}} Studiofilesfolderscreenshots3Inputs */
/** @typedef {{}} Studiofilesgroupdata3Inputs */
/** @typedef {{}} Studiofilesgroupfolders3Inputs */
/** @typedef {{}} Studiofilesgroupimages3Inputs */
/** @typedef {{}} Studiofilesgrouppages3Inputs */
/** @typedef {{}} Studiofilesjsonviewerempty4Inputs */
/** @typedef {{ fileName: NonNullable<unknown> }} Studiofilesjsonviewertitle4Inputs */
/** @typedef {{}} Studiofileslightboxempty3Inputs */
/** @typedef {{}} Studiofileslightboxtitle3Inputs */
/** @typedef {{}} Studiofilesprojecttitle3Inputs */
/** @typedef {{}} Studiofilesrefreshlabel3Inputs */
/** @typedef {{}} Studiofilesthumbnailname3Inputs */
/** @typedef {{}} Studiofilesuploadfailed3Inputs */
/** @typedef {{}} Studiofilesuploadlabel3Inputs */
/** @typedef {{}} Studiofilesvideotoolarge4Inputs */
/** @typedef {{}} Studiofunnelfixformprompt4Inputs */
/** @typedef {{}} Studiofunnelfixseoprompt4Inputs */
/** @typedef {{}} Studiofunnelgapsbothdescription4Inputs */
/** @typedef {{}} Studiofunnelgapsdismiss3Inputs */
/** @typedef {{}} Studiofunnelgapsfixbutton4Inputs */
/** @typedef {{}} Studiofunnelgapsformdescription4Inputs */
/** @typedef {{}} Studiofunnelgapsseodescription4Inputs */
/** @typedef {{}} Studiofunnelgapstitle3Inputs */
/** @typedef {{}} Studiogenerateerrorbuycreditsbutton5Inputs */
/** @typedef {{}} Studiogenerateerrorconnectaibutton5Inputs */
/** @typedef {{}} Studiogenerateerrorinsufficientcreditsdescription5Inputs */
/** @typedef {{}} Studiogenerateerrorinsufficientcreditstitle5Inputs */
/** @typedef {{}} Studiogenerateerrornoaiconnectiondescription6Inputs */
/** @typedef {{}} Studiogenerateerrornoaiconnectiontitle6Inputs */
/** @typedef {{}} Studiogenerateerrortrialexhausteddescription5Inputs */
/** @typedef {{}} Studiogenerateerrortrialexhaustedtitle5Inputs */
/** @typedef {{}} Studiohidechat2Inputs */
/** @typedef {{}} Studiohidesidebar2Inputs */
/** @typedef {{}} Studioinspectempty2Inputs */
/** @typedef {{ threshold: NonNullable<unknown> }} Studiolayersdragdisabledhint4Inputs */
/** @typedef {{}} Studiolayersfooterhint3Inputs */
/** @typedef {{}} Studiomoreactionslabel3Inputs */
/** @typedef {{}} Studionativechatsuggestionheadline4Inputs */
/** @typedef {{}} Studionativechatsuggestiontestimonial4Inputs */
/** @typedef {{}} Studionativechatsuggestiontone4Inputs */
/** @typedef {{}} Studionativechattooltitle4Inputs */
/** @typedef {{}} Studionew1Inputs */
/** @typedef {{}} Studiopresent1Inputs */
/** @typedef {{}} Studiopreviewbutton2Inputs */
/** @typedef {{}} Studiopreviewerrortoast3Inputs */
/** @typedef {{}} Studiopublishanywaybutton3Inputs */
/** @typedef {{}} Studiopublishattachcampaignbutton4Inputs */
/** @typedef {{}} Studiopublishauditrequiredtoast4Inputs */
/** @typedef {{}} Studiopublishauditthresholdtoast4Inputs */
/** @typedef {{}} Studiopublishbutton2Inputs */
/** @typedef {{}} Studiopublishcampaignattacherrortoast5Inputs */
/** @typedef {{}} Studiopublishcampaignattachedtoast4Inputs */
/** @typedef {{}} Studiopublishcampaignsearchempty4Inputs */
/** @typedef {{}} Studiopublishcampaignsearchplaceholder4Inputs */
/** @typedef {{}} Studiopublishdialogdescription3Inputs */
/** @typedef {{}} Studiopublishdialogtitle3Inputs */
/** @typedef {{}} Studiopublisherrortoast3Inputs */
/** @typedef {{}} Studiopublishnocampaignwarning4Inputs */
/** @typedef {{}} Studiopublishsubdomainlabel3Inputs */
/** @typedef {{}} Studiopublishsubdomaintakentoast4Inputs */
/** @typedef {{ hostname: NonNullable<unknown> }} Studiopublishsuccesstoast3Inputs */
/** @typedef {{}} Studiopublishunverifiedassetstoast4Inputs */
/** @typedef {{}} Studiopublishupdatebutton3Inputs */
/** @typedef {{}} Studiorenameerrortoast3Inputs */
/** @typedef {{}} Studiorollbackbutton2Inputs */
/** @typedef {{}} Studiorollbackerrortoast3Inputs */
/** @typedef {{}} Studiorollbacksuccesstoast3Inputs */
/** @typedef {{}} Studiosavebutton2Inputs */
/** @typedef {{}} Studiosaveshortcuthint3Inputs */
/** @typedef {{}} Studioseocanonicalurlhint4Inputs */
/** @typedef {{}} Studioseocanonicalurllabel4Inputs */
/** @typedef {{}} Studioseodescriptionlabel3Inputs */
/** @typedef {{}} Studioseonofollowlabel3Inputs */
/** @typedef {{}} Studioseonoindexhint3Inputs */
/** @typedef {{}} Studioseonoindexlabel3Inputs */
/** @typedef {{}} Studioseoogimagelabel4Inputs */
/** @typedef {{}} Studioseopaneltitle3Inputs */
/** @typedef {{}} Studioseosharepreviewlabel4Inputs */
/** @typedef {{}} Studioseosharepreviewnoimage5Inputs */
/** @typedef {{}} Studioseostructureddatatypearticle5Inputs */
/** @typedef {{}} Studioseostructureddatatypeauto5Inputs */
/** @typedef {{}} Studioseostructureddatatypecourse5Inputs */
/** @typedef {{}} Studioseostructureddatatypelabel5Inputs */
/** @typedef {{}} Studioseostructureddatatypelocalbusiness6Inputs */
/** @typedef {{}} Studioseostructureddatatypeorganization5Inputs */
/** @typedef {{}} Studioseostructureddatatypeproduct5Inputs */
/** @typedef {{}} Studioseostructureddatatypewebpage6Inputs */
/** @typedef {{}} Studioseotitlehint3Inputs */
/** @typedef {{}} Studioseotitlelabel3Inputs */
/** @typedef {{}} Studioseotwittercardlabel4Inputs */
/** @typedef {{}} Studioseotwittercardsummary4Inputs */
/** @typedef {{}} Studioseotwittercardsummarylargeimage6Inputs */
/** @typedef {{}} Studioshare1Inputs */
/** @typedef {{}} Studioshortcutcomment2Inputs */
/** @typedef {{}} Studioshortcutdraw2Inputs */
/** @typedef {{}} Studioshortcutedit2Inputs */
/** @typedef {{}} Studioshortcutzoomin3Inputs */
/** @typedef {{}} Studioshortcutzoomout3Inputs */
/** @typedef {{}} Studioshowchat2Inputs */
/** @typedef {{}} Studioshowsidebar2Inputs */
/** @typedef {{}} Studiosidebarfiles2Inputs */
/** @typedef {{}} Studiosidebarhistory2Inputs */
/** @typedef {{}} Studiosidebarinspect2Inputs */
/** @typedef {{}} Studiosidebarlayers2Inputs */
/** @typedef {{}} Studioskillsbutton2Inputs */
/** @typedef {{}} Studioskillsemptytitle3Inputs */
/** @typedef {{}} Studioskillsloaderrortitle4Inputs */
/** @typedef {{}} Studioskillspopoverdescription3Inputs */
/** @typedef {{}} Studioskillspopovertitle3Inputs */
/** @typedef {{}} Studioskillstoggleerrortoast4Inputs */
/** @typedef {{}} Studiotabdesignfiles3Inputs */
/** @typedef {{}} Studiotokenscoloraccentforegroundlabel5Inputs */
/** @typedef {{}} Studiotokenscoloraccentlabel4Inputs */
/** @typedef {{}} Studiotokenscolorborderlabel4Inputs */
/** @typedef {{}} Studiotokenscolorforegroundlabel4Inputs */
/** @typedef {{}} Studiotokenscolormutedlabel4Inputs */
/** @typedef {{}} Studiotokenscolorprimaryforegroundlabel5Inputs */
/** @typedef {{}} Studiotokenscolorprimarylabel4Inputs */
/** @typedef {{}} Studiotokenscolorsurfacelabel4Inputs */
/** @typedef {{}} Studiotokensfontbodylabel4Inputs */
/** @typedef {{}} Studiotokensfontcustomoption4Inputs */
/** @typedef {{}} Studiotokensfontheadinglabel4Inputs */
/** @typedef {{}} Studiotokenspaneltitle3Inputs */
/** @typedef {{}} Studiotokensradiushint3Inputs */
/** @typedef {{}} Studiotokensradiuslabel3Inputs */
/** @typedef {{}} Studiotokensresetbutton3Inputs */
/** @typedef {{}} Studiotoolbarcomment2Inputs */
/** @typedef {{}} Studiotoolbardraw2Inputs */
/** @typedef {{}} Studiotoolbaredit2Inputs */
/** @typedef {{}} Studiotoolbarrefresh2Inputs */
/** @typedef {{}} Studiotoolbartweaks2Inputs */
/** @typedef {{}} Studiotoolbarzoomin3Inputs */
/** @typedef {{}} Studiotoolbarzoomout3Inputs */
/** @typedef {{}} Studiounpublishbutton2Inputs */
/** @typedef {{}} Studiounpublisherrortoast3Inputs */
/** @typedef {{}} Studiounpublishsuccesstoast3Inputs */
/** @typedef {{}} Studioversioncomparebutton3Inputs */
/** @typedef {{}} Studioversioncomparehint3Inputs */
/** @typedef {{}} Studioversioncurrentbadge3Inputs */
/** @typedef {{}} Studioversiondiffcodetab4Inputs */
/** @typedef {{}} Studioversiondiffpreviewtab4Inputs */
/** @typedef {{}} Studioversiondifftitle3Inputs */
/** @typedef {{}} Studioversionhistoryempty3Inputs */
/** @typedef {{}} Studioversionhistorytitle3Inputs */
/** @typedef {{}} Studioversionlabelbutton3Inputs */
/** @typedef {{}} Studioversionlabelerrortoast4Inputs */
/** @typedef {{}} Studioversionlabelplaceholder3Inputs */
/** @typedef {{}} Studioversionlabelsave3Inputs */
/** @typedef {{}} Studioversionoriginaifull4Inputs */
/** @typedef {{}} Studioversionoriginaipatch4Inputs */
/** @typedef {{}} Studioversionoriginimport3Inputs */
/** @typedef {{}} Studioversionoriginmanual3Inputs */
/** @typedef {{}} Studioversionoriginrestore3Inputs */
/** @typedef {{}} Studioversionrestorebutton3Inputs */
/** @typedef {{}} Studioversionrestoreerrortoast4Inputs */
/** @typedef {{}} Studioversionviewchatbutton4Inputs */
/** @typedef {{}} Todaycreatefirstoffer3Inputs */
/** @typedef {{}} Todaycreateoffer2Inputs */
/** @typedef {{}} Todaydescription1Inputs */
/** @typedef {{}} Todayerrortitle2Inputs */
/** @typedef {{}} Todayeyebrow1Inputs */
/** @typedef {{}} Todaygettingstarteddescription3Inputs */
/** @typedef {{}} Todaygettingstartedtitle3Inputs */
/** @typedef {{}} Todayliveoffers2Inputs */
/** @typedef {{}} Todayloading1Inputs */
/** @typedef {{}} Todaynextsteptitle3Inputs */
/** @typedef {{}} Todaynextstepwithoffer4Inputs */
/** @typedef {{}} Todaynextstepwithoutoffer4Inputs */
/** @typedef {{}} Todayopenaction2Inputs */
/** @typedef {{}} Todayqueuedescription2Inputs */
/** @typedef {{}} Todayqueueempty2Inputs */
/** @typedef {{}} Todayqueuetitle2Inputs */
/** @typedef {{}} Todaysystemready2Inputs */
/** @typedef {{}} Todaysystemstatus2Inputs */
/** @typedef {{}} Todaytitle1Inputs */
/** @typedef {{}} Usageinsightsbyeventtitle4Inputs */
/** @typedef {{}} Usageinsightscolumncount3Inputs */
/** @typedef {{}} Usageinsightscolumnevent3Inputs */
/** @typedef {{}} Usageinsightsdescription2Inputs */
/** @typedef {{}} Usageinsightsemptytitle3Inputs */
/** @typedef {{}} Usageinsightsloaderrortitle4Inputs */
/** @typedef {{}} Usageinsightstitle2Inputs */
/** @typedef {{}} Usageinsightstotalevents3Inputs */
/** @typedef {{}} Verifiedbody1Inputs */
/** @typedef {{}} Verifiedloginlink2Inputs */
/** @typedef {{}} Verifiedtitle1Inputs */
/** @typedef {{}} Verifyemailbody2Inputs */
/** @typedef {{}} Verifyemailtitle2Inputs */
export type LocalizedString = import("../runtime.js").LocalizedString;
export type Acceptinviteerrorbody3Inputs = {};
export type Acceptinviteerrortitle3Inputs = {};
export type Acceptinvitetitle2Inputs = {};
export type Aiapikeylabel3Inputs = {};
export type Aiconnectbutton2Inputs = {};
export type Aiconnectdialogdescription3Inputs = {};
export type Aiconnectdialogtitle3Inputs = {};
export type Aiconnectguidetitle3Inputs = {};
export type Aiconnectsubmit2Inputs = {};
export type Aiconnectioncolumnactions3Inputs = {};
export type Aiconnectioncolumnmodel3Inputs = {};
export type Aiconnectioncolumnprovider3Inputs = {};
export type Aiconnectioncolumnstatus3Inputs = {};
export type Aiconnectiondefaultbadge3Inputs = {};
export type Aiconnectionstatusactive3Inputs = {};
export type Aiconnectionstatusinvalid3Inputs = {};
export type Aiconnectionsdescription2Inputs = {};
export type Aiconnectionsemptytitle3Inputs = {};
export type Aiconnectionsloaderrortitle4Inputs = {};
export type Aiconnectionstitle2Inputs = {};
export type Aicreditbalancelabel3Inputs = {};
export type Aicreditbalancelowwarning4Inputs = {};
export type Aidefaultmodellabel3Inputs = {};
export type Aifetchmodelsempty3Inputs = {};
export type Aifetchmodelserror3Inputs = {};
export type Aifetchmodelswaitingforkey5Inputs = {};
export type Aiinvalidapikeyerror4Inputs = {};
export type Aiproviderdescriptionanthropic3Inputs = {};
export type Aiproviderdescriptiongroq3Inputs = {};
export type Aiproviderdescriptionnvidia3Inputs = {};
export type Aiproviderdescriptionopenai3Inputs = {};
export type Aiproviderdescriptionopenrouter3Inputs = {};
export type Aiproviderlabel2Inputs = {};
export type Airemoveconfirmbody3Inputs = {};
export type Airemoveconfirmtitle3Inputs = {};
export type Airemoveconnectionaction3Inputs = {};
export type Airemoveconnectionerrortoast4Inputs = {};
export type Aisetdefaultaction3Inputs = {};
export type Aisetdefaulterrortoast4Inputs = {};
export type Aitrialremaininglabel3Inputs = {};
export type Aitrialremaininglowwarning4Inputs = {};
export type Aiusagecolumncost3Inputs = {};
export type Aiusagecolumndate3Inputs = {};
export type Aiusagecolumnmodel3Inputs = {};
export type Aiusagecolumntokens3Inputs = {};
export type Aiusagedescription2Inputs = {};
export type Aiusageemptytitle3Inputs = {};
export type Aiusageloaderrortitle4Inputs = {};
export type Aiusagetitle2Inputs = {};
export type Appname1Inputs = {};
export type Apptagline1Inputs = {};
export type Auditlogcolumnaction3Inputs = {};
export type Auditlogcolumnactor3Inputs = {};
export type Auditlogcolumntarget3Inputs = {};
export type Auditlogcolumntime3Inputs = {};
export type Auditlogdescription2Inputs = {};
export type Auditlogemptytitle3Inputs = {};
export type Auditlogloaderrortitle4Inputs = {};
export type Auditlogtitle2Inputs = {};
export type Authsidedescription2Inputs = {};
export type Authsidetitle2Inputs = {};
export type Authvaluelead2Inputs = {};
export type Authvaluenext2Inputs = {};
export type Authvaluepayment2Inputs = {};
export type Campaignsaccountnamelabel3Inputs = {};
export type Campaignsaccountnumberlabel3Inputs = {};
export type Campaignsactionduplicate2Inputs = {};
export type Campaignsaddbutton2Inputs = {};
export type Campaignsadddialogtitle3Inputs = {};
export type Campaignsanalyticsaction2Inputs = {};
export type Campaignsanalyticsbysourceempty4Inputs = {};
export type Campaignsanalyticsbysourcetitle4Inputs = {};
export type Campaignsanalyticsconversion2Inputs = {};
export type Campaignsanalyticsdescription2Inputs = {};
export type Campaignsanalyticsloaderrortitle4Inputs = {};
export type Campaignsanalyticsorders2Inputs = {};
export type Campaignsanalyticsrevenue2Inputs = {};
export type Campaignsanalyticssourcecolumn3Inputs = {};
export type Campaignsanalyticssubmits2Inputs = {};
export type Campaignsanalyticstitle2Inputs = {};
export type Campaignsanalyticsviews2Inputs = {};
export type Campaignsassignmentmodelabel3Inputs = {};
export type Campaignsassignmentmodemanual3Inputs = {};
export type Campaignsassignmentmoderoundrobin4Inputs = {};
export type Campaignsbankbinlabel3Inputs = {};
export type Campaignsbulkchangestatus3Inputs = {};
export type Campaignsbulkclearselection3Inputs = {};
export type Campaignsbulkdelete2Inputs = {};
export type Campaignsbulkdeleteconfirm3Inputs = {};
export type Campaignsbulkdeleteconfirmdescription4Inputs = {
  count: NonNullable<unknown>;
};
export type Campaignsbulkdeleteconfirmtitle4Inputs = {};
export type Campaignsbulkdeleteerrortoast4Inputs = {};
export type Campaignsbulkselectedcount3Inputs = {
  count: NonNullable<unknown>;
};
export type Campaignsbulkstatuserrortoast4Inputs = {};
export type Campaignscolumnactions2Inputs = {};
export type Campaignscolumnname2Inputs = {};
export type Campaignscolumnproducts2Inputs = {};
export type Campaignscolumnstatus2Inputs = {};
export type Campaignsdescription1Inputs = {};
export type Campaignsdialogdescription2Inputs = {};
export type Campaignsduplicateerrortoast3Inputs = {};
export type Campaignseditdialogtitle3Inputs = {};
export type Campaignsemptytitle2Inputs = {};
export type Campaignsendsatlabel3Inputs = {};
export type Campaignsformfieldaddbutton4Inputs = {};
export type Campaignsformfieldkeyplaceholder4Inputs = {};
export type Campaignsformfieldlabellabel4Inputs = {};
export type Campaignsformfieldoptionsplaceholder4Inputs = {};
export type Campaignsformfieldremovelabel4Inputs = {};
export type Campaignsformfieldrequiredlabel4Inputs = {};
export type Campaignsformfieldtypecheckbox4Inputs = {};
export type Campaignsformfieldtypeselect4Inputs = {};
export type Campaignsformfieldtypetext4Inputs = {};
export type Campaignsformfieldsempty3Inputs = {};
export type Campaignsformfieldssectiontitle4Inputs = {};
export type Campaignsgoallabel2Inputs = {};
export type Campaignsloaderrortitle3Inputs = {};
export type Campaignsnamelabel2Inputs = {};
export type Campaignsogimagecreatefirsthint5Inputs = {};
export type Campaignsogimagedescription3Inputs = {};
export type Campaignsogimagelabel3Inputs = {};
export type Campaignsogimagesectiontitle4Inputs = {};
export type Campaignspaymentenabledlabel3Inputs = {};
export type Campaignspaymentsectiontitle3Inputs = {};
export type Campaignspopupbodylabel3Inputs = {};
export type Campaignspopupmanualpendingtitle4Inputs = {};
export type Campaignspopuppaidtitle3Inputs = {};
export type Campaignspopupregisteredtitle3Inputs = {};
export type Campaignspopuptitlelabel3Inputs = {};
export type Campaignspopupssectiontitle3Inputs = {};
export type Campaignsproductsempty2Inputs = {};
export type Campaignsproductslabel2Inputs = {};
export type Campaignsremoveconfirmaction3Inputs = {};
export type Campaignsremoveconfirmbody3Inputs = {};
export type Campaignsremoveconfirmtitle3Inputs = {
  name: NonNullable<unknown>;
};
export type Campaignsremoveerrortoast3Inputs = {};
export type Campaignssaveerrortoast3Inputs = {};
export type Campaignssavesubmit2Inputs = {};
export type Campaignssearchplaceholder2Inputs = {};
export type Campaignssepayautolabel3Inputs = {};
export type Campaignsstartsatlabel3Inputs = {};
export type Campaignsstatusactive2Inputs = {};
export type Campaignsstatusdraft2Inputs = {};
export type Campaignsstatusended2Inputs = {};
export type Campaignsstatuslabel2Inputs = {};
export type Campaignsstatuspaused2Inputs = {};
export type Campaignstitle1Inputs = {};
export type Campaignstransferprefixlabel3Inputs = {};
export type Campaignsutmdefaultaddbutton4Inputs = {};
export type Campaignsutmdefaultsempty3Inputs = {};
export type Campaignsutmdefaultssectiontitle4Inputs = {};
export type Campaignsutmkeyplaceholder3Inputs = {};
export type Campaignsutmremovelabel3Inputs = {};
export type Campaignsutmvalueplaceholder3Inputs = {};
export type Campaignszalourllabel3Inputs = {};
export type Commonall1Inputs = {};
export type Commonback1Inputs = {};
export type Commoncancel1Inputs = {};
export type Commoncontinue1Inputs = {};
export type Commoncopy1Inputs = {};
export type Commoncreate1Inputs = {};
export type Commonedit1Inputs = {};
export type Commonloadmore2Inputs = {};
export type Commonloading1Inputs = {};
export type Commonnext1Inputs = {};
export type Commonpaginationlabel2Inputs = {
  page: NonNullable<unknown>;
  total: NonNullable<unknown>;
};
export type Commonprevious1Inputs = {};
export type Commonredo1Inputs = {};
export type Commonretry1Inputs = {};
export type Commonsave1Inputs = {};
export type Commonundo1Inputs = {};
export type Domainsaddbutton2Inputs = {};
export type Domainsadddialogdescription3Inputs = {};
export type Domainsadddialogtitle3Inputs = {};
export type Domainsadderrortoast3Inputs = {};
export type Domainsaddsubmit2Inputs = {};
export type Domainscnamehostlabel3Inputs = {};
export type Domainscnamevaluelabel3Inputs = {};
export type Domainscolumnactions2Inputs = {};
export type Domainscolumnhostname2Inputs = {};
export type Domainscolumnstatus2Inputs = {};
export type Domainsdescription1Inputs = {};
export type Domainsdnsinstructionsintro3Inputs = {};
export type Domainsemptytitle2Inputs = {};
export type Domainshostnamelabel2Inputs = {};
export type Domainslandingpagelabel3Inputs = {};
export type Domainslandingpageplaceholder3Inputs = {};
export type Domainsloaderrortitle3Inputs = {};
export type Domainsremoveaction2Inputs = {};
export type Domainsremoveconfirmbody3Inputs = {};
export type Domainsremoveconfirmtitle3Inputs = {
  hostname: NonNullable<unknown>;
};
export type Domainsremoveerrortoast3Inputs = {};
export type Domainsstatusactive2Inputs = {};
export type Domainsstatusfailed2Inputs = {};
export type Domainsstatuspending2Inputs = {};
export type Domainstitle1Inputs = {};
export type Domainstxthostlabel3Inputs = {};
export type Domainstxtinstructionsintro3Inputs = {};
export type Domainstxtvaluelabel3Inputs = {};
export type Domainsupgraderequireddescription3Inputs = {};
export type Domainsupgraderequiredtitle3Inputs = {};
export type Domainsvaluecopiedtoast3Inputs = {};
export type Domainsverifybutton2Inputs = {};
export type Domainsverifyerrortoast3Inputs = {};
export type Entityimagedrophint3Inputs = {};
export type Entityimageremovelabel3Inputs = {};
export type Entityimageuploadfailed3Inputs = {};
export type Entityimageuploadlabel3Inputs = {};
export type Entityimageuploadinglabel3Inputs = {};
export type Featureupgradegenericdescription3Inputs = {};
export type Featureupgradegenerictitle3Inputs = {};
export type Forgotpasswordbody2Inputs = {};
export type Forgotpasswordemaillabel3Inputs = {};
export type Forgotpasswordsent2Inputs = {};
export type Forgotpasswordsubmit2Inputs = {};
export type Forgotpasswordtitle2Inputs = {};
export type Inboxdescription1Inputs = {};
export type Inboxtitle1Inputs = {};
export type Landingsactionassigntocampaign4Inputs = {};
export type Landingsactiondelete2Inputs = {};
export type Landingsactionduplicate2Inputs = {};
export type Landingsactionremovefromcampaign4Inputs = {};
export type Landingsactionrename2Inputs = {};
export type Landingsactionviewlive3Inputs = {};
export type Landingsassigntocampaignempty4Inputs = {};
export type Landingsbadgedraft2Inputs = {};
export type Landingsbadgepublished2Inputs = {};
export type Landingscardactionslabel3Inputs = {};
export type Landingscreateaibutton3Inputs = {};
export type Landingscreateaidescription3Inputs = {};
export type Landingscreateaititle3Inputs = {};
export type Landingscreateerrortoast3Inputs = {};
export type Landingscreateheading2Inputs = {};
export type Landingscreateimportdescription3Inputs = {};
export type Landingscreateimporttitle3Inputs = {};
export type Landingscreatemanualbutton3Inputs = {};
export type Landingscreatemanualdescription3Inputs = {};
export type Landingscreatemanualtitle3Inputs = {};
export type Landingsfilterall2Inputs = {};
export type Landingsfilterallcampaigns3Inputs = {};
export type Landingsfilterdraft2Inputs = {};
export type Landingsfilterpublished2Inputs = {};
export type Landingsimportbutton2Inputs = {};
export type Landingsimportdialogtitle3Inputs = {};
export type Landingsimporterrortoast3Inputs = {};
export type Landingsimporthtmlplaceholder3Inputs = {};
export type Landingsimportnamelabel3Inputs = {};
export type Landingsimportnameplaceholder3Inputs = {};
export type Landingsimportsubmitbutton3Inputs = {};
export type Landingsimportsubmitting2Inputs = {};
export type Landingsimporttabfile3Inputs = {};
export type Landingsimporttabhtml3Inputs = {};
export type Landingsimporttaburl3Inputs = {};
export type Landingsimporturlplaceholder3Inputs = {};
export type Landingsloaderrortitle3Inputs = {};
export type Landingsnoresultstitle3Inputs = {};
export type Landingsreuploadbutton2Inputs = {};
export type Landingsreuploaddialogtitle3Inputs = {};
export type Landingssearchplaceholder2Inputs = {};
export type Landingssortname2Inputs = {};
export type Landingssortupdated2Inputs = {};
export type Landingssummarycount2Inputs = {
  total: NonNullable<unknown>;
  published: NonNullable<unknown>;
};
export type Landingsupdatedat2Inputs = {};
export type Leadsourcecsvimport3Inputs = {};
export type Leadsourcedirect2Inputs = {};
export type Leadsourcefacebook2Inputs = {};
export type Leadsourcegeneric2Inputs = {};
export type Leadsourcegoogleads3Inputs = {};
export type Leadsourcelandingpage3Inputs = {};
export type Leadsourcemanual2Inputs = {};
export type Leadsourcetiktok2Inputs = {};
export type Leadsourcezalooa3Inputs = {};
export type Leadsactioncall2Inputs = {};
export type Leadsactioncopiedtoast3Inputs = {};
export type Leadsactioncopyphone3Inputs = {};
export type Leadsactionzalo2Inputs = {};
export type Leadsactivitycall2Inputs = {};
export type Leadsactivitynote2Inputs = {};
export type Leadsactivityordercreated3Inputs = {};
export type Leadsactivitypayment2Inputs = {};
export type Leadsactivityresubmit2Inputs = {};
export type Leadsactivitystagechange3Inputs = {};
export type Leadsactivitysystem2Inputs = {};
export type Leadsaddnotelabel3Inputs = {};
export type Leadsaddnoteplaceholder3Inputs = {};
export type Leadsaddnotesubmit3Inputs = {};
export type Leadsagedays2Inputs = {
  days: NonNullable<unknown>;
};
export type Leadsagehours2Inputs = {
  hours: NonNullable<unknown>;
};
export type Leadsassignerrortoast3Inputs = {};
export type Leadsassigneelabel2Inputs = {};
export type Leadsassignmentaddrule3Inputs = {};
export type Leadsassignmentdeleteerrortoast4Inputs = {};
export type Leadsassignmenteditrule3Inputs = {};
export type Leadsassignmentemptytitle3Inputs = {};
export type Leadsassignmentfixedassigneelabel4Inputs = {};
export type Leadsassignmentpersonalabel3Inputs = {};
export type Leadsassignmentpersonaplaceholder3Inputs = {};
export type Leadsassignmentpoollabel3Inputs = {};
export type Leadsassignmentreordererrortoast4Inputs = {};
export type Leadsassignmentruledeletelabel4Inputs = {};
export type Leadsassignmentrulesdescription3Inputs = {};
export type Leadsassignmentrulesmanagebutton4Inputs = {};
export type Leadsassignmentrulestitle3Inputs = {};
export type Leadsassignmentsaveerrortoast4Inputs = {};
export type Leadsassignmentslabreachlabel4Inputs = {};
export type Leadsassignmentslabreachnone4Inputs = {};
export type Leadsassignmentslabreachnotify4Inputs = {};
export type Leadsassignmentslabreachreassign4Inputs = {};
export type Leadsassignmentslalabel3Inputs = {};
export type Leadsassignmentstrategyfixed3Inputs = {};
export type Leadsassignmentstrategylabel3Inputs = {};
export type Leadsassignmentstrategyleastactive4Inputs = {};
export type Leadsassignmentstrategyroundrobin4Inputs = {};
export type Leadsbulkassignerrortoast4Inputs = {};
export type Leadsbulkassignto3Inputs = {};
export type Leadsbulkchangestage3Inputs = {};
export type Leadsbulkclearselection3Inputs = {};
export type Leadsbulkdelete2Inputs = {};
export type Leadsbulkdeleteconfirm3Inputs = {};
export type Leadsbulkdeleteconfirmdescription4Inputs = {
  count: NonNullable<unknown>;
};
export type Leadsbulkdeleteconfirmtitle4Inputs = {};
export type Leadsbulkdeleteerrortoast4Inputs = {};
export type Leadsbulkexport2Inputs = {};
export type Leadsbulkselectedcount3Inputs = {
  count: NonNullable<unknown>;
};
export type Leadsbulkstageerrortoast4Inputs = {};
export type Leadscampaignlabel2Inputs = {};
export type Leadscolumnage2Inputs = {};
export type Leadscolumnassignee2Inputs = {};
export type Leadscolumncreatedat3Inputs = {};
export type Leadscolumnname2Inputs = {};
export type Leadscolumnphone2Inputs = {};
export type Leadscolumnstage2Inputs = {};
export type Leadsdescription1Inputs = {};
export type Leadsdsraddbutton3Inputs = {};
export type Leadsdsrcancel2Inputs = {};
export type Leadsdsrcompleteaction3Inputs = {};
export type Leadsdsrcompleteerrortoast4Inputs = {};
export type Leadsdsrcreateerrortoast4Inputs = {};
export type Leadsdsrdialogtitle3Inputs = {};
export type Leadsdsrduelabel3Inputs = {};
export type Leadsdsrempty2Inputs = {};
export type Leadsdsrindicatorlabel3Inputs = {
  count: NonNullable<unknown>;
};
export type Leadsdsrnoteslabel3Inputs = {};
export type Leadsdsrnotesplaceholder3Inputs = {};
export type Leadsdsroverduebadge3Inputs = {};
export type Leadsdsrreceivedatlabel4Inputs = {};
export type Leadsdsrsectionlabel3Inputs = {};
export type Leadsdsrstatuscompleted3Inputs = {};
export type Leadsdsrstatuspending3Inputs = {};
export type Leadsdsrsubmit2Inputs = {};
export type Leadsdsrtypedelete3Inputs = {};
export type Leadsdsrtypeexport3Inputs = {};
export type Leadsdsrtypelabel3Inputs = {};
export type Leadsemptytitle2Inputs = {};
export type Leadsexportbutton2Inputs = {};
export type Leadsexporterrortoast3Inputs = {};
export type Leadsfilterassigneelabel3Inputs = {};
export type Leadsfiltercampaignlabel3Inputs = {};
export type Leadsfilterdatefromlabel4Inputs = {};
export type Leadsfilterdatetolabel4Inputs = {};
export type Leadsfilterpaidlabel3Inputs = {};
export type Leadsfilterpaidno3Inputs = {};
export type Leadsfilterpaidyes3Inputs = {};
export type Leadsfilterproductlabel3Inputs = {};
export type Leadsfilterrepeatcustomerlabel4Inputs = {};
export type Leadsfiltersearchlabel3Inputs = {};
export type Leadsfiltersearchplaceholder3Inputs = {};
export type Leadsfilterutmsourcelabel4Inputs = {};
export type Leadsfilterutmsourceplaceholder4Inputs = {};
export type Leadsimportbutton2Inputs = {};
export type Leadsimportdialogdescription3Inputs = {};
export type Leadsimportdialogtitle3Inputs = {};
export type Leadsimporterrortoast3Inputs = {};
export type Leadsimportmappingcustomfields4Inputs = {};
export type Leadsimportmappingemail3Inputs = {};
export type Leadsimportmappingfullname4Inputs = {};
export type Leadsimportmappingpersona3Inputs = {};
export type Leadsimportmappingphone3Inputs = {};
export type Leadsimportmappingskip3Inputs = {};
export type Leadsimportnorowserror4Inputs = {};
export type Leadsimportpreviewtitle3Inputs = {
  rows: NonNullable<unknown>;
  shown: NonNullable<unknown>;
};
export type Leadsimportskippedrowstoast4Inputs = {
  count: NonNullable<unknown>;
};
export type Leadsimportsubmit2Inputs = {};
export type Leadsimportsuccesstoast3Inputs = {
  created: NonNullable<unknown>;
  merged: NonNullable<unknown>;
};
export type Leadsimportuploadlabel3Inputs = {};
export type Leadskanbanemptycolumn3Inputs = {};
export type Leadsloaderrortitle3Inputs = {};
export type Leadsnoteerrortoast3Inputs = {};
export type Leadsnotificationsbelllabel3Inputs = {
  count: NonNullable<unknown>;
};
export type Leadsnotifychannelemailoption4Inputs = {};
export type Leadsnotifychannellabel3Inputs = {};
export type Leadsnotifychannelsmsoption4Inputs = {};
export type Leadsnotifychannelznsoption4Inputs = {};
export type Leadsnotifyesmsapikeylabel5Inputs = {};
export type Leadsnotifyesmsapikeyplaceholder5Inputs = {};
export type Leadsnotifyesmsbrandnamelabel4Inputs = {};
export type Leadsnotifyesmsbrandnameplaceholder4Inputs = {};
export type Leadsnotifyesmsdescription3Inputs = {};
export type Leadsnotifyesmssecretkeylabel5Inputs = {};
export type Leadsnotifyesmssecretkeyplaceholder5Inputs = {};
export type Leadsnotifyesmstitle3Inputs = {};
export type Leadsnotifyphonelabel3Inputs = {};
export type Leadsnotifyphoneplaceholder3Inputs = {};
export type Leadsnotifysettingsdescription3Inputs = {};
export type Leadsnotifysettingsmanagebutton4Inputs = {};
export type Leadsnotifysettingstitle3Inputs = {};
export type Leadsnotifyznsaccesstokenlabel5Inputs = {};
export type Leadsnotifyznsaccesstokenplaceholder5Inputs = {};
export type Leadsnotifyznsdescription3Inputs = {};
export type Leadsnotifyznstemplateidlabel5Inputs = {};
export type Leadsnotifyznstemplateidplaceholder5Inputs = {};
export type Leadsnotifyznstitle3Inputs = {};
export type Leadsorderawaitingconfirmation3Inputs = {};
export type Leadsordercancel2Inputs = {};
export type Leadsorderconfirmpayment3Inputs = {};
export type Leadsordercountlabel3Inputs = {};
export type Leadsordererrortoast3Inputs = {};
export type Leadsorderfulfill2Inputs = {};
export type Leadsorderreasoncancel3Inputs = {};
export type Leadsorderreasonconfirm3Inputs = {};
export type Leadsorderreasonplaceholder3Inputs = {};
export type Leadsorderreasontitle3Inputs = {};
export type Leadsorderslabel2Inputs = {};
export type Leadsrepeatcustomerbadge3Inputs = {};
export type Leadssavedviewdeletelabel4Inputs = {};
export type Leadssavedviewnamelabel4Inputs = {};
export type Leadssavedviewnameplaceholder4Inputs = {};
export type Leadssavedviewsavebutton4Inputs = {};
export type Leadssavedviewsaveconfirm4Inputs = {};
export type Leadssavedviewsaveerrortoast5Inputs = {};
export type Leadssavedviewsharelabel4Inputs = {};
export type Leadsstagechangeerrortoast4Inputs = {};
export type Leadsstagefilteractive3Inputs = {};
export type Leadsstagefilterclear3Inputs = {};
export type Leadssubnavassignmentrules4Inputs = {};
export type Leadssubnavnotifications3Inputs = {};
export type Leadssubnavoverview3Inputs = {};
export type Leadssubnavwebhooks3Inputs = {};
export type Leadstimelinelabel2Inputs = {};
export type Leadstitle1Inputs = {};
export type Leadstotalpaidlabel3Inputs = {};
export type Leadsunassigned1Inputs = {};
export type Leadsunreadindicatorlabel3Inputs = {};
export type Leadsviewkanban2Inputs = {};
export type Leadsviewlist2Inputs = {};
export type Leadswebhookcampaignlabel3Inputs = {};
export type Leadswebhookcampaignplaceholder3Inputs = {};
export type Leadswebhookcarddescription3Inputs = {};
export type Leadswebhookconfiguredbadge3Inputs = {};
export type Leadswebhookgenericdescription3Inputs = {};
export type Leadswebhookgenericgeneratebutton4Inputs = {};
export type Leadswebhookgenerickeyrevealedlabel5Inputs = {};
export type Leadswebhookgenerickeyrevealedwarning5Inputs = {};
export type Leadswebhookgenericrevokebutton4Inputs = {};
export type Leadswebhookgenericrotatebutton4Inputs = {};
export type Leadswebhookgenerictitle3Inputs = {};
export type Leadswebhookgoogleadsdescription4Inputs = {};
export type Leadswebhookgoogleadstitle4Inputs = {};
export type Leadswebhooklastusedlabel4Inputs = {};
export type Leadswebhookmissingbadge3Inputs = {};
export type Leadswebhookneverusedlabel4Inputs = {};
export type Leadswebhooknocampaignwarning4Inputs = {};
export type Leadswebhooknocampaignsempty4Inputs = {};
export type Leadswebhookpageaccesstokendescription5Inputs = {};
export type Leadswebhookpageaccesstokenlabel5Inputs = {};
export type Leadswebhookpageaccesstokenplaceholder5Inputs = {};
export type Leadswebhookremovebutton3Inputs = {};
export type Leadswebhookremovedtoast3Inputs = {};
export type Leadswebhooksaveerrortoast4Inputs = {};
export type Leadswebhooksavedtoast3Inputs = {};
export type Leadswebhooksecretlabel3Inputs = {};
export type Leadswebhooksecretplaceholder3Inputs = {};
export type Leadswebhooksecretrotateplaceholder4Inputs = {};
export type Leadswebhooksettingsdescription3Inputs = {};
export type Leadswebhooksettingsmanagebutton4Inputs = {};
export type Leadswebhooksettingstitle3Inputs = {};
export type Leadswebhooksharedsecretbadge4Inputs = {};
export type Leadswebhooktiktokadvertiserlabel4Inputs = {};
export type Leadswebhooktiktokconnectbutton4Inputs = {};
export type Leadswebhooktiktokdescription3Inputs = {};
export type Leadswebhooktiktokreconnectbutton4Inputs = {};
export type Leadswebhooktiktoktitle3Inputs = {};
export type Leadswebhookurllabel3Inputs = {};
export type Leadswebhookverifytokenlabel4Inputs = {};
export type Leadswebhookverifytokenplaceholder4Inputs = {};
export type Loginemaillabel2Inputs = {};
export type Loginerror1Inputs = {};
export type Loginforgotpassword2Inputs = {};
export type Loginnoaccount2Inputs = {};
export type Loginpasswordlabel2Inputs = {};
export type Loginsignuplink2Inputs = {};
export type Loginsubmit1Inputs = {};
export type Logintitle1Inputs = {};
export type Memberscancelinviteconfirmaction4Inputs = {};
export type Memberscancelinviteconfirmbody4Inputs = {};
export type Memberscancelinviteconfirmtitle4Inputs = {
  email: NonNullable<unknown>;
};
export type Memberscancelinvitesuccesstoast4Inputs = {
  email: NonNullable<unknown>;
};
export type Memberscolumnactions2Inputs = {};
export type Memberscolumnemail2Inputs = {};
export type Memberscolumnexpires2Inputs = {};
export type Memberscolumnmember2Inputs = {};
export type Memberscolumnrole2Inputs = {};
export type Memberscolumnseeallleads4Inputs = {};
export type Membersdescription1Inputs = {};
export type Membersemaillabel2Inputs = {};
export type Membersinvitationaccept2Inputs = {};
export type Membersinvitationbannertext3Inputs = {
  org: NonNullable<unknown>;
  role: NonNullable<unknown>;
};
export type Membersinvitationexpiredbadge3Inputs = {};
export type Membersinvitationreject2Inputs = {};
export type Membersinvitationresponderrortoast4Inputs = {};
export type Membersinvitebutton2Inputs = {};
export type Membersinvitedialogdescription3Inputs = {};
export type Membersinvitedialogtitle3Inputs = {};
export type Membersinvitesubmit2Inputs = {};
export type Membersinvitesuccesstoast3Inputs = {
  email: NonNullable<unknown>;
};
export type Membersloaderrortitle3Inputs = {};
export type Memberspendingdescription2Inputs = {};
export type Memberspendingemptytitle3Inputs = {};
export type Memberspendingtitle2Inputs = {};
export type Membersremoveconfirmaction3Inputs = {};
export type Membersremoveconfirmbody3Inputs = {};
export type Membersremoveconfirmtitle3Inputs = {
  member: NonNullable<unknown>;
};
export type Membersremovesuccesstoast3Inputs = {
  member: NonNullable<unknown>;
};
export type Membersrolelabel2Inputs = {};
export type Membersseeallleadserrortoast5Inputs = {};
export type Memberstitle1Inputs = {};
export type Offerworkspaceback2Inputs = {};
export type Offerworkspacecampaign2Inputs = {};
export type Offerworkspacecontentmissing3Inputs = {};
export type Offerworkspacecontentready3Inputs = {};
export type Offerworkspacecontenttitle3Inputs = {};
export type Offerworkspacedescription2Inputs = {};
export type Offerworkspacedraft2Inputs = {};
export type Offerworkspaceeditoraction3Inputs = {};
export type Offerworkspaceemptytitle3Inputs = {};
export type Offerworkspaceeyebrow2Inputs = {};
export type Offerworkspaceinboxaction3Inputs = {};
export type Offerworkspaceloaderrortitle4Inputs = {};
export type Offerworkspacenexttitle3Inputs = {};
export type Offerworkspacenocampaign3Inputs = {};
export type Offerworkspaceready2Inputs = {};
export type Offerworkspacestepcontent3Inputs = {};
export type Offerworkspacestepleadform4Inputs = {};
export type Offerworkspacesteppublish3Inputs = {};
export type Offerworkspaceversion2Inputs = {};
export type Offerssurfacenotice2Inputs = {};
export type Onboardingbody1Inputs = {};
export type Onboardingchecklistdismiss2Inputs = {};
export type Onboardingchecklistsubtitle2Inputs = {
  done: NonNullable<unknown>;
  total: NonNullable<unknown>;
};
export type Onboardingchecklisttitle2Inputs = {};
export type Onboardingcreateerror2Inputs = {};
export type Onboardingdeliverylabel2Inputs = {};
export type Onboardingdeliverylink2Inputs = {};
export type Onboardingdeliverymanual2Inputs = {};
export type Onboardingdeliveryschedule2Inputs = {};
export type Onboardingdeliveryzalo2Inputs = {};
export type Onboardingdescriptionlabel2Inputs = {};
export type Onboardingdescriptionplaceholder2Inputs = {};
export type Onboardingorgnamehelp3Inputs = {};
export type Onboardingorgnamelabel3Inputs = {};
export type Onboardingorgnameplaceholder3Inputs = {};
export type Onboardingpricehelp2Inputs = {};
export type Onboardingpricelabel2Inputs = {};
export type Onboardingprivacynote2Inputs = {};
export type Onboardingproductnamelabel3Inputs = {};
export type Onboardingproductnameplaceholder3Inputs = {};
export type Onboardingproducttypelabel3Inputs = {};
export type Onboardingsetupdescription2Inputs = {};
export type Onboardingsetuptitle2Inputs = {};
export type Onboardingstepconnectai3Inputs = {};
export type Onboardingstepconnectpayment3Inputs = {};
export type Onboardingstepcreatelanding3Inputs = {};
export type Onboardingstepcreateproduct3Inputs = {};
export type Onboardingsteppublishlanding3Inputs = {};
export type Onboardingsubmit1Inputs = {};
export type Onboardingtitle1Inputs = {};
export type Orgswitchercreateorg3Inputs = {};
export type Orgswitchernoorg3Inputs = {};
export type Orgswitcherswitcherror3Inputs = {};
export type Paymentaccountnamelabel3Inputs = {};
export type Paymentaccountnumberlabel3Inputs = {};
export type Paymentapikeylabel3Inputs = {};
export type Paymentbankbinlabel3Inputs = {};
export type Paymentconnectbutton2Inputs = {};
export type Paymentconnectdialogdescription3Inputs = {};
export type Paymentconnectdialogtitle3Inputs = {};
export type Paymentconnecterrortoast3Inputs = {};
export type Paymentconnectsubmit2Inputs = {};
export type Paymentconnectioncolumnaccount3Inputs = {};
export type Paymentconnectioncolumnactions3Inputs = {};
export type Paymentconnectioncolumnbank3Inputs = {};
export type Paymentconnectioncolumnstatus3Inputs = {};
export type Paymentconnectionsdescription2Inputs = {};
export type Paymentconnectionsemptytitle3Inputs = {};
export type Paymentconnectionsloaderrortitle4Inputs = {};
export type Paymentconnectionstitle2Inputs = {};
export type Paymentguidedescription2Inputs = {};
export type Paymentguideloaderrortitle4Inputs = {};
export type Paymentguidetitle2Inputs = {};
export type Paymentremoveconfirmbody3Inputs = {};
export type Paymentremoveconfirmtitle3Inputs = {};
export type Paymentremoveconnectionaction3Inputs = {};
export type Paymentremoveconnectionerrortoast4Inputs = {};
export type Platformactiondonetoast3Inputs = {};
export type Platformactionerrortoast3Inputs = {};
export type Platformauditcolumnaction3Inputs = {};
export type Platformauditcolumnmeta3Inputs = {};
export type Platformauditcolumnstaff3Inputs = {};
export type Platformauditcolumntime3Inputs = {};
export type Platformauditdescription2Inputs = {};
export type Platformauditempty2Inputs = {};
export type Platformbacktoorgs3Inputs = {};
export type Platformcancel1Inputs = {};
export type Platformconfirm1Inputs = {};
export type Platformdisableorgaction3Inputs = {};
export type Platformdisableorgdescription3Inputs = {};
export type Platformenableorgaction3Inputs = {};
export type Platformenableorgdescription3Inputs = {};
export type Platformfeatureforceoff3Inputs = {};
export type Platformfeatureforceon3Inputs = {};
export type Platformfeatureplanexcluded3Inputs = {};
export type Platformfeatureplanincluded3Inputs = {};
export type Platformfeaturesempty2Inputs = {};
export type Platformfeatureshint2Inputs = {};
export type Platformfeatureslabel2Inputs = {};
export type Platformorgdetailloaderrortitle5Inputs = {};
export type Platformorgscolumnaicredit4Inputs = {};
export type Platformorgscolumncreated3Inputs = {};
export type Platformorgscolumnname3Inputs = {};
export type Platformorgscolumnowner3Inputs = {};
export type Platformorgscolumnplan3Inputs = {};
export type Platformorgscolumnstatus3Inputs = {};
export type Platformorgsdescription2Inputs = {};
export type Platformorgsemptytitle3Inputs = {};
export type Platformorgsloaderrortitle4Inputs = {};
export type Platformorgsloading2Inputs = {};
export type Platformorgssearchplaceholder3Inputs = {};
export type Platformorgsstatusactive3Inputs = {};
export type Platformorgsstatusall3Inputs = {};
export type Platformorgsstatusdisabled3Inputs = {};
export type Platformorgstitle2Inputs = {};
export type Platformplanlabel2Inputs = {};
export type Platformreasonlabel2Inputs = {};
export type Platformreasonplaceholder2Inputs = {};
export type Platformreasonsharedhint3Inputs = {};
export type Platformrefundassistaction3Inputs = {};
export type Platformrefundassistdescription3Inputs = {};
export type Platformrefundassistdonetoast4Inputs = {};
export type Platformrefundassisttitle3Inputs = {};
export type Platformrefundorderidplaceholder4Inputs = {};
export type Platformstaffaddaction3Inputs = {};
export type Platformstaffaddemailplaceholder4Inputs = {};
export type Platformstaffadderrortoast4Inputs = {};
export type Platformstaffaddsuccesstoast4Inputs = {
  email: NonNullable<unknown>;
};
export type Platformstaffcolumnactions3Inputs = {};
export type Platformstaffcolumncreated3Inputs = {};
export type Platformstaffcolumnemail3Inputs = {};
export type Platformstaffcolumnrole3Inputs = {};
export type Platformstaffdescription2Inputs = {};
export type Platformstaffemptytitle3Inputs = {};
export type Platformstaffloaderrortitle4Inputs = {};
export type Platformstaffloading2Inputs = {};
export type Platformstaffremoveaction3Inputs = {};
export type Platformstaffremoveconfirmaction4Inputs = {};
export type Platformstaffremoveconfirmbody4Inputs = {};
export type Platformstaffremoveconfirmtitle4Inputs = {
  email: NonNullable<unknown>;
};
export type Platformstaffremoveerrortoast4Inputs = {};
export type Platformstaffremovesuccesstoast4Inputs = {
  email: NonNullable<unknown>;
};
export type Platformstataicreditbalance4Inputs = {};
export type Platformstataicreditspent4Inputs = {};
export type Platformstatcampaigns2Inputs = {};
export type Platformstatcreated2Inputs = {};
export type Platformstatleads2Inputs = {};
export type Platformstatmembers2Inputs = {};
export type Platformstattrialuses3Inputs = {};
export type Platformsubscriptiondescription2Inputs = {};
export type Platformsubscriptionsave2Inputs = {};
export type Platformsubscriptiontitle2Inputs = {};
export type Platformtabaudit2Inputs = {};
export type Platformtabbilling2Inputs = {};
export type Platformtaborgs2Inputs = {};
export type Platformtaboverview2Inputs = {};
export type Platformtabstaff2Inputs = {};
export type Productsaddbutton2Inputs = {};
export type Productsadddialogtitle3Inputs = {};
export type Productscolumnactions2Inputs = {};
export type Productscolumnname2Inputs = {};
export type Productscolumnprice2Inputs = {};
export type Productscolumntype2Inputs = {};
export type Productscourseactivationlabel3Inputs = {};
export type Productscourseclassschedulelabel4Inputs = {};
export type Productscoursefieldstitle3Inputs = {};
export type Productscoursestartsatlabel4Inputs = {};
export type Productscoursezalolabel3Inputs = {};
export type Productsdescription1Inputs = {};
export type Productsdescriptionlabel2Inputs = {};
export type Productsdialogdescription2Inputs = {};
export type Productseditdialogtitle3Inputs = {};
export type Productsemptytitle2Inputs = {};
export type Productsimagelabel2Inputs = {};
export type Productsloaderrortitle3Inputs = {};
export type Productsnamelabel2Inputs = {};
export type Productspricelabel2Inputs = {};
export type Productsremoveconfirmaction3Inputs = {};
export type Productsremoveconfirmbody3Inputs = {};
export type Productsremoveconfirmtitle3Inputs = {
  name: NonNullable<unknown>;
};
export type Productsremoveerrortoast3Inputs = {};
export type Productssaveerrortoast3Inputs = {};
export type Productssavesubmit2Inputs = {};
export type Productstitle1Inputs = {};
export type Productstypecourse2Inputs = {};
export type Productstypelabel2Inputs = {};
export type Productstypeother2Inputs = {};
export type Productstypeproduct2Inputs = {};
export type Productstypeservice2Inputs = {};
export type Promptlibrarycopiedtoast3Inputs = {};
export type Promptlibrarydescription2Inputs = {};
export type Promptlibrarydevicedesktop3Inputs = {};
export type Promptlibrarydevicemobile3Inputs = {};
export type Promptlibrarydevicetablet3Inputs = {};
export type Promptlibraryemptytitle3Inputs = {};
export type Promptlibraryfullpromptsection4Inputs = {};
export type Promptlibrarygeneratecta3Inputs = {};
export type Promptlibrarygeneratedialogdescription4Inputs = {};
export type Promptlibrarygeneratedialogtitle4Inputs = {};
export type Promptlibrarygeneratenamelabel4Inputs = {};
export type Promptlibrarygeneratepromptlabel4Inputs = {};
export type Promptlibrarygeneratesubmit3Inputs = {};
export type Promptlibrarygeneratesubmitting3Inputs = {};
export type Promptlibraryimportcta3Inputs = {};
export type Promptlibraryloaderrortitle4Inputs = {};
export type Promptlibrarynopreview3Inputs = {};
export type Promptlibrarypreviewsection3Inputs = {};
export type Promptlibrarysearchplaceholder3Inputs = {};
export type Promptlibrarysummarysection3Inputs = {};
export type Promptlibrarytitle2Inputs = {};
export type Promptlibraryusetemplatecta4Inputs = {};
export type Promptlibraryusetemplatesubmitting4Inputs = {};
export type Prompttemplateaddsection3Inputs = {};
export type Prompttemplateaddvariable3Inputs = {};
export type Prompttemplatecolumnslug3Inputs = {};
export type Prompttemplatecolumntype3Inputs = {};
export type Prompttemplatecolumnversion3Inputs = {};
export type Prompttemplatecompareheading3Inputs = {};
export type Prompttemplatecompareselect3Inputs = {};
export type Prompttemplatecompilebutton3Inputs = {};
export type Prompttemplatecompileerrortoast4Inputs = {};
export type Prompttemplateconnectionlabel3Inputs = {};
export type Prompttemplateconnectionplatform3Inputs = {};
export type Prompttemplateconnectiontrial3Inputs = {};
export type Prompttemplatecreatebutton3Inputs = {};
export type Prompttemplatecreatedialogdescription4Inputs = {};
export type Prompttemplatecreatedialogtitle4Inputs = {};
export type Prompttemplatehistoryemptybody4Inputs = {};
export type Prompttemplatehistoryemptytitle4Inputs = {};
export type Prompttemplatehistoryheading3Inputs = {};
export type Prompttemplatelighthouseunavailable3Inputs = {};
export type Prompttemplatenotfoundtitle4Inputs = {};
export type Prompttemplateopenaction3Inputs = {};
export type Prompttemplateremoveaction3Inputs = {};
export type Prompttemplateremoveconfirmbody4Inputs = {};
export type Prompttemplateremoveconfirmtitle4Inputs = {};
export type Prompttemplateremoveerrortoast4Inputs = {};
export type Prompttemplateremovesection3Inputs = {};
export type Prompttemplateremovevariable3Inputs = {};
export type Prompttemplateruntestbutton4Inputs = {};
export type Prompttemplatesaveerrortoast4Inputs = {};
export type Prompttemplatesectionkeyplaceholder4Inputs = {};
export type Prompttemplatesectionslabel3Inputs = {};
export type Prompttemplatesluglabel3Inputs = {};
export type Prompttemplatetabcompile3Inputs = {};
export type Prompttemplatetabedit3Inputs = {};
export type Prompttemplatetabtestbench4Inputs = {};
export type Prompttemplatetestrunerrortoast5Inputs = {};
export type Prompttemplatetestrunheading4Inputs = {};
export type Prompttemplatevariablekeyplaceholder4Inputs = {};
export type Prompttemplatevariablelabelplaceholder4Inputs = {};
export type Prompttemplatevariablerequired3Inputs = {};
export type Prompttemplatevariableslabel3Inputs = {};
export type Prompttemplatesdescription2Inputs = {};
export type Prompttemplatesemptytitle3Inputs = {};
export type Prompttemplatesloaderrortitle4Inputs = {};
export type Prompttemplatestitle2Inputs = {};
export type Reconciliationcolumnamount2Inputs = {};
export type Reconciliationcolumncandidates2Inputs = {};
export type Reconciliationcolumnreason2Inputs = {};
export type Reconciliationcolumntransaction2Inputs = {};
export type Reconciliationdescription1Inputs = {};
export type Reconciliationdismissaction2Inputs = {};
export type Reconciliationdismissconfirmbody3Inputs = {};
export type Reconciliationdismissconfirmtitle3Inputs = {};
export type Reconciliationdismisserrortoast3Inputs = {};
export type Reconciliationemptytitle2Inputs = {};
export type Reconciliationloaderrortitle3Inputs = {};
export type Reconciliationreasonalreadypaid3Inputs = {};
export type Reconciliationreasonambiguous2Inputs = {};
export type Reconciliationreasonnocandidate3Inputs = {};
export type Reconciliationresolveerrortoast3Inputs = {};
export type Reconciliationsearchbutton2Inputs = {};
export type Reconciliationsearcherrortoast3Inputs = {};
export type Reconciliationsearchnoresults3Inputs = {};
export type Reconciliationsearchplaceholder2Inputs = {};
export type Reconciliationselectorderaction3Inputs = {};
export type Reconciliationsummaryalreadypaid3Inputs = {};
export type Reconciliationsummaryambiguous2Inputs = {};
export type Reconciliationsummarynocandidate3Inputs = {};
export type Reconciliationsummarytitle2Inputs = {};
export type Reconciliationtitle1Inputs = {};
export type Refundrequestscolumnamount3Inputs = {};
export type Refundrequestscolumnlead3Inputs = {};
export type Refundrequestscolumnorder3Inputs = {};
export type Refundrequestscolumnreason3Inputs = {};
export type Refundrequestscolumnstatus3Inputs = {};
export type Refundrequestscompleteerrortoast4Inputs = {};
export type Refundrequestsdescription2Inputs = {};
export type Refundrequestsduplicatebadge3Inputs = {};
export type Refundrequestsemptytitle3Inputs = {};
export type Refundrequestsevidencelabel3Inputs = {};
export type Refundrequestsfilterall3Inputs = {};
export type Refundrequestsloaderrortitle4Inputs = {};
export type Refundrequestsmarktransferredbutton4Inputs = {};
export type Refundrequestsreasoncustomer3Inputs = {};
export type Refundrequestsreasonduplicatepayment4Inputs = {};
export type Refundrequestsreasonother3Inputs = {};
export type Refundrequestsreasonwrongmatch4Inputs = {};
export type Refundrequestsrejectbutton3Inputs = {};
export type Refundrequestsrejectconfirmaction4Inputs = {};
export type Refundrequestsrejectconfirmbody4Inputs = {};
export type Refundrequestsrejectconfirmtitle4Inputs = {};
export type Refundrequestsrejecterrortoast4Inputs = {};
export type Refundrequestsremitterlabel3Inputs = {};
export type Refundrequestsstatuscompleted3Inputs = {};
export type Refundrequestsstatuspending3Inputs = {};
export type Refundrequestsstatusprocessing3Inputs = {};
export type Refundrequestsstatusrejected3Inputs = {};
export type Refundrequeststitle2Inputs = {};
export type Refundrequestsuploaderrortoast4Inputs = {};
export type Refundrequestsuploadevidencebutton4Inputs = {};
export type Resetpasswordinvalidlink3Inputs = {};
export type Resetpasswordnewlabel3Inputs = {};
export type Resetpasswordsubmit2Inputs = {};
export type Resetpasswordsuccess2Inputs = {};
export type Resetpasswordtitle2Inputs = {};
export type Roleadmin1Inputs = {};
export type Roleadmindescription2Inputs = {};
export type Roleeditor1Inputs = {};
export type Roleeditordescription2Inputs = {};
export type Roleowner1Inputs = {};
export type Roleownerdescription2Inputs = {};
export type Rolesales1Inputs = {};
export type Rolesalesdescription2Inputs = {};
export type Routeerrordescription2Inputs = {};
export type Routeerrortitle2Inputs = {};
export type Routenotfoundbackbutton4Inputs = {};
export type Routenotfounddescription3Inputs = {};
export type Routenotfoundtitle3Inputs = {};
export type Settingsbrandbodyfontlabel4Inputs = {};
export type Settingsbrandheadingfontlabel4Inputs = {};
export type Settingsbrandkitdescription3Inputs = {};
export type Settingsbrandkittitle3Inputs = {};
export type Settingsbrandprimarycolorlabel4Inputs = {};
export type Settingsbrandsecondarycolorlabel4Inputs = {};
export type Settingsdescription1Inputs = {};
export type Settingsforbiddentitle2Inputs = {};
export type Settingsleaddigestfrequencydaily4Inputs = {};
export type Settingsleaddigestfrequencydescription4Inputs = {};
export type Settingsleaddigestfrequencyhourly4Inputs = {};
export type Settingsleaddigestfrequencylabel4Inputs = {};
export type Settingsloaderrortitle3Inputs = {};
export type Settingslogodescription2Inputs = {};
export type Settingslogolabel2Inputs = {};
export type Settingssaveerrortoast3Inputs = {};
export type Settingstitle1Inputs = {};
export type Shellainav2Inputs = {};
export type Shellcampaignsnav2Inputs = {};
export type Shellcommandpaletteempty3Inputs = {};
export type Shellcommandpaletteinputplaceholder4Inputs = {};
export type Shelldomainsnav2Inputs = {};
export type Shellgroupadvanced2Inputs = {};
export type Shellgroupai2Inputs = {};
export type Shellgroupcontent2Inputs = {};
export type Shellgrouporganization2Inputs = {};
export type Shellgrouppayments2Inputs = {};
export type Shellgroupsales2Inputs = {};
export type Shellgroupsetup2Inputs = {};
export type Shellgroupworkspace2Inputs = {};
export type Shelllandingsnav2Inputs = {};
export type Shellleadsnav2Inputs = {};
export type Shelllogout1Inputs = {};
export type Shellmembersnav2Inputs = {};
export type Shelloffersnav2Inputs = {};
export type Shellordersnav2Inputs = {};
export type Shellpaymentsnav2Inputs = {};
export type Shellproductsnav2Inputs = {};
export type Shellpromptlibrarynav3Inputs = {};
export type Shellprompttemplatesnav3Inputs = {};
export type Shellreconciliationnav2Inputs = {};
export type Shellrefundrequestsnav3Inputs = {};
export type Shellsearchplaceholder2Inputs = {};
export type Shellsettingsnav2Inputs = {};
export type Shellsignedinas3Inputs = {
  email: NonNullable<unknown>;
};
export type Shellskillsnav2Inputs = {};
export type Shellthemetoggletodark4Inputs = {};
export type Shellthemetoggletolight4Inputs = {};
export type Shelltodaynav2Inputs = {};
export type Signupemaillabel2Inputs = {};
export type Signuphasaccount2Inputs = {};
export type Signuploginlink2Inputs = {};
export type Signupnamelabel2Inputs = {};
export type Signuppasswordlabel2Inputs = {};
export type Signupsubmit1Inputs = {};
export type Signuptitle1Inputs = {};
export type Skillcolumnactions2Inputs = {};
export type Skillcolumndefault2Inputs = {};
export type Skillcolumnname2Inputs = {};
export type Skillcolumnslug2Inputs = {};
export type Skillcolumntype2Inputs = {};
export type Skillcontentlabel2Inputs = {};
export type Skillcreatebutton2Inputs = {};
export type Skillcreatedialogtitle3Inputs = {};
export type Skilldialogdescription2Inputs = {};
export type Skilleditdialogtitle3Inputs = {};
export type Skillnamelabel2Inputs = {};
export type Skillpreviewlabel2Inputs = {};
export type Skillremoveaction2Inputs = {};
export type Skillremoveconfirmbody3Inputs = {};
export type Skillremoveconfirmtitle3Inputs = {};
export type Skillremoveerrortoast3Inputs = {};
export type Skillsaveerrortoast3Inputs = {};
export type Skillsluglabel2Inputs = {};
export type Skilltoggledefaulterrortoast4Inputs = {};
export type Skilltypeplatform2Inputs = {};
export type Skilltypetenant2Inputs = {};
export type Skillsdescription1Inputs = {};
export type Skillsemptytitle2Inputs = {};
export type Skillsloaderrortitle3Inputs = {};
export type Skillstitle1Inputs = {};
export type Studioautosavesaved2Inputs = {
  time: NonNullable<unknown>;
};
export type Studioautosavesaving2Inputs = {};
export type Studiobacktocustomimport4Inputs = {};
export type Studiochatemptydescription3Inputs = {};
export type Studiochatemptytitle3Inputs = {};
export type Studiochatplaceholder2Inputs = {};
export type Studiochatsuggestioncta3Inputs = {};
export type Studiochatsuggestionheadline3Inputs = {};
export type Studiochatsuggestionmobile3Inputs = {};
export type Studiochatviewversionlink4Inputs = {};
export type Studiocomingsoon2Inputs = {};
export type Studiocommentattachlabel3Inputs = {};
export type Studiocommentchatchip3Inputs = {};
export type Studiocommentplaceholder2Inputs = {};
export type Studiocommentqueuebutton3Inputs = {};
export type Studiocommentqueueempty3Inputs = {};
export type Studiocommentqueueerrortoast4Inputs = {};
export type Studiocommentqueuetitle3Inputs = {};
export type Studiocommentqueuedtoast3Inputs = {};
export type Studiocommentsendallbutton4Inputs = {};
export type Studiocommentsendallerrortoast5Inputs = {};
export type Studiocommentsendalltoast4Inputs = {};
export type Studiocommentsenderrortoast4Inputs = {};
export type Studiocommentsendtochatbutton5Inputs = {};
export type Studiocommentsenttoast3Inputs = {};
export type Studiodeleteconfirmaction3Inputs = {};
export type Studiodeleteconfirmcancel3Inputs = {};
export type Studiodeleteconfirmdescription3Inputs = {};
export type Studiodeleteconfirmtitle3Inputs = {};
export type Studiodeleteerrortoast3Inputs = {};
export type Studiodeployhistorytitle3Inputs = {};
export type Studiodeploystatussuperseded3Inputs = {};
export type Studiodeploystatusunpublished3Inputs = {};
export type Studiodesignfilesemptydescription4Inputs = {};
export type Studiodesignfilesemptytitle4Inputs = {};
export type Studiodownload1Inputs = {};
export type Studiodrawclear2Inputs = {};
export type Studiodrawcolorlabel3Inputs = {};
export type Studiodrawpromptplaceholder3Inputs = {};
export type Studiodrawsendbutton3Inputs = {};
export type Studiodrawsenderrortoast4Inputs = {};
export type Studiodrawsending2Inputs = {};
export type Studiodrawtoolarrow3Inputs = {};
export type Studiodrawtoolpen3Inputs = {};
export type Studiodrawtoolrect3Inputs = {};
export type Studiodrawundo2Inputs = {};
export type Studioduplicateerrortoast3Inputs = {};
export type Studioexport1Inputs = {};
export type Studioexporthtml2Inputs = {};
export type Studioexportpng2Inputs = {};
export type Studioexportzip2Inputs = {};
export type Studiofilesfolderassets3Inputs = {};
export type Studiofilesfolderscreenshots3Inputs = {};
export type Studiofilesgroupdata3Inputs = {};
export type Studiofilesgroupfolders3Inputs = {};
export type Studiofilesgroupimages3Inputs = {};
export type Studiofilesgrouppages3Inputs = {};
export type Studiofilesjsonviewerempty4Inputs = {};
export type Studiofilesjsonviewertitle4Inputs = {
  fileName: NonNullable<unknown>;
};
export type Studiofileslightboxempty3Inputs = {};
export type Studiofileslightboxtitle3Inputs = {};
export type Studiofilesprojecttitle3Inputs = {};
export type Studiofilesrefreshlabel3Inputs = {};
export type Studiofilesthumbnailname3Inputs = {};
export type Studiofilesuploadfailed3Inputs = {};
export type Studiofilesuploadlabel3Inputs = {};
export type Studiofilesvideotoolarge4Inputs = {};
export type Studiofunnelfixformprompt4Inputs = {};
export type Studiofunnelfixseoprompt4Inputs = {};
export type Studiofunnelgapsbothdescription4Inputs = {};
export type Studiofunnelgapsdismiss3Inputs = {};
export type Studiofunnelgapsfixbutton4Inputs = {};
export type Studiofunnelgapsformdescription4Inputs = {};
export type Studiofunnelgapsseodescription4Inputs = {};
export type Studiofunnelgapstitle3Inputs = {};
export type Studiogenerateerrorbuycreditsbutton5Inputs = {};
export type Studiogenerateerrorconnectaibutton5Inputs = {};
export type Studiogenerateerrorinsufficientcreditsdescription5Inputs = {};
export type Studiogenerateerrorinsufficientcreditstitle5Inputs = {};
export type Studiogenerateerrornoaiconnectiondescription6Inputs = {};
export type Studiogenerateerrornoaiconnectiontitle6Inputs = {};
export type Studiogenerateerrortrialexhausteddescription5Inputs = {};
export type Studiogenerateerrortrialexhaustedtitle5Inputs = {};
export type Studiohidechat2Inputs = {};
export type Studiohidesidebar2Inputs = {};
export type Studioinspectempty2Inputs = {};
export type Studiolayersdragdisabledhint4Inputs = {
  threshold: NonNullable<unknown>;
};
export type Studiolayersfooterhint3Inputs = {};
export type Studiomoreactionslabel3Inputs = {};
export type Studionativechatsuggestionheadline4Inputs = {};
export type Studionativechatsuggestiontestimonial4Inputs = {};
export type Studionativechatsuggestiontone4Inputs = {};
export type Studionativechattooltitle4Inputs = {};
export type Studionew1Inputs = {};
export type Studiopresent1Inputs = {};
export type Studiopreviewbutton2Inputs = {};
export type Studiopreviewerrortoast3Inputs = {};
export type Studiopublishanywaybutton3Inputs = {};
export type Studiopublishattachcampaignbutton4Inputs = {};
export type Studiopublishauditrequiredtoast4Inputs = {};
export type Studiopublishauditthresholdtoast4Inputs = {};
export type Studiopublishbutton2Inputs = {};
export type Studiopublishcampaignattacherrortoast5Inputs = {};
export type Studiopublishcampaignattachedtoast4Inputs = {};
export type Studiopublishcampaignsearchempty4Inputs = {};
export type Studiopublishcampaignsearchplaceholder4Inputs = {};
export type Studiopublishdialogdescription3Inputs = {};
export type Studiopublishdialogtitle3Inputs = {};
export type Studiopublisherrortoast3Inputs = {};
export type Studiopublishnocampaignwarning4Inputs = {};
export type Studiopublishsubdomainlabel3Inputs = {};
export type Studiopublishsubdomaintakentoast4Inputs = {};
export type Studiopublishsuccesstoast3Inputs = {
  hostname: NonNullable<unknown>;
};
export type Studiopublishunverifiedassetstoast4Inputs = {};
export type Studiopublishupdatebutton3Inputs = {};
export type Studiorenameerrortoast3Inputs = {};
export type Studiorollbackbutton2Inputs = {};
export type Studiorollbackerrortoast3Inputs = {};
export type Studiorollbacksuccesstoast3Inputs = {};
export type Studiosavebutton2Inputs = {};
export type Studiosaveshortcuthint3Inputs = {};
export type Studioseocanonicalurlhint4Inputs = {};
export type Studioseocanonicalurllabel4Inputs = {};
export type Studioseodescriptionlabel3Inputs = {};
export type Studioseonofollowlabel3Inputs = {};
export type Studioseonoindexhint3Inputs = {};
export type Studioseonoindexlabel3Inputs = {};
export type Studioseoogimagelabel4Inputs = {};
export type Studioseopaneltitle3Inputs = {};
export type Studioseosharepreviewlabel4Inputs = {};
export type Studioseosharepreviewnoimage5Inputs = {};
export type Studioseostructureddatatypearticle5Inputs = {};
export type Studioseostructureddatatypeauto5Inputs = {};
export type Studioseostructureddatatypecourse5Inputs = {};
export type Studioseostructureddatatypelabel5Inputs = {};
export type Studioseostructureddatatypelocalbusiness6Inputs = {};
export type Studioseostructureddatatypeorganization5Inputs = {};
export type Studioseostructureddatatypeproduct5Inputs = {};
export type Studioseostructureddatatypewebpage6Inputs = {};
export type Studioseotitlehint3Inputs = {};
export type Studioseotitlelabel3Inputs = {};
export type Studioseotwittercardlabel4Inputs = {};
export type Studioseotwittercardsummary4Inputs = {};
export type Studioseotwittercardsummarylargeimage6Inputs = {};
export type Studioshare1Inputs = {};
export type Studioshortcutcomment2Inputs = {};
export type Studioshortcutdraw2Inputs = {};
export type Studioshortcutedit2Inputs = {};
export type Studioshortcutzoomin3Inputs = {};
export type Studioshortcutzoomout3Inputs = {};
export type Studioshowchat2Inputs = {};
export type Studioshowsidebar2Inputs = {};
export type Studiosidebarfiles2Inputs = {};
export type Studiosidebarhistory2Inputs = {};
export type Studiosidebarinspect2Inputs = {};
export type Studiosidebarlayers2Inputs = {};
export type Studioskillsbutton2Inputs = {};
export type Studioskillsemptytitle3Inputs = {};
export type Studioskillsloaderrortitle4Inputs = {};
export type Studioskillspopoverdescription3Inputs = {};
export type Studioskillspopovertitle3Inputs = {};
export type Studioskillstoggleerrortoast4Inputs = {};
export type Studiotabdesignfiles3Inputs = {};
export type Studiotokenscoloraccentforegroundlabel5Inputs = {};
export type Studiotokenscoloraccentlabel4Inputs = {};
export type Studiotokenscolorborderlabel4Inputs = {};
export type Studiotokenscolorforegroundlabel4Inputs = {};
export type Studiotokenscolormutedlabel4Inputs = {};
export type Studiotokenscolorprimaryforegroundlabel5Inputs = {};
export type Studiotokenscolorprimarylabel4Inputs = {};
export type Studiotokenscolorsurfacelabel4Inputs = {};
export type Studiotokensfontbodylabel4Inputs = {};
export type Studiotokensfontcustomoption4Inputs = {};
export type Studiotokensfontheadinglabel4Inputs = {};
export type Studiotokenspaneltitle3Inputs = {};
export type Studiotokensradiushint3Inputs = {};
export type Studiotokensradiuslabel3Inputs = {};
export type Studiotokensresetbutton3Inputs = {};
export type Studiotoolbarcomment2Inputs = {};
export type Studiotoolbardraw2Inputs = {};
export type Studiotoolbaredit2Inputs = {};
export type Studiotoolbarrefresh2Inputs = {};
export type Studiotoolbartweaks2Inputs = {};
export type Studiotoolbarzoomin3Inputs = {};
export type Studiotoolbarzoomout3Inputs = {};
export type Studiounpublishbutton2Inputs = {};
export type Studiounpublisherrortoast3Inputs = {};
export type Studiounpublishsuccesstoast3Inputs = {};
export type Studioversioncomparebutton3Inputs = {};
export type Studioversioncomparehint3Inputs = {};
export type Studioversioncurrentbadge3Inputs = {};
export type Studioversiondiffcodetab4Inputs = {};
export type Studioversiondiffpreviewtab4Inputs = {};
export type Studioversiondifftitle3Inputs = {};
export type Studioversionhistoryempty3Inputs = {};
export type Studioversionhistorytitle3Inputs = {};
export type Studioversionlabelbutton3Inputs = {};
export type Studioversionlabelerrortoast4Inputs = {};
export type Studioversionlabelplaceholder3Inputs = {};
export type Studioversionlabelsave3Inputs = {};
export type Studioversionoriginaifull4Inputs = {};
export type Studioversionoriginaipatch4Inputs = {};
export type Studioversionoriginimport3Inputs = {};
export type Studioversionoriginmanual3Inputs = {};
export type Studioversionoriginrestore3Inputs = {};
export type Studioversionrestorebutton3Inputs = {};
export type Studioversionrestoreerrortoast4Inputs = {};
export type Studioversionviewchatbutton4Inputs = {};
export type Todaycreatefirstoffer3Inputs = {};
export type Todaycreateoffer2Inputs = {};
export type Todaydescription1Inputs = {};
export type Todayerrortitle2Inputs = {};
export type Todayeyebrow1Inputs = {};
export type Todaygettingstarteddescription3Inputs = {};
export type Todaygettingstartedtitle3Inputs = {};
export type Todayliveoffers2Inputs = {};
export type Todayloading1Inputs = {};
export type Todaynextsteptitle3Inputs = {};
export type Todaynextstepwithoffer4Inputs = {};
export type Todaynextstepwithoutoffer4Inputs = {};
export type Todayopenaction2Inputs = {};
export type Todayqueuedescription2Inputs = {};
export type Todayqueueempty2Inputs = {};
export type Todayqueuetitle2Inputs = {};
export type Todaysystemready2Inputs = {};
export type Todaysystemstatus2Inputs = {};
export type Todaytitle1Inputs = {};
export type Usageinsightsbyeventtitle4Inputs = {};
export type Usageinsightscolumncount3Inputs = {};
export type Usageinsightscolumnevent3Inputs = {};
export type Usageinsightsdescription2Inputs = {};
export type Usageinsightsemptytitle3Inputs = {};
export type Usageinsightsloaderrortitle4Inputs = {};
export type Usageinsightstitle2Inputs = {};
export type Usageinsightstotalevents3Inputs = {};
export type Verifiedbody1Inputs = {};
export type Verifiedloginlink2Inputs = {};
export type Verifiedtitle1Inputs = {};
export type Verifyemailbody2Inputs = {};
export type Verifyemailtitle2Inputs = {};
export declare const acceptinviteerrorbody3: (
  inputs: Acceptinviteerrorbody3Inputs
) => LocalizedString;
export declare const acceptinviteerrortitle3: (
  inputs: Acceptinviteerrortitle3Inputs
) => LocalizedString;
export declare const acceptinvitetitle2: (
  inputs: Acceptinvitetitle2Inputs
) => LocalizedString;
export declare const aiapikeylabel3: (
  inputs: Aiapikeylabel3Inputs
) => LocalizedString;
export declare const aiconnectbutton2: (
  inputs: Aiconnectbutton2Inputs
) => LocalizedString;
export declare const aiconnectdialogdescription3: (
  inputs: Aiconnectdialogdescription3Inputs
) => LocalizedString;
export declare const aiconnectdialogtitle3: (
  inputs: Aiconnectdialogtitle3Inputs
) => LocalizedString;
export declare const aiconnectguidetitle3: (
  inputs: Aiconnectguidetitle3Inputs
) => LocalizedString;
export declare const aiconnectsubmit2: (
  inputs: Aiconnectsubmit2Inputs
) => LocalizedString;
export declare const aiconnectioncolumnactions3: (
  inputs: Aiconnectioncolumnactions3Inputs
) => LocalizedString;
export declare const aiconnectioncolumnmodel3: (
  inputs: Aiconnectioncolumnmodel3Inputs
) => LocalizedString;
export declare const aiconnectioncolumnprovider3: (
  inputs: Aiconnectioncolumnprovider3Inputs
) => LocalizedString;
export declare const aiconnectioncolumnstatus3: (
  inputs: Aiconnectioncolumnstatus3Inputs
) => LocalizedString;
export declare const aiconnectiondefaultbadge3: (
  inputs: Aiconnectiondefaultbadge3Inputs
) => LocalizedString;
export declare const aiconnectionstatusactive3: (
  inputs: Aiconnectionstatusactive3Inputs
) => LocalizedString;
export declare const aiconnectionstatusinvalid3: (
  inputs: Aiconnectionstatusinvalid3Inputs
) => LocalizedString;
export declare const aiconnectionsdescription2: (
  inputs: Aiconnectionsdescription2Inputs
) => LocalizedString;
export declare const aiconnectionsemptytitle3: (
  inputs: Aiconnectionsemptytitle3Inputs
) => LocalizedString;
export declare const aiconnectionsloaderrortitle4: (
  inputs: Aiconnectionsloaderrortitle4Inputs
) => LocalizedString;
export declare const aiconnectionstitle2: (
  inputs: Aiconnectionstitle2Inputs
) => LocalizedString;
export declare const aicreditbalancelabel3: (
  inputs: Aicreditbalancelabel3Inputs
) => LocalizedString;
export declare const aicreditbalancelowwarning4: (
  inputs: Aicreditbalancelowwarning4Inputs
) => LocalizedString;
export declare const aidefaultmodellabel3: (
  inputs: Aidefaultmodellabel3Inputs
) => LocalizedString;
export declare const aifetchmodelsempty3: (
  inputs: Aifetchmodelsempty3Inputs
) => LocalizedString;
export declare const aifetchmodelserror3: (
  inputs: Aifetchmodelserror3Inputs
) => LocalizedString;
export declare const aifetchmodelswaitingforkey5: (
  inputs: Aifetchmodelswaitingforkey5Inputs
) => LocalizedString;
export declare const aiinvalidapikeyerror4: (
  inputs: Aiinvalidapikeyerror4Inputs
) => LocalizedString;
export declare const aiproviderdescriptionanthropic3: (
  inputs: Aiproviderdescriptionanthropic3Inputs
) => LocalizedString;
export declare const aiproviderdescriptiongroq3: (
  inputs: Aiproviderdescriptiongroq3Inputs
) => LocalizedString;
export declare const aiproviderdescriptionnvidia3: (
  inputs: Aiproviderdescriptionnvidia3Inputs
) => LocalizedString;
export declare const aiproviderdescriptionopenai3: (
  inputs: Aiproviderdescriptionopenai3Inputs
) => LocalizedString;
export declare const aiproviderdescriptionopenrouter3: (
  inputs: Aiproviderdescriptionopenrouter3Inputs
) => LocalizedString;
export declare const aiproviderlabel2: (
  inputs: Aiproviderlabel2Inputs
) => LocalizedString;
export declare const airemoveconfirmbody3: (
  inputs: Airemoveconfirmbody3Inputs
) => LocalizedString;
export declare const airemoveconfirmtitle3: (
  inputs: Airemoveconfirmtitle3Inputs
) => LocalizedString;
export declare const airemoveconnectionaction3: (
  inputs: Airemoveconnectionaction3Inputs
) => LocalizedString;
export declare const airemoveconnectionerrortoast4: (
  inputs: Airemoveconnectionerrortoast4Inputs
) => LocalizedString;
export declare const aisetdefaultaction3: (
  inputs: Aisetdefaultaction3Inputs
) => LocalizedString;
export declare const aisetdefaulterrortoast4: (
  inputs: Aisetdefaulterrortoast4Inputs
) => LocalizedString;
export declare const aitrialremaininglabel3: (
  inputs: Aitrialremaininglabel3Inputs
) => LocalizedString;
export declare const aitrialremaininglowwarning4: (
  inputs: Aitrialremaininglowwarning4Inputs
) => LocalizedString;
export declare const aiusagecolumncost3: (
  inputs: Aiusagecolumncost3Inputs
) => LocalizedString;
export declare const aiusagecolumndate3: (
  inputs: Aiusagecolumndate3Inputs
) => LocalizedString;
export declare const aiusagecolumnmodel3: (
  inputs: Aiusagecolumnmodel3Inputs
) => LocalizedString;
export declare const aiusagecolumntokens3: (
  inputs: Aiusagecolumntokens3Inputs
) => LocalizedString;
export declare const aiusagedescription2: (
  inputs: Aiusagedescription2Inputs
) => LocalizedString;
export declare const aiusageemptytitle3: (
  inputs: Aiusageemptytitle3Inputs
) => LocalizedString;
export declare const aiusageloaderrortitle4: (
  inputs: Aiusageloaderrortitle4Inputs
) => LocalizedString;
export declare const aiusagetitle2: (
  inputs: Aiusagetitle2Inputs
) => LocalizedString;
export declare const appname1: (inputs: Appname1Inputs) => LocalizedString;
export declare const apptagline1: (
  inputs: Apptagline1Inputs
) => LocalizedString;
export declare const auditlogcolumnaction3: (
  inputs: Auditlogcolumnaction3Inputs
) => LocalizedString;
export declare const auditlogcolumnactor3: (
  inputs: Auditlogcolumnactor3Inputs
) => LocalizedString;
export declare const auditlogcolumntarget3: (
  inputs: Auditlogcolumntarget3Inputs
) => LocalizedString;
export declare const auditlogcolumntime3: (
  inputs: Auditlogcolumntime3Inputs
) => LocalizedString;
export declare const auditlogdescription2: (
  inputs: Auditlogdescription2Inputs
) => LocalizedString;
export declare const auditlogemptytitle3: (
  inputs: Auditlogemptytitle3Inputs
) => LocalizedString;
export declare const auditlogloaderrortitle4: (
  inputs: Auditlogloaderrortitle4Inputs
) => LocalizedString;
export declare const auditlogtitle2: (
  inputs: Auditlogtitle2Inputs
) => LocalizedString;
export declare const authsidedescription2: (
  inputs: Authsidedescription2Inputs
) => LocalizedString;
export declare const authsidetitle2: (
  inputs: Authsidetitle2Inputs
) => LocalizedString;
export declare const authvaluelead2: (
  inputs: Authvaluelead2Inputs
) => LocalizedString;
export declare const authvaluenext2: (
  inputs: Authvaluenext2Inputs
) => LocalizedString;
export declare const authvaluepayment2: (
  inputs: Authvaluepayment2Inputs
) => LocalizedString;
export declare const campaignsaccountnamelabel3: (
  inputs: Campaignsaccountnamelabel3Inputs
) => LocalizedString;
export declare const campaignsaccountnumberlabel3: (
  inputs: Campaignsaccountnumberlabel3Inputs
) => LocalizedString;
export declare const campaignsactionduplicate2: (
  inputs: Campaignsactionduplicate2Inputs
) => LocalizedString;
export declare const campaignsaddbutton2: (
  inputs: Campaignsaddbutton2Inputs
) => LocalizedString;
export declare const campaignsadddialogtitle3: (
  inputs: Campaignsadddialogtitle3Inputs
) => LocalizedString;
export declare const campaignsanalyticsaction2: (
  inputs: Campaignsanalyticsaction2Inputs
) => LocalizedString;
export declare const campaignsanalyticsbysourceempty4: (
  inputs: Campaignsanalyticsbysourceempty4Inputs
) => LocalizedString;
export declare const campaignsanalyticsbysourcetitle4: (
  inputs: Campaignsanalyticsbysourcetitle4Inputs
) => LocalizedString;
export declare const campaignsanalyticsconversion2: (
  inputs: Campaignsanalyticsconversion2Inputs
) => LocalizedString;
export declare const campaignsanalyticsdescription2: (
  inputs: Campaignsanalyticsdescription2Inputs
) => LocalizedString;
export declare const campaignsanalyticsloaderrortitle4: (
  inputs: Campaignsanalyticsloaderrortitle4Inputs
) => LocalizedString;
export declare const campaignsanalyticsorders2: (
  inputs: Campaignsanalyticsorders2Inputs
) => LocalizedString;
export declare const campaignsanalyticsrevenue2: (
  inputs: Campaignsanalyticsrevenue2Inputs
) => LocalizedString;
export declare const campaignsanalyticssourcecolumn3: (
  inputs: Campaignsanalyticssourcecolumn3Inputs
) => LocalizedString;
export declare const campaignsanalyticssubmits2: (
  inputs: Campaignsanalyticssubmits2Inputs
) => LocalizedString;
export declare const campaignsanalyticstitle2: (
  inputs: Campaignsanalyticstitle2Inputs
) => LocalizedString;
export declare const campaignsanalyticsviews2: (
  inputs: Campaignsanalyticsviews2Inputs
) => LocalizedString;
export declare const campaignsassignmentmodelabel3: (
  inputs: Campaignsassignmentmodelabel3Inputs
) => LocalizedString;
export declare const campaignsassignmentmodemanual3: (
  inputs: Campaignsassignmentmodemanual3Inputs
) => LocalizedString;
export declare const campaignsassignmentmoderoundrobin4: (
  inputs: Campaignsassignmentmoderoundrobin4Inputs
) => LocalizedString;
export declare const campaignsbankbinlabel3: (
  inputs: Campaignsbankbinlabel3Inputs
) => LocalizedString;
export declare const campaignsbulkchangestatus3: (
  inputs: Campaignsbulkchangestatus3Inputs
) => LocalizedString;
export declare const campaignsbulkclearselection3: (
  inputs: Campaignsbulkclearselection3Inputs
) => LocalizedString;
export declare const campaignsbulkdelete2: (
  inputs: Campaignsbulkdelete2Inputs
) => LocalizedString;
export declare const campaignsbulkdeleteconfirm3: (
  inputs: Campaignsbulkdeleteconfirm3Inputs
) => LocalizedString;
export declare const campaignsbulkdeleteconfirmdescription4: (
  inputs: Campaignsbulkdeleteconfirmdescription4Inputs
) => LocalizedString;
export declare const campaignsbulkdeleteconfirmtitle4: (
  inputs: Campaignsbulkdeleteconfirmtitle4Inputs
) => LocalizedString;
export declare const campaignsbulkdeleteerrortoast4: (
  inputs: Campaignsbulkdeleteerrortoast4Inputs
) => LocalizedString;
export declare const campaignsbulkselectedcount3: (
  inputs: Campaignsbulkselectedcount3Inputs
) => LocalizedString;
export declare const campaignsbulkstatuserrortoast4: (
  inputs: Campaignsbulkstatuserrortoast4Inputs
) => LocalizedString;
export declare const campaignscolumnactions2: (
  inputs: Campaignscolumnactions2Inputs
) => LocalizedString;
export declare const campaignscolumnname2: (
  inputs: Campaignscolumnname2Inputs
) => LocalizedString;
export declare const campaignscolumnproducts2: (
  inputs: Campaignscolumnproducts2Inputs
) => LocalizedString;
export declare const campaignscolumnstatus2: (
  inputs: Campaignscolumnstatus2Inputs
) => LocalizedString;
export declare const campaignsdescription1: (
  inputs: Campaignsdescription1Inputs
) => LocalizedString;
export declare const campaignsdialogdescription2: (
  inputs: Campaignsdialogdescription2Inputs
) => LocalizedString;
export declare const campaignsduplicateerrortoast3: (
  inputs: Campaignsduplicateerrortoast3Inputs
) => LocalizedString;
export declare const campaignseditdialogtitle3: (
  inputs: Campaignseditdialogtitle3Inputs
) => LocalizedString;
export declare const campaignsemptytitle2: (
  inputs: Campaignsemptytitle2Inputs
) => LocalizedString;
export declare const campaignsendsatlabel3: (
  inputs: Campaignsendsatlabel3Inputs
) => LocalizedString;
export declare const campaignsformfieldaddbutton4: (
  inputs: Campaignsformfieldaddbutton4Inputs
) => LocalizedString;
export declare const campaignsformfieldkeyplaceholder4: (
  inputs: Campaignsformfieldkeyplaceholder4Inputs
) => LocalizedString;
export declare const campaignsformfieldlabellabel4: (
  inputs: Campaignsformfieldlabellabel4Inputs
) => LocalizedString;
export declare const campaignsformfieldoptionsplaceholder4: (
  inputs: Campaignsformfieldoptionsplaceholder4Inputs
) => LocalizedString;
export declare const campaignsformfieldremovelabel4: (
  inputs: Campaignsformfieldremovelabel4Inputs
) => LocalizedString;
export declare const campaignsformfieldrequiredlabel4: (
  inputs: Campaignsformfieldrequiredlabel4Inputs
) => LocalizedString;
export declare const campaignsformfieldtypecheckbox4: (
  inputs: Campaignsformfieldtypecheckbox4Inputs
) => LocalizedString;
export declare const campaignsformfieldtypeselect4: (
  inputs: Campaignsformfieldtypeselect4Inputs
) => LocalizedString;
export declare const campaignsformfieldtypetext4: (
  inputs: Campaignsformfieldtypetext4Inputs
) => LocalizedString;
export declare const campaignsformfieldsempty3: (
  inputs: Campaignsformfieldsempty3Inputs
) => LocalizedString;
export declare const campaignsformfieldssectiontitle4: (
  inputs: Campaignsformfieldssectiontitle4Inputs
) => LocalizedString;
export declare const campaignsgoallabel2: (
  inputs: Campaignsgoallabel2Inputs
) => LocalizedString;
export declare const campaignsloaderrortitle3: (
  inputs: Campaignsloaderrortitle3Inputs
) => LocalizedString;
export declare const campaignsnamelabel2: (
  inputs: Campaignsnamelabel2Inputs
) => LocalizedString;
export declare const campaignsogimagecreatefirsthint5: (
  inputs: Campaignsogimagecreatefirsthint5Inputs
) => LocalizedString;
export declare const campaignsogimagedescription3: (
  inputs: Campaignsogimagedescription3Inputs
) => LocalizedString;
export declare const campaignsogimagelabel3: (
  inputs: Campaignsogimagelabel3Inputs
) => LocalizedString;
export declare const campaignsogimagesectiontitle4: (
  inputs: Campaignsogimagesectiontitle4Inputs
) => LocalizedString;
export declare const campaignspaymentenabledlabel3: (
  inputs: Campaignspaymentenabledlabel3Inputs
) => LocalizedString;
export declare const campaignspaymentsectiontitle3: (
  inputs: Campaignspaymentsectiontitle3Inputs
) => LocalizedString;
export declare const campaignspopupbodylabel3: (
  inputs: Campaignspopupbodylabel3Inputs
) => LocalizedString;
export declare const campaignspopupmanualpendingtitle4: (
  inputs: Campaignspopupmanualpendingtitle4Inputs
) => LocalizedString;
export declare const campaignspopuppaidtitle3: (
  inputs: Campaignspopuppaidtitle3Inputs
) => LocalizedString;
export declare const campaignspopupregisteredtitle3: (
  inputs: Campaignspopupregisteredtitle3Inputs
) => LocalizedString;
export declare const campaignspopuptitlelabel3: (
  inputs: Campaignspopuptitlelabel3Inputs
) => LocalizedString;
export declare const campaignspopupssectiontitle3: (
  inputs: Campaignspopupssectiontitle3Inputs
) => LocalizedString;
export declare const campaignsproductsempty2: (
  inputs: Campaignsproductsempty2Inputs
) => LocalizedString;
export declare const campaignsproductslabel2: (
  inputs: Campaignsproductslabel2Inputs
) => LocalizedString;
export declare const campaignsremoveconfirmaction3: (
  inputs: Campaignsremoveconfirmaction3Inputs
) => LocalizedString;
export declare const campaignsremoveconfirmbody3: (
  inputs: Campaignsremoveconfirmbody3Inputs
) => LocalizedString;
export declare const campaignsremoveconfirmtitle3: (
  inputs: Campaignsremoveconfirmtitle3Inputs
) => LocalizedString;
export declare const campaignsremoveerrortoast3: (
  inputs: Campaignsremoveerrortoast3Inputs
) => LocalizedString;
export declare const campaignssaveerrortoast3: (
  inputs: Campaignssaveerrortoast3Inputs
) => LocalizedString;
export declare const campaignssavesubmit2: (
  inputs: Campaignssavesubmit2Inputs
) => LocalizedString;
export declare const campaignssearchplaceholder2: (
  inputs: Campaignssearchplaceholder2Inputs
) => LocalizedString;
export declare const campaignssepayautolabel3: (
  inputs: Campaignssepayautolabel3Inputs
) => LocalizedString;
export declare const campaignsstartsatlabel3: (
  inputs: Campaignsstartsatlabel3Inputs
) => LocalizedString;
export declare const campaignsstatusactive2: (
  inputs: Campaignsstatusactive2Inputs
) => LocalizedString;
export declare const campaignsstatusdraft2: (
  inputs: Campaignsstatusdraft2Inputs
) => LocalizedString;
export declare const campaignsstatusended2: (
  inputs: Campaignsstatusended2Inputs
) => LocalizedString;
export declare const campaignsstatuslabel2: (
  inputs: Campaignsstatuslabel2Inputs
) => LocalizedString;
export declare const campaignsstatuspaused2: (
  inputs: Campaignsstatuspaused2Inputs
) => LocalizedString;
export declare const campaignstitle1: (
  inputs: Campaignstitle1Inputs
) => LocalizedString;
export declare const campaignstransferprefixlabel3: (
  inputs: Campaignstransferprefixlabel3Inputs
) => LocalizedString;
export declare const campaignsutmdefaultaddbutton4: (
  inputs: Campaignsutmdefaultaddbutton4Inputs
) => LocalizedString;
export declare const campaignsutmdefaultsempty3: (
  inputs: Campaignsutmdefaultsempty3Inputs
) => LocalizedString;
export declare const campaignsutmdefaultssectiontitle4: (
  inputs: Campaignsutmdefaultssectiontitle4Inputs
) => LocalizedString;
export declare const campaignsutmkeyplaceholder3: (
  inputs: Campaignsutmkeyplaceholder3Inputs
) => LocalizedString;
export declare const campaignsutmremovelabel3: (
  inputs: Campaignsutmremovelabel3Inputs
) => LocalizedString;
export declare const campaignsutmvalueplaceholder3: (
  inputs: Campaignsutmvalueplaceholder3Inputs
) => LocalizedString;
export declare const campaignszalourllabel3: (
  inputs: Campaignszalourllabel3Inputs
) => LocalizedString;
export declare const commonall1: (inputs: Commonall1Inputs) => LocalizedString;
export declare const commonback1: (
  inputs: Commonback1Inputs
) => LocalizedString;
export declare const commoncancel1: (
  inputs: Commoncancel1Inputs
) => LocalizedString;
export declare const commoncontinue1: (
  inputs: Commoncontinue1Inputs
) => LocalizedString;
export declare const commoncopy1: (
  inputs: Commoncopy1Inputs
) => LocalizedString;
export declare const commoncreate1: (
  inputs: Commoncreate1Inputs
) => LocalizedString;
export declare const commonedit1: (
  inputs: Commonedit1Inputs
) => LocalizedString;
export declare const commonloadmore2: (
  inputs: Commonloadmore2Inputs
) => LocalizedString;
export declare const commonloading1: (
  inputs: Commonloading1Inputs
) => LocalizedString;
export declare const commonnext1: (
  inputs: Commonnext1Inputs
) => LocalizedString;
export declare const commonpaginationlabel2: (
  inputs: Commonpaginationlabel2Inputs
) => LocalizedString;
export declare const commonprevious1: (
  inputs: Commonprevious1Inputs
) => LocalizedString;
export declare const commonredo1: (
  inputs: Commonredo1Inputs
) => LocalizedString;
export declare const commonretry1: (
  inputs: Commonretry1Inputs
) => LocalizedString;
export declare const commonsave1: (
  inputs: Commonsave1Inputs
) => LocalizedString;
export declare const commonundo1: (
  inputs: Commonundo1Inputs
) => LocalizedString;
export declare const domainsaddbutton2: (
  inputs: Domainsaddbutton2Inputs
) => LocalizedString;
export declare const domainsadddialogdescription3: (
  inputs: Domainsadddialogdescription3Inputs
) => LocalizedString;
export declare const domainsadddialogtitle3: (
  inputs: Domainsadddialogtitle3Inputs
) => LocalizedString;
export declare const domainsadderrortoast3: (
  inputs: Domainsadderrortoast3Inputs
) => LocalizedString;
export declare const domainsaddsubmit2: (
  inputs: Domainsaddsubmit2Inputs
) => LocalizedString;
export declare const domainscnamehostlabel3: (
  inputs: Domainscnamehostlabel3Inputs
) => LocalizedString;
export declare const domainscnamevaluelabel3: (
  inputs: Domainscnamevaluelabel3Inputs
) => LocalizedString;
export declare const domainscolumnactions2: (
  inputs: Domainscolumnactions2Inputs
) => LocalizedString;
export declare const domainscolumnhostname2: (
  inputs: Domainscolumnhostname2Inputs
) => LocalizedString;
export declare const domainscolumnstatus2: (
  inputs: Domainscolumnstatus2Inputs
) => LocalizedString;
export declare const domainsdescription1: (
  inputs: Domainsdescription1Inputs
) => LocalizedString;
export declare const domainsdnsinstructionsintro3: (
  inputs: Domainsdnsinstructionsintro3Inputs
) => LocalizedString;
export declare const domainsemptytitle2: (
  inputs: Domainsemptytitle2Inputs
) => LocalizedString;
export declare const domainshostnamelabel2: (
  inputs: Domainshostnamelabel2Inputs
) => LocalizedString;
export declare const domainslandingpagelabel3: (
  inputs: Domainslandingpagelabel3Inputs
) => LocalizedString;
export declare const domainslandingpageplaceholder3: (
  inputs: Domainslandingpageplaceholder3Inputs
) => LocalizedString;
export declare const domainsloaderrortitle3: (
  inputs: Domainsloaderrortitle3Inputs
) => LocalizedString;
export declare const domainsremoveaction2: (
  inputs: Domainsremoveaction2Inputs
) => LocalizedString;
export declare const domainsremoveconfirmbody3: (
  inputs: Domainsremoveconfirmbody3Inputs
) => LocalizedString;
export declare const domainsremoveconfirmtitle3: (
  inputs: Domainsremoveconfirmtitle3Inputs
) => LocalizedString;
export declare const domainsremoveerrortoast3: (
  inputs: Domainsremoveerrortoast3Inputs
) => LocalizedString;
export declare const domainsstatusactive2: (
  inputs: Domainsstatusactive2Inputs
) => LocalizedString;
export declare const domainsstatusfailed2: (
  inputs: Domainsstatusfailed2Inputs
) => LocalizedString;
export declare const domainsstatuspending2: (
  inputs: Domainsstatuspending2Inputs
) => LocalizedString;
export declare const domainstitle1: (
  inputs: Domainstitle1Inputs
) => LocalizedString;
export declare const domainstxthostlabel3: (
  inputs: Domainstxthostlabel3Inputs
) => LocalizedString;
export declare const domainstxtinstructionsintro3: (
  inputs: Domainstxtinstructionsintro3Inputs
) => LocalizedString;
export declare const domainstxtvaluelabel3: (
  inputs: Domainstxtvaluelabel3Inputs
) => LocalizedString;
export declare const domainsupgraderequireddescription3: (
  inputs: Domainsupgraderequireddescription3Inputs
) => LocalizedString;
export declare const domainsupgraderequiredtitle3: (
  inputs: Domainsupgraderequiredtitle3Inputs
) => LocalizedString;
export declare const domainsvaluecopiedtoast3: (
  inputs: Domainsvaluecopiedtoast3Inputs
) => LocalizedString;
export declare const domainsverifybutton2: (
  inputs: Domainsverifybutton2Inputs
) => LocalizedString;
export declare const domainsverifyerrortoast3: (
  inputs: Domainsverifyerrortoast3Inputs
) => LocalizedString;
export declare const entityimagedrophint3: (
  inputs: Entityimagedrophint3Inputs
) => LocalizedString;
export declare const entityimageremovelabel3: (
  inputs: Entityimageremovelabel3Inputs
) => LocalizedString;
export declare const entityimageuploadfailed3: (
  inputs: Entityimageuploadfailed3Inputs
) => LocalizedString;
export declare const entityimageuploadlabel3: (
  inputs: Entityimageuploadlabel3Inputs
) => LocalizedString;
export declare const entityimageuploadinglabel3: (
  inputs: Entityimageuploadinglabel3Inputs
) => LocalizedString;
export declare const featureupgradegenericdescription3: (
  inputs: Featureupgradegenericdescription3Inputs
) => LocalizedString;
export declare const featureupgradegenerictitle3: (
  inputs: Featureupgradegenerictitle3Inputs
) => LocalizedString;
export declare const forgotpasswordbody2: (
  inputs: Forgotpasswordbody2Inputs
) => LocalizedString;
export declare const forgotpasswordemaillabel3: (
  inputs: Forgotpasswordemaillabel3Inputs
) => LocalizedString;
export declare const forgotpasswordsent2: (
  inputs: Forgotpasswordsent2Inputs
) => LocalizedString;
export declare const forgotpasswordsubmit2: (
  inputs: Forgotpasswordsubmit2Inputs
) => LocalizedString;
export declare const forgotpasswordtitle2: (
  inputs: Forgotpasswordtitle2Inputs
) => LocalizedString;
export declare const inboxdescription1: (
  inputs: Inboxdescription1Inputs
) => LocalizedString;
export declare const inboxtitle1: (
  inputs: Inboxtitle1Inputs
) => LocalizedString;
export declare const landingsactionassigntocampaign4: (
  inputs: Landingsactionassigntocampaign4Inputs
) => LocalizedString;
export declare const landingsactiondelete2: (
  inputs: Landingsactiondelete2Inputs
) => LocalizedString;
export declare const landingsactionduplicate2: (
  inputs: Landingsactionduplicate2Inputs
) => LocalizedString;
export declare const landingsactionremovefromcampaign4: (
  inputs: Landingsactionremovefromcampaign4Inputs
) => LocalizedString;
export declare const landingsactionrename2: (
  inputs: Landingsactionrename2Inputs
) => LocalizedString;
export declare const landingsactionviewlive3: (
  inputs: Landingsactionviewlive3Inputs
) => LocalizedString;
export declare const landingsassigntocampaignempty4: (
  inputs: Landingsassigntocampaignempty4Inputs
) => LocalizedString;
export declare const landingsbadgedraft2: (
  inputs: Landingsbadgedraft2Inputs
) => LocalizedString;
export declare const landingsbadgepublished2: (
  inputs: Landingsbadgepublished2Inputs
) => LocalizedString;
export declare const landingscardactionslabel3: (
  inputs: Landingscardactionslabel3Inputs
) => LocalizedString;
export declare const landingscreateaibutton3: (
  inputs: Landingscreateaibutton3Inputs
) => LocalizedString;
export declare const landingscreateaidescription3: (
  inputs: Landingscreateaidescription3Inputs
) => LocalizedString;
export declare const landingscreateaititle3: (
  inputs: Landingscreateaititle3Inputs
) => LocalizedString;
export declare const landingscreateerrortoast3: (
  inputs: Landingscreateerrortoast3Inputs
) => LocalizedString;
export declare const landingscreateheading2: (
  inputs: Landingscreateheading2Inputs
) => LocalizedString;
export declare const landingscreateimportdescription3: (
  inputs: Landingscreateimportdescription3Inputs
) => LocalizedString;
export declare const landingscreateimporttitle3: (
  inputs: Landingscreateimporttitle3Inputs
) => LocalizedString;
export declare const landingscreatemanualbutton3: (
  inputs: Landingscreatemanualbutton3Inputs
) => LocalizedString;
export declare const landingscreatemanualdescription3: (
  inputs: Landingscreatemanualdescription3Inputs
) => LocalizedString;
export declare const landingscreatemanualtitle3: (
  inputs: Landingscreatemanualtitle3Inputs
) => LocalizedString;
export declare const landingsfilterall2: (
  inputs: Landingsfilterall2Inputs
) => LocalizedString;
export declare const landingsfilterallcampaigns3: (
  inputs: Landingsfilterallcampaigns3Inputs
) => LocalizedString;
export declare const landingsfilterdraft2: (
  inputs: Landingsfilterdraft2Inputs
) => LocalizedString;
export declare const landingsfilterpublished2: (
  inputs: Landingsfilterpublished2Inputs
) => LocalizedString;
export declare const landingsimportbutton2: (
  inputs: Landingsimportbutton2Inputs
) => LocalizedString;
export declare const landingsimportdialogtitle3: (
  inputs: Landingsimportdialogtitle3Inputs
) => LocalizedString;
export declare const landingsimporterrortoast3: (
  inputs: Landingsimporterrortoast3Inputs
) => LocalizedString;
export declare const landingsimporthtmlplaceholder3: (
  inputs: Landingsimporthtmlplaceholder3Inputs
) => LocalizedString;
export declare const landingsimportnamelabel3: (
  inputs: Landingsimportnamelabel3Inputs
) => LocalizedString;
export declare const landingsimportnameplaceholder3: (
  inputs: Landingsimportnameplaceholder3Inputs
) => LocalizedString;
export declare const landingsimportsubmitbutton3: (
  inputs: Landingsimportsubmitbutton3Inputs
) => LocalizedString;
export declare const landingsimportsubmitting2: (
  inputs: Landingsimportsubmitting2Inputs
) => LocalizedString;
export declare const landingsimporttabfile3: (
  inputs: Landingsimporttabfile3Inputs
) => LocalizedString;
export declare const landingsimporttabhtml3: (
  inputs: Landingsimporttabhtml3Inputs
) => LocalizedString;
export declare const landingsimporttaburl3: (
  inputs: Landingsimporttaburl3Inputs
) => LocalizedString;
export declare const landingsimporturlplaceholder3: (
  inputs: Landingsimporturlplaceholder3Inputs
) => LocalizedString;
export declare const landingsloaderrortitle3: (
  inputs: Landingsloaderrortitle3Inputs
) => LocalizedString;
export declare const landingsnoresultstitle3: (
  inputs: Landingsnoresultstitle3Inputs
) => LocalizedString;
export declare const landingsreuploadbutton2: (
  inputs: Landingsreuploadbutton2Inputs
) => LocalizedString;
export declare const landingsreuploaddialogtitle3: (
  inputs: Landingsreuploaddialogtitle3Inputs
) => LocalizedString;
export declare const landingssearchplaceholder2: (
  inputs: Landingssearchplaceholder2Inputs
) => LocalizedString;
export declare const landingssortname2: (
  inputs: Landingssortname2Inputs
) => LocalizedString;
export declare const landingssortupdated2: (
  inputs: Landingssortupdated2Inputs
) => LocalizedString;
export declare const landingssummarycount2: (
  inputs: Landingssummarycount2Inputs
) => LocalizedString;
export declare const landingsupdatedat2: (
  inputs: Landingsupdatedat2Inputs
) => LocalizedString;
export declare const leadsourcecsvimport3: (
  inputs: Leadsourcecsvimport3Inputs
) => LocalizedString;
export declare const leadsourcedirect2: (
  inputs: Leadsourcedirect2Inputs
) => LocalizedString;
export declare const leadsourcefacebook2: (
  inputs: Leadsourcefacebook2Inputs
) => LocalizedString;
export declare const leadsourcegeneric2: (
  inputs: Leadsourcegeneric2Inputs
) => LocalizedString;
export declare const leadsourcegoogleads3: (
  inputs: Leadsourcegoogleads3Inputs
) => LocalizedString;
export declare const leadsourcelandingpage3: (
  inputs: Leadsourcelandingpage3Inputs
) => LocalizedString;
export declare const leadsourcemanual2: (
  inputs: Leadsourcemanual2Inputs
) => LocalizedString;
export declare const leadsourcetiktok2: (
  inputs: Leadsourcetiktok2Inputs
) => LocalizedString;
export declare const leadsourcezalooa3: (
  inputs: Leadsourcezalooa3Inputs
) => LocalizedString;
export declare const leadsactioncall2: (
  inputs: Leadsactioncall2Inputs
) => LocalizedString;
export declare const leadsactioncopiedtoast3: (
  inputs: Leadsactioncopiedtoast3Inputs
) => LocalizedString;
export declare const leadsactioncopyphone3: (
  inputs: Leadsactioncopyphone3Inputs
) => LocalizedString;
export declare const leadsactionzalo2: (
  inputs: Leadsactionzalo2Inputs
) => LocalizedString;
export declare const leadsactivitycall2: (
  inputs: Leadsactivitycall2Inputs
) => LocalizedString;
export declare const leadsactivitynote2: (
  inputs: Leadsactivitynote2Inputs
) => LocalizedString;
export declare const leadsactivityordercreated3: (
  inputs: Leadsactivityordercreated3Inputs
) => LocalizedString;
export declare const leadsactivitypayment2: (
  inputs: Leadsactivitypayment2Inputs
) => LocalizedString;
export declare const leadsactivityresubmit2: (
  inputs: Leadsactivityresubmit2Inputs
) => LocalizedString;
export declare const leadsactivitystagechange3: (
  inputs: Leadsactivitystagechange3Inputs
) => LocalizedString;
export declare const leadsactivitysystem2: (
  inputs: Leadsactivitysystem2Inputs
) => LocalizedString;
export declare const leadsaddnotelabel3: (
  inputs: Leadsaddnotelabel3Inputs
) => LocalizedString;
export declare const leadsaddnoteplaceholder3: (
  inputs: Leadsaddnoteplaceholder3Inputs
) => LocalizedString;
export declare const leadsaddnotesubmit3: (
  inputs: Leadsaddnotesubmit3Inputs
) => LocalizedString;
export declare const leadsagedays2: (
  inputs: Leadsagedays2Inputs
) => LocalizedString;
export declare const leadsagehours2: (
  inputs: Leadsagehours2Inputs
) => LocalizedString;
export declare const leadsassignerrortoast3: (
  inputs: Leadsassignerrortoast3Inputs
) => LocalizedString;
export declare const leadsassigneelabel2: (
  inputs: Leadsassigneelabel2Inputs
) => LocalizedString;
export declare const leadsassignmentaddrule3: (
  inputs: Leadsassignmentaddrule3Inputs
) => LocalizedString;
export declare const leadsassignmentdeleteerrortoast4: (
  inputs: Leadsassignmentdeleteerrortoast4Inputs
) => LocalizedString;
export declare const leadsassignmenteditrule3: (
  inputs: Leadsassignmenteditrule3Inputs
) => LocalizedString;
export declare const leadsassignmentemptytitle3: (
  inputs: Leadsassignmentemptytitle3Inputs
) => LocalizedString;
export declare const leadsassignmentfixedassigneelabel4: (
  inputs: Leadsassignmentfixedassigneelabel4Inputs
) => LocalizedString;
export declare const leadsassignmentpersonalabel3: (
  inputs: Leadsassignmentpersonalabel3Inputs
) => LocalizedString;
export declare const leadsassignmentpersonaplaceholder3: (
  inputs: Leadsassignmentpersonaplaceholder3Inputs
) => LocalizedString;
export declare const leadsassignmentpoollabel3: (
  inputs: Leadsassignmentpoollabel3Inputs
) => LocalizedString;
export declare const leadsassignmentreordererrortoast4: (
  inputs: Leadsassignmentreordererrortoast4Inputs
) => LocalizedString;
export declare const leadsassignmentruledeletelabel4: (
  inputs: Leadsassignmentruledeletelabel4Inputs
) => LocalizedString;
export declare const leadsassignmentrulesdescription3: (
  inputs: Leadsassignmentrulesdescription3Inputs
) => LocalizedString;
export declare const leadsassignmentrulesmanagebutton4: (
  inputs: Leadsassignmentrulesmanagebutton4Inputs
) => LocalizedString;
export declare const leadsassignmentrulestitle3: (
  inputs: Leadsassignmentrulestitle3Inputs
) => LocalizedString;
export declare const leadsassignmentsaveerrortoast4: (
  inputs: Leadsassignmentsaveerrortoast4Inputs
) => LocalizedString;
export declare const leadsassignmentslabreachlabel4: (
  inputs: Leadsassignmentslabreachlabel4Inputs
) => LocalizedString;
export declare const leadsassignmentslabreachnone4: (
  inputs: Leadsassignmentslabreachnone4Inputs
) => LocalizedString;
export declare const leadsassignmentslabreachnotify4: (
  inputs: Leadsassignmentslabreachnotify4Inputs
) => LocalizedString;
export declare const leadsassignmentslabreachreassign4: (
  inputs: Leadsassignmentslabreachreassign4Inputs
) => LocalizedString;
export declare const leadsassignmentslalabel3: (
  inputs: Leadsassignmentslalabel3Inputs
) => LocalizedString;
export declare const leadsassignmentstrategyfixed3: (
  inputs: Leadsassignmentstrategyfixed3Inputs
) => LocalizedString;
export declare const leadsassignmentstrategylabel3: (
  inputs: Leadsassignmentstrategylabel3Inputs
) => LocalizedString;
export declare const leadsassignmentstrategyleastactive4: (
  inputs: Leadsassignmentstrategyleastactive4Inputs
) => LocalizedString;
export declare const leadsassignmentstrategyroundrobin4: (
  inputs: Leadsassignmentstrategyroundrobin4Inputs
) => LocalizedString;
export declare const leadsbulkassignerrortoast4: (
  inputs: Leadsbulkassignerrortoast4Inputs
) => LocalizedString;
export declare const leadsbulkassignto3: (
  inputs: Leadsbulkassignto3Inputs
) => LocalizedString;
export declare const leadsbulkchangestage3: (
  inputs: Leadsbulkchangestage3Inputs
) => LocalizedString;
export declare const leadsbulkclearselection3: (
  inputs: Leadsbulkclearselection3Inputs
) => LocalizedString;
export declare const leadsbulkdelete2: (
  inputs: Leadsbulkdelete2Inputs
) => LocalizedString;
export declare const leadsbulkdeleteconfirm3: (
  inputs: Leadsbulkdeleteconfirm3Inputs
) => LocalizedString;
export declare const leadsbulkdeleteconfirmdescription4: (
  inputs: Leadsbulkdeleteconfirmdescription4Inputs
) => LocalizedString;
export declare const leadsbulkdeleteconfirmtitle4: (
  inputs: Leadsbulkdeleteconfirmtitle4Inputs
) => LocalizedString;
export declare const leadsbulkdeleteerrortoast4: (
  inputs: Leadsbulkdeleteerrortoast4Inputs
) => LocalizedString;
export declare const leadsbulkexport2: (
  inputs: Leadsbulkexport2Inputs
) => LocalizedString;
export declare const leadsbulkselectedcount3: (
  inputs: Leadsbulkselectedcount3Inputs
) => LocalizedString;
export declare const leadsbulkstageerrortoast4: (
  inputs: Leadsbulkstageerrortoast4Inputs
) => LocalizedString;
export declare const leadscampaignlabel2: (
  inputs: Leadscampaignlabel2Inputs
) => LocalizedString;
export declare const leadscolumnage2: (
  inputs: Leadscolumnage2Inputs
) => LocalizedString;
export declare const leadscolumnassignee2: (
  inputs: Leadscolumnassignee2Inputs
) => LocalizedString;
export declare const leadscolumncreatedat3: (
  inputs: Leadscolumncreatedat3Inputs
) => LocalizedString;
export declare const leadscolumnname2: (
  inputs: Leadscolumnname2Inputs
) => LocalizedString;
export declare const leadscolumnphone2: (
  inputs: Leadscolumnphone2Inputs
) => LocalizedString;
export declare const leadscolumnstage2: (
  inputs: Leadscolumnstage2Inputs
) => LocalizedString;
export declare const leadsdescription1: (
  inputs: Leadsdescription1Inputs
) => LocalizedString;
export declare const leadsdsraddbutton3: (
  inputs: Leadsdsraddbutton3Inputs
) => LocalizedString;
export declare const leadsdsrcancel2: (
  inputs: Leadsdsrcancel2Inputs
) => LocalizedString;
export declare const leadsdsrcompleteaction3: (
  inputs: Leadsdsrcompleteaction3Inputs
) => LocalizedString;
export declare const leadsdsrcompleteerrortoast4: (
  inputs: Leadsdsrcompleteerrortoast4Inputs
) => LocalizedString;
export declare const leadsdsrcreateerrortoast4: (
  inputs: Leadsdsrcreateerrortoast4Inputs
) => LocalizedString;
export declare const leadsdsrdialogtitle3: (
  inputs: Leadsdsrdialogtitle3Inputs
) => LocalizedString;
export declare const leadsdsrduelabel3: (
  inputs: Leadsdsrduelabel3Inputs
) => LocalizedString;
export declare const leadsdsrempty2: (
  inputs: Leadsdsrempty2Inputs
) => LocalizedString;
export declare const leadsdsrindicatorlabel3: (
  inputs: Leadsdsrindicatorlabel3Inputs
) => LocalizedString;
export declare const leadsdsrnoteslabel3: (
  inputs: Leadsdsrnoteslabel3Inputs
) => LocalizedString;
export declare const leadsdsrnotesplaceholder3: (
  inputs: Leadsdsrnotesplaceholder3Inputs
) => LocalizedString;
export declare const leadsdsroverduebadge3: (
  inputs: Leadsdsroverduebadge3Inputs
) => LocalizedString;
export declare const leadsdsrreceivedatlabel4: (
  inputs: Leadsdsrreceivedatlabel4Inputs
) => LocalizedString;
export declare const leadsdsrsectionlabel3: (
  inputs: Leadsdsrsectionlabel3Inputs
) => LocalizedString;
export declare const leadsdsrstatuscompleted3: (
  inputs: Leadsdsrstatuscompleted3Inputs
) => LocalizedString;
export declare const leadsdsrstatuspending3: (
  inputs: Leadsdsrstatuspending3Inputs
) => LocalizedString;
export declare const leadsdsrsubmit2: (
  inputs: Leadsdsrsubmit2Inputs
) => LocalizedString;
export declare const leadsdsrtypedelete3: (
  inputs: Leadsdsrtypedelete3Inputs
) => LocalizedString;
export declare const leadsdsrtypeexport3: (
  inputs: Leadsdsrtypeexport3Inputs
) => LocalizedString;
export declare const leadsdsrtypelabel3: (
  inputs: Leadsdsrtypelabel3Inputs
) => LocalizedString;
export declare const leadsemptytitle2: (
  inputs: Leadsemptytitle2Inputs
) => LocalizedString;
export declare const leadsexportbutton2: (
  inputs: Leadsexportbutton2Inputs
) => LocalizedString;
export declare const leadsexporterrortoast3: (
  inputs: Leadsexporterrortoast3Inputs
) => LocalizedString;
export declare const leadsfilterassigneelabel3: (
  inputs: Leadsfilterassigneelabel3Inputs
) => LocalizedString;
export declare const leadsfiltercampaignlabel3: (
  inputs: Leadsfiltercampaignlabel3Inputs
) => LocalizedString;
export declare const leadsfilterdatefromlabel4: (
  inputs: Leadsfilterdatefromlabel4Inputs
) => LocalizedString;
export declare const leadsfilterdatetolabel4: (
  inputs: Leadsfilterdatetolabel4Inputs
) => LocalizedString;
export declare const leadsfilterpaidlabel3: (
  inputs: Leadsfilterpaidlabel3Inputs
) => LocalizedString;
export declare const leadsfilterpaidno3: (
  inputs: Leadsfilterpaidno3Inputs
) => LocalizedString;
export declare const leadsfilterpaidyes3: (
  inputs: Leadsfilterpaidyes3Inputs
) => LocalizedString;
export declare const leadsfilterproductlabel3: (
  inputs: Leadsfilterproductlabel3Inputs
) => LocalizedString;
export declare const leadsfilterrepeatcustomerlabel4: (
  inputs: Leadsfilterrepeatcustomerlabel4Inputs
) => LocalizedString;
export declare const leadsfiltersearchlabel3: (
  inputs: Leadsfiltersearchlabel3Inputs
) => LocalizedString;
export declare const leadsfiltersearchplaceholder3: (
  inputs: Leadsfiltersearchplaceholder3Inputs
) => LocalizedString;
export declare const leadsfilterutmsourcelabel4: (
  inputs: Leadsfilterutmsourcelabel4Inputs
) => LocalizedString;
export declare const leadsfilterutmsourceplaceholder4: (
  inputs: Leadsfilterutmsourceplaceholder4Inputs
) => LocalizedString;
export declare const leadsimportbutton2: (
  inputs: Leadsimportbutton2Inputs
) => LocalizedString;
export declare const leadsimportdialogdescription3: (
  inputs: Leadsimportdialogdescription3Inputs
) => LocalizedString;
export declare const leadsimportdialogtitle3: (
  inputs: Leadsimportdialogtitle3Inputs
) => LocalizedString;
export declare const leadsimporterrortoast3: (
  inputs: Leadsimporterrortoast3Inputs
) => LocalizedString;
export declare const leadsimportmappingcustomfields4: (
  inputs: Leadsimportmappingcustomfields4Inputs
) => LocalizedString;
export declare const leadsimportmappingemail3: (
  inputs: Leadsimportmappingemail3Inputs
) => LocalizedString;
export declare const leadsimportmappingfullname4: (
  inputs: Leadsimportmappingfullname4Inputs
) => LocalizedString;
export declare const leadsimportmappingpersona3: (
  inputs: Leadsimportmappingpersona3Inputs
) => LocalizedString;
export declare const leadsimportmappingphone3: (
  inputs: Leadsimportmappingphone3Inputs
) => LocalizedString;
export declare const leadsimportmappingskip3: (
  inputs: Leadsimportmappingskip3Inputs
) => LocalizedString;
export declare const leadsimportnorowserror4: (
  inputs: Leadsimportnorowserror4Inputs
) => LocalizedString;
export declare const leadsimportpreviewtitle3: (
  inputs: Leadsimportpreviewtitle3Inputs
) => LocalizedString;
export declare const leadsimportskippedrowstoast4: (
  inputs: Leadsimportskippedrowstoast4Inputs
) => LocalizedString;
export declare const leadsimportsubmit2: (
  inputs: Leadsimportsubmit2Inputs
) => LocalizedString;
export declare const leadsimportsuccesstoast3: (
  inputs: Leadsimportsuccesstoast3Inputs
) => LocalizedString;
export declare const leadsimportuploadlabel3: (
  inputs: Leadsimportuploadlabel3Inputs
) => LocalizedString;
export declare const leadskanbanemptycolumn3: (
  inputs: Leadskanbanemptycolumn3Inputs
) => LocalizedString;
export declare const leadsloaderrortitle3: (
  inputs: Leadsloaderrortitle3Inputs
) => LocalizedString;
export declare const leadsnoteerrortoast3: (
  inputs: Leadsnoteerrortoast3Inputs
) => LocalizedString;
export declare const leadsnotificationsbelllabel3: (
  inputs: Leadsnotificationsbelllabel3Inputs
) => LocalizedString;
export declare const leadsnotifychannelemailoption4: (
  inputs: Leadsnotifychannelemailoption4Inputs
) => LocalizedString;
export declare const leadsnotifychannellabel3: (
  inputs: Leadsnotifychannellabel3Inputs
) => LocalizedString;
export declare const leadsnotifychannelsmsoption4: (
  inputs: Leadsnotifychannelsmsoption4Inputs
) => LocalizedString;
export declare const leadsnotifychannelznsoption4: (
  inputs: Leadsnotifychannelznsoption4Inputs
) => LocalizedString;
export declare const leadsnotifyesmsapikeylabel5: (
  inputs: Leadsnotifyesmsapikeylabel5Inputs
) => LocalizedString;
export declare const leadsnotifyesmsapikeyplaceholder5: (
  inputs: Leadsnotifyesmsapikeyplaceholder5Inputs
) => LocalizedString;
export declare const leadsnotifyesmsbrandnamelabel4: (
  inputs: Leadsnotifyesmsbrandnamelabel4Inputs
) => LocalizedString;
export declare const leadsnotifyesmsbrandnameplaceholder4: (
  inputs: Leadsnotifyesmsbrandnameplaceholder4Inputs
) => LocalizedString;
export declare const leadsnotifyesmsdescription3: (
  inputs: Leadsnotifyesmsdescription3Inputs
) => LocalizedString;
export declare const leadsnotifyesmssecretkeylabel5: (
  inputs: Leadsnotifyesmssecretkeylabel5Inputs
) => LocalizedString;
export declare const leadsnotifyesmssecretkeyplaceholder5: (
  inputs: Leadsnotifyesmssecretkeyplaceholder5Inputs
) => LocalizedString;
export declare const leadsnotifyesmstitle3: (
  inputs: Leadsnotifyesmstitle3Inputs
) => LocalizedString;
export declare const leadsnotifyphonelabel3: (
  inputs: Leadsnotifyphonelabel3Inputs
) => LocalizedString;
export declare const leadsnotifyphoneplaceholder3: (
  inputs: Leadsnotifyphoneplaceholder3Inputs
) => LocalizedString;
export declare const leadsnotifysettingsdescription3: (
  inputs: Leadsnotifysettingsdescription3Inputs
) => LocalizedString;
export declare const leadsnotifysettingsmanagebutton4: (
  inputs: Leadsnotifysettingsmanagebutton4Inputs
) => LocalizedString;
export declare const leadsnotifysettingstitle3: (
  inputs: Leadsnotifysettingstitle3Inputs
) => LocalizedString;
export declare const leadsnotifyznsaccesstokenlabel5: (
  inputs: Leadsnotifyznsaccesstokenlabel5Inputs
) => LocalizedString;
export declare const leadsnotifyznsaccesstokenplaceholder5: (
  inputs: Leadsnotifyznsaccesstokenplaceholder5Inputs
) => LocalizedString;
export declare const leadsnotifyznsdescription3: (
  inputs: Leadsnotifyznsdescription3Inputs
) => LocalizedString;
export declare const leadsnotifyznstemplateidlabel5: (
  inputs: Leadsnotifyznstemplateidlabel5Inputs
) => LocalizedString;
export declare const leadsnotifyznstemplateidplaceholder5: (
  inputs: Leadsnotifyznstemplateidplaceholder5Inputs
) => LocalizedString;
export declare const leadsnotifyznstitle3: (
  inputs: Leadsnotifyznstitle3Inputs
) => LocalizedString;
export declare const leadsorderawaitingconfirmation3: (
  inputs: Leadsorderawaitingconfirmation3Inputs
) => LocalizedString;
export declare const leadsordercancel2: (
  inputs: Leadsordercancel2Inputs
) => LocalizedString;
export declare const leadsorderconfirmpayment3: (
  inputs: Leadsorderconfirmpayment3Inputs
) => LocalizedString;
export declare const leadsordercountlabel3: (
  inputs: Leadsordercountlabel3Inputs
) => LocalizedString;
export declare const leadsordererrortoast3: (
  inputs: Leadsordererrortoast3Inputs
) => LocalizedString;
export declare const leadsorderfulfill2: (
  inputs: Leadsorderfulfill2Inputs
) => LocalizedString;
export declare const leadsorderreasoncancel3: (
  inputs: Leadsorderreasoncancel3Inputs
) => LocalizedString;
export declare const leadsorderreasonconfirm3: (
  inputs: Leadsorderreasonconfirm3Inputs
) => LocalizedString;
export declare const leadsorderreasonplaceholder3: (
  inputs: Leadsorderreasonplaceholder3Inputs
) => LocalizedString;
export declare const leadsorderreasontitle3: (
  inputs: Leadsorderreasontitle3Inputs
) => LocalizedString;
export declare const leadsorderslabel2: (
  inputs: Leadsorderslabel2Inputs
) => LocalizedString;
export declare const leadsrepeatcustomerbadge3: (
  inputs: Leadsrepeatcustomerbadge3Inputs
) => LocalizedString;
export declare const leadssavedviewdeletelabel4: (
  inputs: Leadssavedviewdeletelabel4Inputs
) => LocalizedString;
export declare const leadssavedviewnamelabel4: (
  inputs: Leadssavedviewnamelabel4Inputs
) => LocalizedString;
export declare const leadssavedviewnameplaceholder4: (
  inputs: Leadssavedviewnameplaceholder4Inputs
) => LocalizedString;
export declare const leadssavedviewsavebutton4: (
  inputs: Leadssavedviewsavebutton4Inputs
) => LocalizedString;
export declare const leadssavedviewsaveconfirm4: (
  inputs: Leadssavedviewsaveconfirm4Inputs
) => LocalizedString;
export declare const leadssavedviewsaveerrortoast5: (
  inputs: Leadssavedviewsaveerrortoast5Inputs
) => LocalizedString;
export declare const leadssavedviewsharelabel4: (
  inputs: Leadssavedviewsharelabel4Inputs
) => LocalizedString;
export declare const leadsstagechangeerrortoast4: (
  inputs: Leadsstagechangeerrortoast4Inputs
) => LocalizedString;
export declare const leadsstagefilteractive3: (
  inputs: Leadsstagefilteractive3Inputs
) => LocalizedString;
export declare const leadsstagefilterclear3: (
  inputs: Leadsstagefilterclear3Inputs
) => LocalizedString;
export declare const leadssubnavassignmentrules4: (
  inputs: Leadssubnavassignmentrules4Inputs
) => LocalizedString;
export declare const leadssubnavnotifications3: (
  inputs: Leadssubnavnotifications3Inputs
) => LocalizedString;
export declare const leadssubnavoverview3: (
  inputs: Leadssubnavoverview3Inputs
) => LocalizedString;
export declare const leadssubnavwebhooks3: (
  inputs: Leadssubnavwebhooks3Inputs
) => LocalizedString;
export declare const leadstimelinelabel2: (
  inputs: Leadstimelinelabel2Inputs
) => LocalizedString;
export declare const leadstitle1: (
  inputs: Leadstitle1Inputs
) => LocalizedString;
export declare const leadstotalpaidlabel3: (
  inputs: Leadstotalpaidlabel3Inputs
) => LocalizedString;
export declare const leadsunassigned1: (
  inputs: Leadsunassigned1Inputs
) => LocalizedString;
export declare const leadsunreadindicatorlabel3: (
  inputs: Leadsunreadindicatorlabel3Inputs
) => LocalizedString;
export declare const leadsviewkanban2: (
  inputs: Leadsviewkanban2Inputs
) => LocalizedString;
export declare const leadsviewlist2: (
  inputs: Leadsviewlist2Inputs
) => LocalizedString;
export declare const leadswebhookcampaignlabel3: (
  inputs: Leadswebhookcampaignlabel3Inputs
) => LocalizedString;
export declare const leadswebhookcampaignplaceholder3: (
  inputs: Leadswebhookcampaignplaceholder3Inputs
) => LocalizedString;
export declare const leadswebhookcarddescription3: (
  inputs: Leadswebhookcarddescription3Inputs
) => LocalizedString;
export declare const leadswebhookconfiguredbadge3: (
  inputs: Leadswebhookconfiguredbadge3Inputs
) => LocalizedString;
export declare const leadswebhookgenericdescription3: (
  inputs: Leadswebhookgenericdescription3Inputs
) => LocalizedString;
export declare const leadswebhookgenericgeneratebutton4: (
  inputs: Leadswebhookgenericgeneratebutton4Inputs
) => LocalizedString;
export declare const leadswebhookgenerickeyrevealedlabel5: (
  inputs: Leadswebhookgenerickeyrevealedlabel5Inputs
) => LocalizedString;
export declare const leadswebhookgenerickeyrevealedwarning5: (
  inputs: Leadswebhookgenerickeyrevealedwarning5Inputs
) => LocalizedString;
export declare const leadswebhookgenericrevokebutton4: (
  inputs: Leadswebhookgenericrevokebutton4Inputs
) => LocalizedString;
export declare const leadswebhookgenericrotatebutton4: (
  inputs: Leadswebhookgenericrotatebutton4Inputs
) => LocalizedString;
export declare const leadswebhookgenerictitle3: (
  inputs: Leadswebhookgenerictitle3Inputs
) => LocalizedString;
export declare const leadswebhookgoogleadsdescription4: (
  inputs: Leadswebhookgoogleadsdescription4Inputs
) => LocalizedString;
export declare const leadswebhookgoogleadstitle4: (
  inputs: Leadswebhookgoogleadstitle4Inputs
) => LocalizedString;
export declare const leadswebhooklastusedlabel4: (
  inputs: Leadswebhooklastusedlabel4Inputs
) => LocalizedString;
export declare const leadswebhookmissingbadge3: (
  inputs: Leadswebhookmissingbadge3Inputs
) => LocalizedString;
export declare const leadswebhookneverusedlabel4: (
  inputs: Leadswebhookneverusedlabel4Inputs
) => LocalizedString;
export declare const leadswebhooknocampaignwarning4: (
  inputs: Leadswebhooknocampaignwarning4Inputs
) => LocalizedString;
export declare const leadswebhooknocampaignsempty4: (
  inputs: Leadswebhooknocampaignsempty4Inputs
) => LocalizedString;
export declare const leadswebhookpageaccesstokendescription5: (
  inputs: Leadswebhookpageaccesstokendescription5Inputs
) => LocalizedString;
export declare const leadswebhookpageaccesstokenlabel5: (
  inputs: Leadswebhookpageaccesstokenlabel5Inputs
) => LocalizedString;
export declare const leadswebhookpageaccesstokenplaceholder5: (
  inputs: Leadswebhookpageaccesstokenplaceholder5Inputs
) => LocalizedString;
export declare const leadswebhookremovebutton3: (
  inputs: Leadswebhookremovebutton3Inputs
) => LocalizedString;
export declare const leadswebhookremovedtoast3: (
  inputs: Leadswebhookremovedtoast3Inputs
) => LocalizedString;
export declare const leadswebhooksaveerrortoast4: (
  inputs: Leadswebhooksaveerrortoast4Inputs
) => LocalizedString;
export declare const leadswebhooksavedtoast3: (
  inputs: Leadswebhooksavedtoast3Inputs
) => LocalizedString;
export declare const leadswebhooksecretlabel3: (
  inputs: Leadswebhooksecretlabel3Inputs
) => LocalizedString;
export declare const leadswebhooksecretplaceholder3: (
  inputs: Leadswebhooksecretplaceholder3Inputs
) => LocalizedString;
export declare const leadswebhooksecretrotateplaceholder4: (
  inputs: Leadswebhooksecretrotateplaceholder4Inputs
) => LocalizedString;
export declare const leadswebhooksettingsdescription3: (
  inputs: Leadswebhooksettingsdescription3Inputs
) => LocalizedString;
export declare const leadswebhooksettingsmanagebutton4: (
  inputs: Leadswebhooksettingsmanagebutton4Inputs
) => LocalizedString;
export declare const leadswebhooksettingstitle3: (
  inputs: Leadswebhooksettingstitle3Inputs
) => LocalizedString;
export declare const leadswebhooksharedsecretbadge4: (
  inputs: Leadswebhooksharedsecretbadge4Inputs
) => LocalizedString;
export declare const leadswebhooktiktokadvertiserlabel4: (
  inputs: Leadswebhooktiktokadvertiserlabel4Inputs
) => LocalizedString;
export declare const leadswebhooktiktokconnectbutton4: (
  inputs: Leadswebhooktiktokconnectbutton4Inputs
) => LocalizedString;
export declare const leadswebhooktiktokdescription3: (
  inputs: Leadswebhooktiktokdescription3Inputs
) => LocalizedString;
export declare const leadswebhooktiktokreconnectbutton4: (
  inputs: Leadswebhooktiktokreconnectbutton4Inputs
) => LocalizedString;
export declare const leadswebhooktiktoktitle3: (
  inputs: Leadswebhooktiktoktitle3Inputs
) => LocalizedString;
export declare const leadswebhookurllabel3: (
  inputs: Leadswebhookurllabel3Inputs
) => LocalizedString;
export declare const leadswebhookverifytokenlabel4: (
  inputs: Leadswebhookverifytokenlabel4Inputs
) => LocalizedString;
export declare const leadswebhookverifytokenplaceholder4: (
  inputs: Leadswebhookverifytokenplaceholder4Inputs
) => LocalizedString;
export declare const loginemaillabel2: (
  inputs: Loginemaillabel2Inputs
) => LocalizedString;
export declare const loginerror1: (
  inputs: Loginerror1Inputs
) => LocalizedString;
export declare const loginforgotpassword2: (
  inputs: Loginforgotpassword2Inputs
) => LocalizedString;
export declare const loginnoaccount2: (
  inputs: Loginnoaccount2Inputs
) => LocalizedString;
export declare const loginpasswordlabel2: (
  inputs: Loginpasswordlabel2Inputs
) => LocalizedString;
export declare const loginsignuplink2: (
  inputs: Loginsignuplink2Inputs
) => LocalizedString;
export declare const loginsubmit1: (
  inputs: Loginsubmit1Inputs
) => LocalizedString;
export declare const logintitle1: (
  inputs: Logintitle1Inputs
) => LocalizedString;
export declare const memberscancelinviteconfirmaction4: (
  inputs: Memberscancelinviteconfirmaction4Inputs
) => LocalizedString;
export declare const memberscancelinviteconfirmbody4: (
  inputs: Memberscancelinviteconfirmbody4Inputs
) => LocalizedString;
export declare const memberscancelinviteconfirmtitle4: (
  inputs: Memberscancelinviteconfirmtitle4Inputs
) => LocalizedString;
export declare const memberscancelinvitesuccesstoast4: (
  inputs: Memberscancelinvitesuccesstoast4Inputs
) => LocalizedString;
export declare const memberscolumnactions2: (
  inputs: Memberscolumnactions2Inputs
) => LocalizedString;
export declare const memberscolumnemail2: (
  inputs: Memberscolumnemail2Inputs
) => LocalizedString;
export declare const memberscolumnexpires2: (
  inputs: Memberscolumnexpires2Inputs
) => LocalizedString;
export declare const memberscolumnmember2: (
  inputs: Memberscolumnmember2Inputs
) => LocalizedString;
export declare const memberscolumnrole2: (
  inputs: Memberscolumnrole2Inputs
) => LocalizedString;
export declare const memberscolumnseeallleads4: (
  inputs: Memberscolumnseeallleads4Inputs
) => LocalizedString;
export declare const membersdescription1: (
  inputs: Membersdescription1Inputs
) => LocalizedString;
export declare const membersemaillabel2: (
  inputs: Membersemaillabel2Inputs
) => LocalizedString;
export declare const membersinvitationaccept2: (
  inputs: Membersinvitationaccept2Inputs
) => LocalizedString;
export declare const membersinvitationbannertext3: (
  inputs: Membersinvitationbannertext3Inputs
) => LocalizedString;
export declare const membersinvitationexpiredbadge3: (
  inputs: Membersinvitationexpiredbadge3Inputs
) => LocalizedString;
export declare const membersinvitationreject2: (
  inputs: Membersinvitationreject2Inputs
) => LocalizedString;
export declare const membersinvitationresponderrortoast4: (
  inputs: Membersinvitationresponderrortoast4Inputs
) => LocalizedString;
export declare const membersinvitebutton2: (
  inputs: Membersinvitebutton2Inputs
) => LocalizedString;
export declare const membersinvitedialogdescription3: (
  inputs: Membersinvitedialogdescription3Inputs
) => LocalizedString;
export declare const membersinvitedialogtitle3: (
  inputs: Membersinvitedialogtitle3Inputs
) => LocalizedString;
export declare const membersinvitesubmit2: (
  inputs: Membersinvitesubmit2Inputs
) => LocalizedString;
export declare const membersinvitesuccesstoast3: (
  inputs: Membersinvitesuccesstoast3Inputs
) => LocalizedString;
export declare const membersloaderrortitle3: (
  inputs: Membersloaderrortitle3Inputs
) => LocalizedString;
export declare const memberspendingdescription2: (
  inputs: Memberspendingdescription2Inputs
) => LocalizedString;
export declare const memberspendingemptytitle3: (
  inputs: Memberspendingemptytitle3Inputs
) => LocalizedString;
export declare const memberspendingtitle2: (
  inputs: Memberspendingtitle2Inputs
) => LocalizedString;
export declare const membersremoveconfirmaction3: (
  inputs: Membersremoveconfirmaction3Inputs
) => LocalizedString;
export declare const membersremoveconfirmbody3: (
  inputs: Membersremoveconfirmbody3Inputs
) => LocalizedString;
export declare const membersremoveconfirmtitle3: (
  inputs: Membersremoveconfirmtitle3Inputs
) => LocalizedString;
export declare const membersremovesuccesstoast3: (
  inputs: Membersremovesuccesstoast3Inputs
) => LocalizedString;
export declare const membersrolelabel2: (
  inputs: Membersrolelabel2Inputs
) => LocalizedString;
export declare const membersseeallleadserrortoast5: (
  inputs: Membersseeallleadserrortoast5Inputs
) => LocalizedString;
export declare const memberstitle1: (
  inputs: Memberstitle1Inputs
) => LocalizedString;
export declare const offerworkspaceback2: (
  inputs: Offerworkspaceback2Inputs
) => LocalizedString;
export declare const offerworkspacecampaign2: (
  inputs: Offerworkspacecampaign2Inputs
) => LocalizedString;
export declare const offerworkspacecontentmissing3: (
  inputs: Offerworkspacecontentmissing3Inputs
) => LocalizedString;
export declare const offerworkspacecontentready3: (
  inputs: Offerworkspacecontentready3Inputs
) => LocalizedString;
export declare const offerworkspacecontenttitle3: (
  inputs: Offerworkspacecontenttitle3Inputs
) => LocalizedString;
export declare const offerworkspacedescription2: (
  inputs: Offerworkspacedescription2Inputs
) => LocalizedString;
export declare const offerworkspacedraft2: (
  inputs: Offerworkspacedraft2Inputs
) => LocalizedString;
export declare const offerworkspaceeditoraction3: (
  inputs: Offerworkspaceeditoraction3Inputs
) => LocalizedString;
export declare const offerworkspaceemptytitle3: (
  inputs: Offerworkspaceemptytitle3Inputs
) => LocalizedString;
export declare const offerworkspaceeyebrow2: (
  inputs: Offerworkspaceeyebrow2Inputs
) => LocalizedString;
export declare const offerworkspaceinboxaction3: (
  inputs: Offerworkspaceinboxaction3Inputs
) => LocalizedString;
export declare const offerworkspaceloaderrortitle4: (
  inputs: Offerworkspaceloaderrortitle4Inputs
) => LocalizedString;
export declare const offerworkspacenexttitle3: (
  inputs: Offerworkspacenexttitle3Inputs
) => LocalizedString;
export declare const offerworkspacenocampaign3: (
  inputs: Offerworkspacenocampaign3Inputs
) => LocalizedString;
export declare const offerworkspaceready2: (
  inputs: Offerworkspaceready2Inputs
) => LocalizedString;
export declare const offerworkspacestepcontent3: (
  inputs: Offerworkspacestepcontent3Inputs
) => LocalizedString;
export declare const offerworkspacestepleadform4: (
  inputs: Offerworkspacestepleadform4Inputs
) => LocalizedString;
export declare const offerworkspacesteppublish3: (
  inputs: Offerworkspacesteppublish3Inputs
) => LocalizedString;
export declare const offerworkspaceversion2: (
  inputs: Offerworkspaceversion2Inputs
) => LocalizedString;
export declare const offerssurfacenotice2: (
  inputs: Offerssurfacenotice2Inputs
) => LocalizedString;
export declare const onboardingbody1: (
  inputs: Onboardingbody1Inputs
) => LocalizedString;
export declare const onboardingchecklistdismiss2: (
  inputs: Onboardingchecklistdismiss2Inputs
) => LocalizedString;
export declare const onboardingchecklistsubtitle2: (
  inputs: Onboardingchecklistsubtitle2Inputs
) => LocalizedString;
export declare const onboardingchecklisttitle2: (
  inputs: Onboardingchecklisttitle2Inputs
) => LocalizedString;
export declare const onboardingcreateerror2: (
  inputs: Onboardingcreateerror2Inputs
) => LocalizedString;
export declare const onboardingdeliverylabel2: (
  inputs: Onboardingdeliverylabel2Inputs
) => LocalizedString;
export declare const onboardingdeliverylink2: (
  inputs: Onboardingdeliverylink2Inputs
) => LocalizedString;
export declare const onboardingdeliverymanual2: (
  inputs: Onboardingdeliverymanual2Inputs
) => LocalizedString;
export declare const onboardingdeliveryschedule2: (
  inputs: Onboardingdeliveryschedule2Inputs
) => LocalizedString;
export declare const onboardingdeliveryzalo2: (
  inputs: Onboardingdeliveryzalo2Inputs
) => LocalizedString;
export declare const onboardingdescriptionlabel2: (
  inputs: Onboardingdescriptionlabel2Inputs
) => LocalizedString;
export declare const onboardingdescriptionplaceholder2: (
  inputs: Onboardingdescriptionplaceholder2Inputs
) => LocalizedString;
export declare const onboardingorgnamehelp3: (
  inputs: Onboardingorgnamehelp3Inputs
) => LocalizedString;
export declare const onboardingorgnamelabel3: (
  inputs: Onboardingorgnamelabel3Inputs
) => LocalizedString;
export declare const onboardingorgnameplaceholder3: (
  inputs: Onboardingorgnameplaceholder3Inputs
) => LocalizedString;
export declare const onboardingpricehelp2: (
  inputs: Onboardingpricehelp2Inputs
) => LocalizedString;
export declare const onboardingpricelabel2: (
  inputs: Onboardingpricelabel2Inputs
) => LocalizedString;
export declare const onboardingprivacynote2: (
  inputs: Onboardingprivacynote2Inputs
) => LocalizedString;
export declare const onboardingproductnamelabel3: (
  inputs: Onboardingproductnamelabel3Inputs
) => LocalizedString;
export declare const onboardingproductnameplaceholder3: (
  inputs: Onboardingproductnameplaceholder3Inputs
) => LocalizedString;
export declare const onboardingproducttypelabel3: (
  inputs: Onboardingproducttypelabel3Inputs
) => LocalizedString;
export declare const onboardingsetupdescription2: (
  inputs: Onboardingsetupdescription2Inputs
) => LocalizedString;
export declare const onboardingsetuptitle2: (
  inputs: Onboardingsetuptitle2Inputs
) => LocalizedString;
export declare const onboardingstepconnectai3: (
  inputs: Onboardingstepconnectai3Inputs
) => LocalizedString;
export declare const onboardingstepconnectpayment3: (
  inputs: Onboardingstepconnectpayment3Inputs
) => LocalizedString;
export declare const onboardingstepcreatelanding3: (
  inputs: Onboardingstepcreatelanding3Inputs
) => LocalizedString;
export declare const onboardingstepcreateproduct3: (
  inputs: Onboardingstepcreateproduct3Inputs
) => LocalizedString;
export declare const onboardingsteppublishlanding3: (
  inputs: Onboardingsteppublishlanding3Inputs
) => LocalizedString;
export declare const onboardingsubmit1: (
  inputs: Onboardingsubmit1Inputs
) => LocalizedString;
export declare const onboardingtitle1: (
  inputs: Onboardingtitle1Inputs
) => LocalizedString;
export declare const orgswitchercreateorg3: (
  inputs: Orgswitchercreateorg3Inputs
) => LocalizedString;
export declare const orgswitchernoorg3: (
  inputs: Orgswitchernoorg3Inputs
) => LocalizedString;
export declare const orgswitcherswitcherror3: (
  inputs: Orgswitcherswitcherror3Inputs
) => LocalizedString;
export declare const paymentaccountnamelabel3: (
  inputs: Paymentaccountnamelabel3Inputs
) => LocalizedString;
export declare const paymentaccountnumberlabel3: (
  inputs: Paymentaccountnumberlabel3Inputs
) => LocalizedString;
export declare const paymentapikeylabel3: (
  inputs: Paymentapikeylabel3Inputs
) => LocalizedString;
export declare const paymentbankbinlabel3: (
  inputs: Paymentbankbinlabel3Inputs
) => LocalizedString;
export declare const paymentconnectbutton2: (
  inputs: Paymentconnectbutton2Inputs
) => LocalizedString;
export declare const paymentconnectdialogdescription3: (
  inputs: Paymentconnectdialogdescription3Inputs
) => LocalizedString;
export declare const paymentconnectdialogtitle3: (
  inputs: Paymentconnectdialogtitle3Inputs
) => LocalizedString;
export declare const paymentconnecterrortoast3: (
  inputs: Paymentconnecterrortoast3Inputs
) => LocalizedString;
export declare const paymentconnectsubmit2: (
  inputs: Paymentconnectsubmit2Inputs
) => LocalizedString;
export declare const paymentconnectioncolumnaccount3: (
  inputs: Paymentconnectioncolumnaccount3Inputs
) => LocalizedString;
export declare const paymentconnectioncolumnactions3: (
  inputs: Paymentconnectioncolumnactions3Inputs
) => LocalizedString;
export declare const paymentconnectioncolumnbank3: (
  inputs: Paymentconnectioncolumnbank3Inputs
) => LocalizedString;
export declare const paymentconnectioncolumnstatus3: (
  inputs: Paymentconnectioncolumnstatus3Inputs
) => LocalizedString;
export declare const paymentconnectionsdescription2: (
  inputs: Paymentconnectionsdescription2Inputs
) => LocalizedString;
export declare const paymentconnectionsemptytitle3: (
  inputs: Paymentconnectionsemptytitle3Inputs
) => LocalizedString;
export declare const paymentconnectionsloaderrortitle4: (
  inputs: Paymentconnectionsloaderrortitle4Inputs
) => LocalizedString;
export declare const paymentconnectionstitle2: (
  inputs: Paymentconnectionstitle2Inputs
) => LocalizedString;
export declare const paymentguidedescription2: (
  inputs: Paymentguidedescription2Inputs
) => LocalizedString;
export declare const paymentguideloaderrortitle4: (
  inputs: Paymentguideloaderrortitle4Inputs
) => LocalizedString;
export declare const paymentguidetitle2: (
  inputs: Paymentguidetitle2Inputs
) => LocalizedString;
export declare const paymentremoveconfirmbody3: (
  inputs: Paymentremoveconfirmbody3Inputs
) => LocalizedString;
export declare const paymentremoveconfirmtitle3: (
  inputs: Paymentremoveconfirmtitle3Inputs
) => LocalizedString;
export declare const paymentremoveconnectionaction3: (
  inputs: Paymentremoveconnectionaction3Inputs
) => LocalizedString;
export declare const paymentremoveconnectionerrortoast4: (
  inputs: Paymentremoveconnectionerrortoast4Inputs
) => LocalizedString;
export declare const platformactiondonetoast3: (
  inputs: Platformactiondonetoast3Inputs
) => LocalizedString;
export declare const platformactionerrortoast3: (
  inputs: Platformactionerrortoast3Inputs
) => LocalizedString;
export declare const platformauditcolumnaction3: (
  inputs: Platformauditcolumnaction3Inputs
) => LocalizedString;
export declare const platformauditcolumnmeta3: (
  inputs: Platformauditcolumnmeta3Inputs
) => LocalizedString;
export declare const platformauditcolumnstaff3: (
  inputs: Platformauditcolumnstaff3Inputs
) => LocalizedString;
export declare const platformauditcolumntime3: (
  inputs: Platformauditcolumntime3Inputs
) => LocalizedString;
export declare const platformauditdescription2: (
  inputs: Platformauditdescription2Inputs
) => LocalizedString;
export declare const platformauditempty2: (
  inputs: Platformauditempty2Inputs
) => LocalizedString;
export declare const platformbacktoorgs3: (
  inputs: Platformbacktoorgs3Inputs
) => LocalizedString;
export declare const platformcancel1: (
  inputs: Platformcancel1Inputs
) => LocalizedString;
export declare const platformconfirm1: (
  inputs: Platformconfirm1Inputs
) => LocalizedString;
export declare const platformdisableorgaction3: (
  inputs: Platformdisableorgaction3Inputs
) => LocalizedString;
export declare const platformdisableorgdescription3: (
  inputs: Platformdisableorgdescription3Inputs
) => LocalizedString;
export declare const platformenableorgaction3: (
  inputs: Platformenableorgaction3Inputs
) => LocalizedString;
export declare const platformenableorgdescription3: (
  inputs: Platformenableorgdescription3Inputs
) => LocalizedString;
export declare const platformfeatureforceoff3: (
  inputs: Platformfeatureforceoff3Inputs
) => LocalizedString;
export declare const platformfeatureforceon3: (
  inputs: Platformfeatureforceon3Inputs
) => LocalizedString;
export declare const platformfeatureplanexcluded3: (
  inputs: Platformfeatureplanexcluded3Inputs
) => LocalizedString;
export declare const platformfeatureplanincluded3: (
  inputs: Platformfeatureplanincluded3Inputs
) => LocalizedString;
export declare const platformfeaturesempty2: (
  inputs: Platformfeaturesempty2Inputs
) => LocalizedString;
export declare const platformfeatureshint2: (
  inputs: Platformfeatureshint2Inputs
) => LocalizedString;
export declare const platformfeatureslabel2: (
  inputs: Platformfeatureslabel2Inputs
) => LocalizedString;
export declare const platformorgdetailloaderrortitle5: (
  inputs: Platformorgdetailloaderrortitle5Inputs
) => LocalizedString;
export declare const platformorgscolumnaicredit4: (
  inputs: Platformorgscolumnaicredit4Inputs
) => LocalizedString;
export declare const platformorgscolumncreated3: (
  inputs: Platformorgscolumncreated3Inputs
) => LocalizedString;
export declare const platformorgscolumnname3: (
  inputs: Platformorgscolumnname3Inputs
) => LocalizedString;
export declare const platformorgscolumnowner3: (
  inputs: Platformorgscolumnowner3Inputs
) => LocalizedString;
export declare const platformorgscolumnplan3: (
  inputs: Platformorgscolumnplan3Inputs
) => LocalizedString;
export declare const platformorgscolumnstatus3: (
  inputs: Platformorgscolumnstatus3Inputs
) => LocalizedString;
export declare const platformorgsdescription2: (
  inputs: Platformorgsdescription2Inputs
) => LocalizedString;
export declare const platformorgsemptytitle3: (
  inputs: Platformorgsemptytitle3Inputs
) => LocalizedString;
export declare const platformorgsloaderrortitle4: (
  inputs: Platformorgsloaderrortitle4Inputs
) => LocalizedString;
export declare const platformorgsloading2: (
  inputs: Platformorgsloading2Inputs
) => LocalizedString;
export declare const platformorgssearchplaceholder3: (
  inputs: Platformorgssearchplaceholder3Inputs
) => LocalizedString;
export declare const platformorgsstatusactive3: (
  inputs: Platformorgsstatusactive3Inputs
) => LocalizedString;
export declare const platformorgsstatusall3: (
  inputs: Platformorgsstatusall3Inputs
) => LocalizedString;
export declare const platformorgsstatusdisabled3: (
  inputs: Platformorgsstatusdisabled3Inputs
) => LocalizedString;
export declare const platformorgstitle2: (
  inputs: Platformorgstitle2Inputs
) => LocalizedString;
export declare const platformplanlabel2: (
  inputs: Platformplanlabel2Inputs
) => LocalizedString;
export declare const platformreasonlabel2: (
  inputs: Platformreasonlabel2Inputs
) => LocalizedString;
export declare const platformreasonplaceholder2: (
  inputs: Platformreasonplaceholder2Inputs
) => LocalizedString;
export declare const platformreasonsharedhint3: (
  inputs: Platformreasonsharedhint3Inputs
) => LocalizedString;
export declare const platformrefundassistaction3: (
  inputs: Platformrefundassistaction3Inputs
) => LocalizedString;
export declare const platformrefundassistdescription3: (
  inputs: Platformrefundassistdescription3Inputs
) => LocalizedString;
export declare const platformrefundassistdonetoast4: (
  inputs: Platformrefundassistdonetoast4Inputs
) => LocalizedString;
export declare const platformrefundassisttitle3: (
  inputs: Platformrefundassisttitle3Inputs
) => LocalizedString;
export declare const platformrefundorderidplaceholder4: (
  inputs: Platformrefundorderidplaceholder4Inputs
) => LocalizedString;
export declare const platformstaffaddaction3: (
  inputs: Platformstaffaddaction3Inputs
) => LocalizedString;
export declare const platformstaffaddemailplaceholder4: (
  inputs: Platformstaffaddemailplaceholder4Inputs
) => LocalizedString;
export declare const platformstaffadderrortoast4: (
  inputs: Platformstaffadderrortoast4Inputs
) => LocalizedString;
export declare const platformstaffaddsuccesstoast4: (
  inputs: Platformstaffaddsuccesstoast4Inputs
) => LocalizedString;
export declare const platformstaffcolumnactions3: (
  inputs: Platformstaffcolumnactions3Inputs
) => LocalizedString;
export declare const platformstaffcolumncreated3: (
  inputs: Platformstaffcolumncreated3Inputs
) => LocalizedString;
export declare const platformstaffcolumnemail3: (
  inputs: Platformstaffcolumnemail3Inputs
) => LocalizedString;
export declare const platformstaffcolumnrole3: (
  inputs: Platformstaffcolumnrole3Inputs
) => LocalizedString;
export declare const platformstaffdescription2: (
  inputs: Platformstaffdescription2Inputs
) => LocalizedString;
export declare const platformstaffemptytitle3: (
  inputs: Platformstaffemptytitle3Inputs
) => LocalizedString;
export declare const platformstaffloaderrortitle4: (
  inputs: Platformstaffloaderrortitle4Inputs
) => LocalizedString;
export declare const platformstaffloading2: (
  inputs: Platformstaffloading2Inputs
) => LocalizedString;
export declare const platformstaffremoveaction3: (
  inputs: Platformstaffremoveaction3Inputs
) => LocalizedString;
export declare const platformstaffremoveconfirmaction4: (
  inputs: Platformstaffremoveconfirmaction4Inputs
) => LocalizedString;
export declare const platformstaffremoveconfirmbody4: (
  inputs: Platformstaffremoveconfirmbody4Inputs
) => LocalizedString;
export declare const platformstaffremoveconfirmtitle4: (
  inputs: Platformstaffremoveconfirmtitle4Inputs
) => LocalizedString;
export declare const platformstaffremoveerrortoast4: (
  inputs: Platformstaffremoveerrortoast4Inputs
) => LocalizedString;
export declare const platformstaffremovesuccesstoast4: (
  inputs: Platformstaffremovesuccesstoast4Inputs
) => LocalizedString;
export declare const platformstataicreditbalance4: (
  inputs: Platformstataicreditbalance4Inputs
) => LocalizedString;
export declare const platformstataicreditspent4: (
  inputs: Platformstataicreditspent4Inputs
) => LocalizedString;
export declare const platformstatcampaigns2: (
  inputs: Platformstatcampaigns2Inputs
) => LocalizedString;
export declare const platformstatcreated2: (
  inputs: Platformstatcreated2Inputs
) => LocalizedString;
export declare const platformstatleads2: (
  inputs: Platformstatleads2Inputs
) => LocalizedString;
export declare const platformstatmembers2: (
  inputs: Platformstatmembers2Inputs
) => LocalizedString;
export declare const platformstattrialuses3: (
  inputs: Platformstattrialuses3Inputs
) => LocalizedString;
export declare const platformsubscriptiondescription2: (
  inputs: Platformsubscriptiondescription2Inputs
) => LocalizedString;
export declare const platformsubscriptionsave2: (
  inputs: Platformsubscriptionsave2Inputs
) => LocalizedString;
export declare const platformsubscriptiontitle2: (
  inputs: Platformsubscriptiontitle2Inputs
) => LocalizedString;
export declare const platformtabaudit2: (
  inputs: Platformtabaudit2Inputs
) => LocalizedString;
export declare const platformtabbilling2: (
  inputs: Platformtabbilling2Inputs
) => LocalizedString;
export declare const platformtaborgs2: (
  inputs: Platformtaborgs2Inputs
) => LocalizedString;
export declare const platformtaboverview2: (
  inputs: Platformtaboverview2Inputs
) => LocalizedString;
export declare const platformtabstaff2: (
  inputs: Platformtabstaff2Inputs
) => LocalizedString;
export declare const productsaddbutton2: (
  inputs: Productsaddbutton2Inputs
) => LocalizedString;
export declare const productsadddialogtitle3: (
  inputs: Productsadddialogtitle3Inputs
) => LocalizedString;
export declare const productscolumnactions2: (
  inputs: Productscolumnactions2Inputs
) => LocalizedString;
export declare const productscolumnname2: (
  inputs: Productscolumnname2Inputs
) => LocalizedString;
export declare const productscolumnprice2: (
  inputs: Productscolumnprice2Inputs
) => LocalizedString;
export declare const productscolumntype2: (
  inputs: Productscolumntype2Inputs
) => LocalizedString;
export declare const productscourseactivationlabel3: (
  inputs: Productscourseactivationlabel3Inputs
) => LocalizedString;
export declare const productscourseclassschedulelabel4: (
  inputs: Productscourseclassschedulelabel4Inputs
) => LocalizedString;
export declare const productscoursefieldstitle3: (
  inputs: Productscoursefieldstitle3Inputs
) => LocalizedString;
export declare const productscoursestartsatlabel4: (
  inputs: Productscoursestartsatlabel4Inputs
) => LocalizedString;
export declare const productscoursezalolabel3: (
  inputs: Productscoursezalolabel3Inputs
) => LocalizedString;
export declare const productsdescription1: (
  inputs: Productsdescription1Inputs
) => LocalizedString;
export declare const productsdescriptionlabel2: (
  inputs: Productsdescriptionlabel2Inputs
) => LocalizedString;
export declare const productsdialogdescription2: (
  inputs: Productsdialogdescription2Inputs
) => LocalizedString;
export declare const productseditdialogtitle3: (
  inputs: Productseditdialogtitle3Inputs
) => LocalizedString;
export declare const productsemptytitle2: (
  inputs: Productsemptytitle2Inputs
) => LocalizedString;
export declare const productsimagelabel2: (
  inputs: Productsimagelabel2Inputs
) => LocalizedString;
export declare const productsloaderrortitle3: (
  inputs: Productsloaderrortitle3Inputs
) => LocalizedString;
export declare const productsnamelabel2: (
  inputs: Productsnamelabel2Inputs
) => LocalizedString;
export declare const productspricelabel2: (
  inputs: Productspricelabel2Inputs
) => LocalizedString;
export declare const productsremoveconfirmaction3: (
  inputs: Productsremoveconfirmaction3Inputs
) => LocalizedString;
export declare const productsremoveconfirmbody3: (
  inputs: Productsremoveconfirmbody3Inputs
) => LocalizedString;
export declare const productsremoveconfirmtitle3: (
  inputs: Productsremoveconfirmtitle3Inputs
) => LocalizedString;
export declare const productsremoveerrortoast3: (
  inputs: Productsremoveerrortoast3Inputs
) => LocalizedString;
export declare const productssaveerrortoast3: (
  inputs: Productssaveerrortoast3Inputs
) => LocalizedString;
export declare const productssavesubmit2: (
  inputs: Productssavesubmit2Inputs
) => LocalizedString;
export declare const productstitle1: (
  inputs: Productstitle1Inputs
) => LocalizedString;
export declare const productstypecourse2: (
  inputs: Productstypecourse2Inputs
) => LocalizedString;
export declare const productstypelabel2: (
  inputs: Productstypelabel2Inputs
) => LocalizedString;
export declare const productstypeother2: (
  inputs: Productstypeother2Inputs
) => LocalizedString;
export declare const productstypeproduct2: (
  inputs: Productstypeproduct2Inputs
) => LocalizedString;
export declare const productstypeservice2: (
  inputs: Productstypeservice2Inputs
) => LocalizedString;
export declare const promptlibrarycopiedtoast3: (
  inputs: Promptlibrarycopiedtoast3Inputs
) => LocalizedString;
export declare const promptlibrarydescription2: (
  inputs: Promptlibrarydescription2Inputs
) => LocalizedString;
export declare const promptlibrarydevicedesktop3: (
  inputs: Promptlibrarydevicedesktop3Inputs
) => LocalizedString;
export declare const promptlibrarydevicemobile3: (
  inputs: Promptlibrarydevicemobile3Inputs
) => LocalizedString;
export declare const promptlibrarydevicetablet3: (
  inputs: Promptlibrarydevicetablet3Inputs
) => LocalizedString;
export declare const promptlibraryemptytitle3: (
  inputs: Promptlibraryemptytitle3Inputs
) => LocalizedString;
export declare const promptlibraryfullpromptsection4: (
  inputs: Promptlibraryfullpromptsection4Inputs
) => LocalizedString;
export declare const promptlibrarygeneratecta3: (
  inputs: Promptlibrarygeneratecta3Inputs
) => LocalizedString;
export declare const promptlibrarygeneratedialogdescription4: (
  inputs: Promptlibrarygeneratedialogdescription4Inputs
) => LocalizedString;
export declare const promptlibrarygeneratedialogtitle4: (
  inputs: Promptlibrarygeneratedialogtitle4Inputs
) => LocalizedString;
export declare const promptlibrarygeneratenamelabel4: (
  inputs: Promptlibrarygeneratenamelabel4Inputs
) => LocalizedString;
export declare const promptlibrarygeneratepromptlabel4: (
  inputs: Promptlibrarygeneratepromptlabel4Inputs
) => LocalizedString;
export declare const promptlibrarygeneratesubmit3: (
  inputs: Promptlibrarygeneratesubmit3Inputs
) => LocalizedString;
export declare const promptlibrarygeneratesubmitting3: (
  inputs: Promptlibrarygeneratesubmitting3Inputs
) => LocalizedString;
export declare const promptlibraryimportcta3: (
  inputs: Promptlibraryimportcta3Inputs
) => LocalizedString;
export declare const promptlibraryloaderrortitle4: (
  inputs: Promptlibraryloaderrortitle4Inputs
) => LocalizedString;
export declare const promptlibrarynopreview3: (
  inputs: Promptlibrarynopreview3Inputs
) => LocalizedString;
export declare const promptlibrarypreviewsection3: (
  inputs: Promptlibrarypreviewsection3Inputs
) => LocalizedString;
export declare const promptlibrarysearchplaceholder3: (
  inputs: Promptlibrarysearchplaceholder3Inputs
) => LocalizedString;
export declare const promptlibrarysummarysection3: (
  inputs: Promptlibrarysummarysection3Inputs
) => LocalizedString;
export declare const promptlibrarytitle2: (
  inputs: Promptlibrarytitle2Inputs
) => LocalizedString;
export declare const promptlibraryusetemplatecta4: (
  inputs: Promptlibraryusetemplatecta4Inputs
) => LocalizedString;
export declare const promptlibraryusetemplatesubmitting4: (
  inputs: Promptlibraryusetemplatesubmitting4Inputs
) => LocalizedString;
export declare const prompttemplateaddsection3: (
  inputs: Prompttemplateaddsection3Inputs
) => LocalizedString;
export declare const prompttemplateaddvariable3: (
  inputs: Prompttemplateaddvariable3Inputs
) => LocalizedString;
export declare const prompttemplatecolumnslug3: (
  inputs: Prompttemplatecolumnslug3Inputs
) => LocalizedString;
export declare const prompttemplatecolumntype3: (
  inputs: Prompttemplatecolumntype3Inputs
) => LocalizedString;
export declare const prompttemplatecolumnversion3: (
  inputs: Prompttemplatecolumnversion3Inputs
) => LocalizedString;
export declare const prompttemplatecompareheading3: (
  inputs: Prompttemplatecompareheading3Inputs
) => LocalizedString;
export declare const prompttemplatecompareselect3: (
  inputs: Prompttemplatecompareselect3Inputs
) => LocalizedString;
export declare const prompttemplatecompilebutton3: (
  inputs: Prompttemplatecompilebutton3Inputs
) => LocalizedString;
export declare const prompttemplatecompileerrortoast4: (
  inputs: Prompttemplatecompileerrortoast4Inputs
) => LocalizedString;
export declare const prompttemplateconnectionlabel3: (
  inputs: Prompttemplateconnectionlabel3Inputs
) => LocalizedString;
export declare const prompttemplateconnectionplatform3: (
  inputs: Prompttemplateconnectionplatform3Inputs
) => LocalizedString;
export declare const prompttemplateconnectiontrial3: (
  inputs: Prompttemplateconnectiontrial3Inputs
) => LocalizedString;
export declare const prompttemplatecreatebutton3: (
  inputs: Prompttemplatecreatebutton3Inputs
) => LocalizedString;
export declare const prompttemplatecreatedialogdescription4: (
  inputs: Prompttemplatecreatedialogdescription4Inputs
) => LocalizedString;
export declare const prompttemplatecreatedialogtitle4: (
  inputs: Prompttemplatecreatedialogtitle4Inputs
) => LocalizedString;
export declare const prompttemplatehistoryemptybody4: (
  inputs: Prompttemplatehistoryemptybody4Inputs
) => LocalizedString;
export declare const prompttemplatehistoryemptytitle4: (
  inputs: Prompttemplatehistoryemptytitle4Inputs
) => LocalizedString;
export declare const prompttemplatehistoryheading3: (
  inputs: Prompttemplatehistoryheading3Inputs
) => LocalizedString;
export declare const prompttemplatelighthouseunavailable3: (
  inputs: Prompttemplatelighthouseunavailable3Inputs
) => LocalizedString;
export declare const prompttemplatenotfoundtitle4: (
  inputs: Prompttemplatenotfoundtitle4Inputs
) => LocalizedString;
export declare const prompttemplateopenaction3: (
  inputs: Prompttemplateopenaction3Inputs
) => LocalizedString;
export declare const prompttemplateremoveaction3: (
  inputs: Prompttemplateremoveaction3Inputs
) => LocalizedString;
export declare const prompttemplateremoveconfirmbody4: (
  inputs: Prompttemplateremoveconfirmbody4Inputs
) => LocalizedString;
export declare const prompttemplateremoveconfirmtitle4: (
  inputs: Prompttemplateremoveconfirmtitle4Inputs
) => LocalizedString;
export declare const prompttemplateremoveerrortoast4: (
  inputs: Prompttemplateremoveerrortoast4Inputs
) => LocalizedString;
export declare const prompttemplateremovesection3: (
  inputs: Prompttemplateremovesection3Inputs
) => LocalizedString;
export declare const prompttemplateremovevariable3: (
  inputs: Prompttemplateremovevariable3Inputs
) => LocalizedString;
export declare const prompttemplateruntestbutton4: (
  inputs: Prompttemplateruntestbutton4Inputs
) => LocalizedString;
export declare const prompttemplatesaveerrortoast4: (
  inputs: Prompttemplatesaveerrortoast4Inputs
) => LocalizedString;
export declare const prompttemplatesectionkeyplaceholder4: (
  inputs: Prompttemplatesectionkeyplaceholder4Inputs
) => LocalizedString;
export declare const prompttemplatesectionslabel3: (
  inputs: Prompttemplatesectionslabel3Inputs
) => LocalizedString;
export declare const prompttemplatesluglabel3: (
  inputs: Prompttemplatesluglabel3Inputs
) => LocalizedString;
export declare const prompttemplatetabcompile3: (
  inputs: Prompttemplatetabcompile3Inputs
) => LocalizedString;
export declare const prompttemplatetabedit3: (
  inputs: Prompttemplatetabedit3Inputs
) => LocalizedString;
export declare const prompttemplatetabtestbench4: (
  inputs: Prompttemplatetabtestbench4Inputs
) => LocalizedString;
export declare const prompttemplatetestrunerrortoast5: (
  inputs: Prompttemplatetestrunerrortoast5Inputs
) => LocalizedString;
export declare const prompttemplatetestrunheading4: (
  inputs: Prompttemplatetestrunheading4Inputs
) => LocalizedString;
export declare const prompttemplatevariablekeyplaceholder4: (
  inputs: Prompttemplatevariablekeyplaceholder4Inputs
) => LocalizedString;
export declare const prompttemplatevariablelabelplaceholder4: (
  inputs: Prompttemplatevariablelabelplaceholder4Inputs
) => LocalizedString;
export declare const prompttemplatevariablerequired3: (
  inputs: Prompttemplatevariablerequired3Inputs
) => LocalizedString;
export declare const prompttemplatevariableslabel3: (
  inputs: Prompttemplatevariableslabel3Inputs
) => LocalizedString;
export declare const prompttemplatesdescription2: (
  inputs: Prompttemplatesdescription2Inputs
) => LocalizedString;
export declare const prompttemplatesemptytitle3: (
  inputs: Prompttemplatesemptytitle3Inputs
) => LocalizedString;
export declare const prompttemplatesloaderrortitle4: (
  inputs: Prompttemplatesloaderrortitle4Inputs
) => LocalizedString;
export declare const prompttemplatestitle2: (
  inputs: Prompttemplatestitle2Inputs
) => LocalizedString;
export declare const reconciliationcolumnamount2: (
  inputs: Reconciliationcolumnamount2Inputs
) => LocalizedString;
export declare const reconciliationcolumncandidates2: (
  inputs: Reconciliationcolumncandidates2Inputs
) => LocalizedString;
export declare const reconciliationcolumnreason2: (
  inputs: Reconciliationcolumnreason2Inputs
) => LocalizedString;
export declare const reconciliationcolumntransaction2: (
  inputs: Reconciliationcolumntransaction2Inputs
) => LocalizedString;
export declare const reconciliationdescription1: (
  inputs: Reconciliationdescription1Inputs
) => LocalizedString;
export declare const reconciliationdismissaction2: (
  inputs: Reconciliationdismissaction2Inputs
) => LocalizedString;
export declare const reconciliationdismissconfirmbody3: (
  inputs: Reconciliationdismissconfirmbody3Inputs
) => LocalizedString;
export declare const reconciliationdismissconfirmtitle3: (
  inputs: Reconciliationdismissconfirmtitle3Inputs
) => LocalizedString;
export declare const reconciliationdismisserrortoast3: (
  inputs: Reconciliationdismisserrortoast3Inputs
) => LocalizedString;
export declare const reconciliationemptytitle2: (
  inputs: Reconciliationemptytitle2Inputs
) => LocalizedString;
export declare const reconciliationloaderrortitle3: (
  inputs: Reconciliationloaderrortitle3Inputs
) => LocalizedString;
export declare const reconciliationreasonalreadypaid3: (
  inputs: Reconciliationreasonalreadypaid3Inputs
) => LocalizedString;
export declare const reconciliationreasonambiguous2: (
  inputs: Reconciliationreasonambiguous2Inputs
) => LocalizedString;
export declare const reconciliationreasonnocandidate3: (
  inputs: Reconciliationreasonnocandidate3Inputs
) => LocalizedString;
export declare const reconciliationresolveerrortoast3: (
  inputs: Reconciliationresolveerrortoast3Inputs
) => LocalizedString;
export declare const reconciliationsearchbutton2: (
  inputs: Reconciliationsearchbutton2Inputs
) => LocalizedString;
export declare const reconciliationsearcherrortoast3: (
  inputs: Reconciliationsearcherrortoast3Inputs
) => LocalizedString;
export declare const reconciliationsearchnoresults3: (
  inputs: Reconciliationsearchnoresults3Inputs
) => LocalizedString;
export declare const reconciliationsearchplaceholder2: (
  inputs: Reconciliationsearchplaceholder2Inputs
) => LocalizedString;
export declare const reconciliationselectorderaction3: (
  inputs: Reconciliationselectorderaction3Inputs
) => LocalizedString;
export declare const reconciliationsummaryalreadypaid3: (
  inputs: Reconciliationsummaryalreadypaid3Inputs
) => LocalizedString;
export declare const reconciliationsummaryambiguous2: (
  inputs: Reconciliationsummaryambiguous2Inputs
) => LocalizedString;
export declare const reconciliationsummarynocandidate3: (
  inputs: Reconciliationsummarynocandidate3Inputs
) => LocalizedString;
export declare const reconciliationsummarytitle2: (
  inputs: Reconciliationsummarytitle2Inputs
) => LocalizedString;
export declare const reconciliationtitle1: (
  inputs: Reconciliationtitle1Inputs
) => LocalizedString;
export declare const refundrequestscolumnamount3: (
  inputs: Refundrequestscolumnamount3Inputs
) => LocalizedString;
export declare const refundrequestscolumnlead3: (
  inputs: Refundrequestscolumnlead3Inputs
) => LocalizedString;
export declare const refundrequestscolumnorder3: (
  inputs: Refundrequestscolumnorder3Inputs
) => LocalizedString;
export declare const refundrequestscolumnreason3: (
  inputs: Refundrequestscolumnreason3Inputs
) => LocalizedString;
export declare const refundrequestscolumnstatus3: (
  inputs: Refundrequestscolumnstatus3Inputs
) => LocalizedString;
export declare const refundrequestscompleteerrortoast4: (
  inputs: Refundrequestscompleteerrortoast4Inputs
) => LocalizedString;
export declare const refundrequestsdescription2: (
  inputs: Refundrequestsdescription2Inputs
) => LocalizedString;
export declare const refundrequestsduplicatebadge3: (
  inputs: Refundrequestsduplicatebadge3Inputs
) => LocalizedString;
export declare const refundrequestsemptytitle3: (
  inputs: Refundrequestsemptytitle3Inputs
) => LocalizedString;
export declare const refundrequestsevidencelabel3: (
  inputs: Refundrequestsevidencelabel3Inputs
) => LocalizedString;
export declare const refundrequestsfilterall3: (
  inputs: Refundrequestsfilterall3Inputs
) => LocalizedString;
export declare const refundrequestsloaderrortitle4: (
  inputs: Refundrequestsloaderrortitle4Inputs
) => LocalizedString;
export declare const refundrequestsmarktransferredbutton4: (
  inputs: Refundrequestsmarktransferredbutton4Inputs
) => LocalizedString;
export declare const refundrequestsreasoncustomer3: (
  inputs: Refundrequestsreasoncustomer3Inputs
) => LocalizedString;
export declare const refundrequestsreasonduplicatepayment4: (
  inputs: Refundrequestsreasonduplicatepayment4Inputs
) => LocalizedString;
export declare const refundrequestsreasonother3: (
  inputs: Refundrequestsreasonother3Inputs
) => LocalizedString;
export declare const refundrequestsreasonwrongmatch4: (
  inputs: Refundrequestsreasonwrongmatch4Inputs
) => LocalizedString;
export declare const refundrequestsrejectbutton3: (
  inputs: Refundrequestsrejectbutton3Inputs
) => LocalizedString;
export declare const refundrequestsrejectconfirmaction4: (
  inputs: Refundrequestsrejectconfirmaction4Inputs
) => LocalizedString;
export declare const refundrequestsrejectconfirmbody4: (
  inputs: Refundrequestsrejectconfirmbody4Inputs
) => LocalizedString;
export declare const refundrequestsrejectconfirmtitle4: (
  inputs: Refundrequestsrejectconfirmtitle4Inputs
) => LocalizedString;
export declare const refundrequestsrejecterrortoast4: (
  inputs: Refundrequestsrejecterrortoast4Inputs
) => LocalizedString;
export declare const refundrequestsremitterlabel3: (
  inputs: Refundrequestsremitterlabel3Inputs
) => LocalizedString;
export declare const refundrequestsstatuscompleted3: (
  inputs: Refundrequestsstatuscompleted3Inputs
) => LocalizedString;
export declare const refundrequestsstatuspending3: (
  inputs: Refundrequestsstatuspending3Inputs
) => LocalizedString;
export declare const refundrequestsstatusprocessing3: (
  inputs: Refundrequestsstatusprocessing3Inputs
) => LocalizedString;
export declare const refundrequestsstatusrejected3: (
  inputs: Refundrequestsstatusrejected3Inputs
) => LocalizedString;
export declare const refundrequeststitle2: (
  inputs: Refundrequeststitle2Inputs
) => LocalizedString;
export declare const refundrequestsuploaderrortoast4: (
  inputs: Refundrequestsuploaderrortoast4Inputs
) => LocalizedString;
export declare const refundrequestsuploadevidencebutton4: (
  inputs: Refundrequestsuploadevidencebutton4Inputs
) => LocalizedString;
export declare const resetpasswordinvalidlink3: (
  inputs: Resetpasswordinvalidlink3Inputs
) => LocalizedString;
export declare const resetpasswordnewlabel3: (
  inputs: Resetpasswordnewlabel3Inputs
) => LocalizedString;
export declare const resetpasswordsubmit2: (
  inputs: Resetpasswordsubmit2Inputs
) => LocalizedString;
export declare const resetpasswordsuccess2: (
  inputs: Resetpasswordsuccess2Inputs
) => LocalizedString;
export declare const resetpasswordtitle2: (
  inputs: Resetpasswordtitle2Inputs
) => LocalizedString;
export declare const roleadmin1: (inputs: Roleadmin1Inputs) => LocalizedString;
export declare const roleadmindescription2: (
  inputs: Roleadmindescription2Inputs
) => LocalizedString;
export declare const roleeditor1: (
  inputs: Roleeditor1Inputs
) => LocalizedString;
export declare const roleeditordescription2: (
  inputs: Roleeditordescription2Inputs
) => LocalizedString;
export declare const roleowner1: (inputs: Roleowner1Inputs) => LocalizedString;
export declare const roleownerdescription2: (
  inputs: Roleownerdescription2Inputs
) => LocalizedString;
export declare const rolesales1: (inputs: Rolesales1Inputs) => LocalizedString;
export declare const rolesalesdescription2: (
  inputs: Rolesalesdescription2Inputs
) => LocalizedString;
export declare const routeerrordescription2: (
  inputs: Routeerrordescription2Inputs
) => LocalizedString;
export declare const routeerrortitle2: (
  inputs: Routeerrortitle2Inputs
) => LocalizedString;
export declare const routenotfoundbackbutton4: (
  inputs: Routenotfoundbackbutton4Inputs
) => LocalizedString;
export declare const routenotfounddescription3: (
  inputs: Routenotfounddescription3Inputs
) => LocalizedString;
export declare const routenotfoundtitle3: (
  inputs: Routenotfoundtitle3Inputs
) => LocalizedString;
export declare const settingsbrandbodyfontlabel4: (
  inputs: Settingsbrandbodyfontlabel4Inputs
) => LocalizedString;
export declare const settingsbrandheadingfontlabel4: (
  inputs: Settingsbrandheadingfontlabel4Inputs
) => LocalizedString;
export declare const settingsbrandkitdescription3: (
  inputs: Settingsbrandkitdescription3Inputs
) => LocalizedString;
export declare const settingsbrandkittitle3: (
  inputs: Settingsbrandkittitle3Inputs
) => LocalizedString;
export declare const settingsbrandprimarycolorlabel4: (
  inputs: Settingsbrandprimarycolorlabel4Inputs
) => LocalizedString;
export declare const settingsbrandsecondarycolorlabel4: (
  inputs: Settingsbrandsecondarycolorlabel4Inputs
) => LocalizedString;
export declare const settingsdescription1: (
  inputs: Settingsdescription1Inputs
) => LocalizedString;
export declare const settingsforbiddentitle2: (
  inputs: Settingsforbiddentitle2Inputs
) => LocalizedString;
export declare const settingsleaddigestfrequencydaily4: (
  inputs: Settingsleaddigestfrequencydaily4Inputs
) => LocalizedString;
export declare const settingsleaddigestfrequencydescription4: (
  inputs: Settingsleaddigestfrequencydescription4Inputs
) => LocalizedString;
export declare const settingsleaddigestfrequencyhourly4: (
  inputs: Settingsleaddigestfrequencyhourly4Inputs
) => LocalizedString;
export declare const settingsleaddigestfrequencylabel4: (
  inputs: Settingsleaddigestfrequencylabel4Inputs
) => LocalizedString;
export declare const settingsloaderrortitle3: (
  inputs: Settingsloaderrortitle3Inputs
) => LocalizedString;
export declare const settingslogodescription2: (
  inputs: Settingslogodescription2Inputs
) => LocalizedString;
export declare const settingslogolabel2: (
  inputs: Settingslogolabel2Inputs
) => LocalizedString;
export declare const settingssaveerrortoast3: (
  inputs: Settingssaveerrortoast3Inputs
) => LocalizedString;
export declare const settingstitle1: (
  inputs: Settingstitle1Inputs
) => LocalizedString;
export declare const shellainav2: (
  inputs: Shellainav2Inputs
) => LocalizedString;
export declare const shellcampaignsnav2: (
  inputs: Shellcampaignsnav2Inputs
) => LocalizedString;
export declare const shellcommandpaletteempty3: (
  inputs: Shellcommandpaletteempty3Inputs
) => LocalizedString;
export declare const shellcommandpaletteinputplaceholder4: (
  inputs: Shellcommandpaletteinputplaceholder4Inputs
) => LocalizedString;
export declare const shelldomainsnav2: (
  inputs: Shelldomainsnav2Inputs
) => LocalizedString;
export declare const shellgroupadvanced2: (
  inputs: Shellgroupadvanced2Inputs
) => LocalizedString;
export declare const shellgroupai2: (
  inputs: Shellgroupai2Inputs
) => LocalizedString;
export declare const shellgroupcontent2: (
  inputs: Shellgroupcontent2Inputs
) => LocalizedString;
export declare const shellgrouporganization2: (
  inputs: Shellgrouporganization2Inputs
) => LocalizedString;
export declare const shellgrouppayments2: (
  inputs: Shellgrouppayments2Inputs
) => LocalizedString;
export declare const shellgroupsales2: (
  inputs: Shellgroupsales2Inputs
) => LocalizedString;
export declare const shellgroupsetup2: (
  inputs: Shellgroupsetup2Inputs
) => LocalizedString;
export declare const shellgroupworkspace2: (
  inputs: Shellgroupworkspace2Inputs
) => LocalizedString;
export declare const shelllandingsnav2: (
  inputs: Shelllandingsnav2Inputs
) => LocalizedString;
export declare const shellleadsnav2: (
  inputs: Shellleadsnav2Inputs
) => LocalizedString;
export declare const shelllogout1: (
  inputs: Shelllogout1Inputs
) => LocalizedString;
export declare const shellmembersnav2: (
  inputs: Shellmembersnav2Inputs
) => LocalizedString;
export declare const shelloffersnav2: (
  inputs: Shelloffersnav2Inputs
) => LocalizedString;
export declare const shellordersnav2: (
  inputs: Shellordersnav2Inputs
) => LocalizedString;
export declare const shellpaymentsnav2: (
  inputs: Shellpaymentsnav2Inputs
) => LocalizedString;
export declare const shellproductsnav2: (
  inputs: Shellproductsnav2Inputs
) => LocalizedString;
export declare const shellpromptlibrarynav3: (
  inputs: Shellpromptlibrarynav3Inputs
) => LocalizedString;
export declare const shellprompttemplatesnav3: (
  inputs: Shellprompttemplatesnav3Inputs
) => LocalizedString;
export declare const shellreconciliationnav2: (
  inputs: Shellreconciliationnav2Inputs
) => LocalizedString;
export declare const shellrefundrequestsnav3: (
  inputs: Shellrefundrequestsnav3Inputs
) => LocalizedString;
export declare const shellsearchplaceholder2: (
  inputs: Shellsearchplaceholder2Inputs
) => LocalizedString;
export declare const shellsettingsnav2: (
  inputs: Shellsettingsnav2Inputs
) => LocalizedString;
export declare const shellsignedinas3: (
  inputs: Shellsignedinas3Inputs
) => LocalizedString;
export declare const shellskillsnav2: (
  inputs: Shellskillsnav2Inputs
) => LocalizedString;
export declare const shellthemetoggletodark4: (
  inputs: Shellthemetoggletodark4Inputs
) => LocalizedString;
export declare const shellthemetoggletolight4: (
  inputs: Shellthemetoggletolight4Inputs
) => LocalizedString;
export declare const shelltodaynav2: (
  inputs: Shelltodaynav2Inputs
) => LocalizedString;
export declare const signupemaillabel2: (
  inputs: Signupemaillabel2Inputs
) => LocalizedString;
export declare const signuphasaccount2: (
  inputs: Signuphasaccount2Inputs
) => LocalizedString;
export declare const signuploginlink2: (
  inputs: Signuploginlink2Inputs
) => LocalizedString;
export declare const signupnamelabel2: (
  inputs: Signupnamelabel2Inputs
) => LocalizedString;
export declare const signuppasswordlabel2: (
  inputs: Signuppasswordlabel2Inputs
) => LocalizedString;
export declare const signupsubmit1: (
  inputs: Signupsubmit1Inputs
) => LocalizedString;
export declare const signuptitle1: (
  inputs: Signuptitle1Inputs
) => LocalizedString;
export declare const skillcolumnactions2: (
  inputs: Skillcolumnactions2Inputs
) => LocalizedString;
export declare const skillcolumndefault2: (
  inputs: Skillcolumndefault2Inputs
) => LocalizedString;
export declare const skillcolumnname2: (
  inputs: Skillcolumnname2Inputs
) => LocalizedString;
export declare const skillcolumnslug2: (
  inputs: Skillcolumnslug2Inputs
) => LocalizedString;
export declare const skillcolumntype2: (
  inputs: Skillcolumntype2Inputs
) => LocalizedString;
export declare const skillcontentlabel2: (
  inputs: Skillcontentlabel2Inputs
) => LocalizedString;
export declare const skillcreatebutton2: (
  inputs: Skillcreatebutton2Inputs
) => LocalizedString;
export declare const skillcreatedialogtitle3: (
  inputs: Skillcreatedialogtitle3Inputs
) => LocalizedString;
export declare const skilldialogdescription2: (
  inputs: Skilldialogdescription2Inputs
) => LocalizedString;
export declare const skilleditdialogtitle3: (
  inputs: Skilleditdialogtitle3Inputs
) => LocalizedString;
export declare const skillnamelabel2: (
  inputs: Skillnamelabel2Inputs
) => LocalizedString;
export declare const skillpreviewlabel2: (
  inputs: Skillpreviewlabel2Inputs
) => LocalizedString;
export declare const skillremoveaction2: (
  inputs: Skillremoveaction2Inputs
) => LocalizedString;
export declare const skillremoveconfirmbody3: (
  inputs: Skillremoveconfirmbody3Inputs
) => LocalizedString;
export declare const skillremoveconfirmtitle3: (
  inputs: Skillremoveconfirmtitle3Inputs
) => LocalizedString;
export declare const skillremoveerrortoast3: (
  inputs: Skillremoveerrortoast3Inputs
) => LocalizedString;
export declare const skillsaveerrortoast3: (
  inputs: Skillsaveerrortoast3Inputs
) => LocalizedString;
export declare const skillsluglabel2: (
  inputs: Skillsluglabel2Inputs
) => LocalizedString;
export declare const skilltoggledefaulterrortoast4: (
  inputs: Skilltoggledefaulterrortoast4Inputs
) => LocalizedString;
export declare const skilltypeplatform2: (
  inputs: Skilltypeplatform2Inputs
) => LocalizedString;
export declare const skilltypetenant2: (
  inputs: Skilltypetenant2Inputs
) => LocalizedString;
export declare const skillsdescription1: (
  inputs: Skillsdescription1Inputs
) => LocalizedString;
export declare const skillsemptytitle2: (
  inputs: Skillsemptytitle2Inputs
) => LocalizedString;
export declare const skillsloaderrortitle3: (
  inputs: Skillsloaderrortitle3Inputs
) => LocalizedString;
export declare const skillstitle1: (
  inputs: Skillstitle1Inputs
) => LocalizedString;
export declare const studioautosavesaved2: (
  inputs: Studioautosavesaved2Inputs
) => LocalizedString;
export declare const studioautosavesaving2: (
  inputs: Studioautosavesaving2Inputs
) => LocalizedString;
export declare const studiobacktocustomimport4: (
  inputs: Studiobacktocustomimport4Inputs
) => LocalizedString;
export declare const studiochatemptydescription3: (
  inputs: Studiochatemptydescription3Inputs
) => LocalizedString;
export declare const studiochatemptytitle3: (
  inputs: Studiochatemptytitle3Inputs
) => LocalizedString;
export declare const studiochatplaceholder2: (
  inputs: Studiochatplaceholder2Inputs
) => LocalizedString;
export declare const studiochatsuggestioncta3: (
  inputs: Studiochatsuggestioncta3Inputs
) => LocalizedString;
export declare const studiochatsuggestionheadline3: (
  inputs: Studiochatsuggestionheadline3Inputs
) => LocalizedString;
export declare const studiochatsuggestionmobile3: (
  inputs: Studiochatsuggestionmobile3Inputs
) => LocalizedString;
export declare const studiochatviewversionlink4: (
  inputs: Studiochatviewversionlink4Inputs
) => LocalizedString;
export declare const studiocomingsoon2: (
  inputs: Studiocomingsoon2Inputs
) => LocalizedString;
export declare const studiocommentattachlabel3: (
  inputs: Studiocommentattachlabel3Inputs
) => LocalizedString;
export declare const studiocommentchatchip3: (
  inputs: Studiocommentchatchip3Inputs
) => LocalizedString;
export declare const studiocommentplaceholder2: (
  inputs: Studiocommentplaceholder2Inputs
) => LocalizedString;
export declare const studiocommentqueuebutton3: (
  inputs: Studiocommentqueuebutton3Inputs
) => LocalizedString;
export declare const studiocommentqueueempty3: (
  inputs: Studiocommentqueueempty3Inputs
) => LocalizedString;
export declare const studiocommentqueueerrortoast4: (
  inputs: Studiocommentqueueerrortoast4Inputs
) => LocalizedString;
export declare const studiocommentqueuetitle3: (
  inputs: Studiocommentqueuetitle3Inputs
) => LocalizedString;
export declare const studiocommentqueuedtoast3: (
  inputs: Studiocommentqueuedtoast3Inputs
) => LocalizedString;
export declare const studiocommentsendallbutton4: (
  inputs: Studiocommentsendallbutton4Inputs
) => LocalizedString;
export declare const studiocommentsendallerrortoast5: (
  inputs: Studiocommentsendallerrortoast5Inputs
) => LocalizedString;
export declare const studiocommentsendalltoast4: (
  inputs: Studiocommentsendalltoast4Inputs
) => LocalizedString;
export declare const studiocommentsenderrortoast4: (
  inputs: Studiocommentsenderrortoast4Inputs
) => LocalizedString;
export declare const studiocommentsendtochatbutton5: (
  inputs: Studiocommentsendtochatbutton5Inputs
) => LocalizedString;
export declare const studiocommentsenttoast3: (
  inputs: Studiocommentsenttoast3Inputs
) => LocalizedString;
export declare const studiodeleteconfirmaction3: (
  inputs: Studiodeleteconfirmaction3Inputs
) => LocalizedString;
export declare const studiodeleteconfirmcancel3: (
  inputs: Studiodeleteconfirmcancel3Inputs
) => LocalizedString;
export declare const studiodeleteconfirmdescription3: (
  inputs: Studiodeleteconfirmdescription3Inputs
) => LocalizedString;
export declare const studiodeleteconfirmtitle3: (
  inputs: Studiodeleteconfirmtitle3Inputs
) => LocalizedString;
export declare const studiodeleteerrortoast3: (
  inputs: Studiodeleteerrortoast3Inputs
) => LocalizedString;
export declare const studiodeployhistorytitle3: (
  inputs: Studiodeployhistorytitle3Inputs
) => LocalizedString;
export declare const studiodeploystatussuperseded3: (
  inputs: Studiodeploystatussuperseded3Inputs
) => LocalizedString;
export declare const studiodeploystatusunpublished3: (
  inputs: Studiodeploystatusunpublished3Inputs
) => LocalizedString;
export declare const studiodesignfilesemptydescription4: (
  inputs: Studiodesignfilesemptydescription4Inputs
) => LocalizedString;
export declare const studiodesignfilesemptytitle4: (
  inputs: Studiodesignfilesemptytitle4Inputs
) => LocalizedString;
export declare const studiodownload1: (
  inputs: Studiodownload1Inputs
) => LocalizedString;
export declare const studiodrawclear2: (
  inputs: Studiodrawclear2Inputs
) => LocalizedString;
export declare const studiodrawcolorlabel3: (
  inputs: Studiodrawcolorlabel3Inputs
) => LocalizedString;
export declare const studiodrawpromptplaceholder3: (
  inputs: Studiodrawpromptplaceholder3Inputs
) => LocalizedString;
export declare const studiodrawsendbutton3: (
  inputs: Studiodrawsendbutton3Inputs
) => LocalizedString;
export declare const studiodrawsenderrortoast4: (
  inputs: Studiodrawsenderrortoast4Inputs
) => LocalizedString;
export declare const studiodrawsending2: (
  inputs: Studiodrawsending2Inputs
) => LocalizedString;
export declare const studiodrawtoolarrow3: (
  inputs: Studiodrawtoolarrow3Inputs
) => LocalizedString;
export declare const studiodrawtoolpen3: (
  inputs: Studiodrawtoolpen3Inputs
) => LocalizedString;
export declare const studiodrawtoolrect3: (
  inputs: Studiodrawtoolrect3Inputs
) => LocalizedString;
export declare const studiodrawundo2: (
  inputs: Studiodrawundo2Inputs
) => LocalizedString;
export declare const studioduplicateerrortoast3: (
  inputs: Studioduplicateerrortoast3Inputs
) => LocalizedString;
export declare const studioexport1: (
  inputs: Studioexport1Inputs
) => LocalizedString;
export declare const studioexporthtml2: (
  inputs: Studioexporthtml2Inputs
) => LocalizedString;
export declare const studioexportpng2: (
  inputs: Studioexportpng2Inputs
) => LocalizedString;
export declare const studioexportzip2: (
  inputs: Studioexportzip2Inputs
) => LocalizedString;
export declare const studiofilesfolderassets3: (
  inputs: Studiofilesfolderassets3Inputs
) => LocalizedString;
export declare const studiofilesfolderscreenshots3: (
  inputs: Studiofilesfolderscreenshots3Inputs
) => LocalizedString;
export declare const studiofilesgroupdata3: (
  inputs: Studiofilesgroupdata3Inputs
) => LocalizedString;
export declare const studiofilesgroupfolders3: (
  inputs: Studiofilesgroupfolders3Inputs
) => LocalizedString;
export declare const studiofilesgroupimages3: (
  inputs: Studiofilesgroupimages3Inputs
) => LocalizedString;
export declare const studiofilesgrouppages3: (
  inputs: Studiofilesgrouppages3Inputs
) => LocalizedString;
export declare const studiofilesjsonviewerempty4: (
  inputs: Studiofilesjsonviewerempty4Inputs
) => LocalizedString;
export declare const studiofilesjsonviewertitle4: (
  inputs: Studiofilesjsonviewertitle4Inputs
) => LocalizedString;
export declare const studiofileslightboxempty3: (
  inputs: Studiofileslightboxempty3Inputs
) => LocalizedString;
export declare const studiofileslightboxtitle3: (
  inputs: Studiofileslightboxtitle3Inputs
) => LocalizedString;
export declare const studiofilesprojecttitle3: (
  inputs: Studiofilesprojecttitle3Inputs
) => LocalizedString;
export declare const studiofilesrefreshlabel3: (
  inputs: Studiofilesrefreshlabel3Inputs
) => LocalizedString;
export declare const studiofilesthumbnailname3: (
  inputs: Studiofilesthumbnailname3Inputs
) => LocalizedString;
export declare const studiofilesuploadfailed3: (
  inputs: Studiofilesuploadfailed3Inputs
) => LocalizedString;
export declare const studiofilesuploadlabel3: (
  inputs: Studiofilesuploadlabel3Inputs
) => LocalizedString;
export declare const studiofilesvideotoolarge4: (
  inputs: Studiofilesvideotoolarge4Inputs
) => LocalizedString;
export declare const studiofunnelfixformprompt4: (
  inputs: Studiofunnelfixformprompt4Inputs
) => LocalizedString;
export declare const studiofunnelfixseoprompt4: (
  inputs: Studiofunnelfixseoprompt4Inputs
) => LocalizedString;
export declare const studiofunnelgapsbothdescription4: (
  inputs: Studiofunnelgapsbothdescription4Inputs
) => LocalizedString;
export declare const studiofunnelgapsdismiss3: (
  inputs: Studiofunnelgapsdismiss3Inputs
) => LocalizedString;
export declare const studiofunnelgapsfixbutton4: (
  inputs: Studiofunnelgapsfixbutton4Inputs
) => LocalizedString;
export declare const studiofunnelgapsformdescription4: (
  inputs: Studiofunnelgapsformdescription4Inputs
) => LocalizedString;
export declare const studiofunnelgapsseodescription4: (
  inputs: Studiofunnelgapsseodescription4Inputs
) => LocalizedString;
export declare const studiofunnelgapstitle3: (
  inputs: Studiofunnelgapstitle3Inputs
) => LocalizedString;
export declare const studiogenerateerrorbuycreditsbutton5: (
  inputs: Studiogenerateerrorbuycreditsbutton5Inputs
) => LocalizedString;
export declare const studiogenerateerrorconnectaibutton5: (
  inputs: Studiogenerateerrorconnectaibutton5Inputs
) => LocalizedString;
export declare const studiogenerateerrorinsufficientcreditsdescription5: (
  inputs: Studiogenerateerrorinsufficientcreditsdescription5Inputs
) => LocalizedString;
export declare const studiogenerateerrorinsufficientcreditstitle5: (
  inputs: Studiogenerateerrorinsufficientcreditstitle5Inputs
) => LocalizedString;
export declare const studiogenerateerrornoaiconnectiondescription6: (
  inputs: Studiogenerateerrornoaiconnectiondescription6Inputs
) => LocalizedString;
export declare const studiogenerateerrornoaiconnectiontitle6: (
  inputs: Studiogenerateerrornoaiconnectiontitle6Inputs
) => LocalizedString;
export declare const studiogenerateerrortrialexhausteddescription5: (
  inputs: Studiogenerateerrortrialexhausteddescription5Inputs
) => LocalizedString;
export declare const studiogenerateerrortrialexhaustedtitle5: (
  inputs: Studiogenerateerrortrialexhaustedtitle5Inputs
) => LocalizedString;
export declare const studiohidechat2: (
  inputs: Studiohidechat2Inputs
) => LocalizedString;
export declare const studiohidesidebar2: (
  inputs: Studiohidesidebar2Inputs
) => LocalizedString;
export declare const studioinspectempty2: (
  inputs: Studioinspectempty2Inputs
) => LocalizedString;
export declare const studiolayersdragdisabledhint4: (
  inputs: Studiolayersdragdisabledhint4Inputs
) => LocalizedString;
export declare const studiolayersfooterhint3: (
  inputs: Studiolayersfooterhint3Inputs
) => LocalizedString;
export declare const studiomoreactionslabel3: (
  inputs: Studiomoreactionslabel3Inputs
) => LocalizedString;
export declare const studionativechatsuggestionheadline4: (
  inputs: Studionativechatsuggestionheadline4Inputs
) => LocalizedString;
export declare const studionativechatsuggestiontestimonial4: (
  inputs: Studionativechatsuggestiontestimonial4Inputs
) => LocalizedString;
export declare const studionativechatsuggestiontone4: (
  inputs: Studionativechatsuggestiontone4Inputs
) => LocalizedString;
export declare const studionativechattooltitle4: (
  inputs: Studionativechattooltitle4Inputs
) => LocalizedString;
export declare const studionew1: (inputs: Studionew1Inputs) => LocalizedString;
export declare const studiopresent1: (
  inputs: Studiopresent1Inputs
) => LocalizedString;
export declare const studiopreviewbutton2: (
  inputs: Studiopreviewbutton2Inputs
) => LocalizedString;
export declare const studiopreviewerrortoast3: (
  inputs: Studiopreviewerrortoast3Inputs
) => LocalizedString;
export declare const studiopublishanywaybutton3: (
  inputs: Studiopublishanywaybutton3Inputs
) => LocalizedString;
export declare const studiopublishattachcampaignbutton4: (
  inputs: Studiopublishattachcampaignbutton4Inputs
) => LocalizedString;
export declare const studiopublishauditrequiredtoast4: (
  inputs: Studiopublishauditrequiredtoast4Inputs
) => LocalizedString;
export declare const studiopublishauditthresholdtoast4: (
  inputs: Studiopublishauditthresholdtoast4Inputs
) => LocalizedString;
export declare const studiopublishbutton2: (
  inputs: Studiopublishbutton2Inputs
) => LocalizedString;
export declare const studiopublishcampaignattacherrortoast5: (
  inputs: Studiopublishcampaignattacherrortoast5Inputs
) => LocalizedString;
export declare const studiopublishcampaignattachedtoast4: (
  inputs: Studiopublishcampaignattachedtoast4Inputs
) => LocalizedString;
export declare const studiopublishcampaignsearchempty4: (
  inputs: Studiopublishcampaignsearchempty4Inputs
) => LocalizedString;
export declare const studiopublishcampaignsearchplaceholder4: (
  inputs: Studiopublishcampaignsearchplaceholder4Inputs
) => LocalizedString;
export declare const studiopublishdialogdescription3: (
  inputs: Studiopublishdialogdescription3Inputs
) => LocalizedString;
export declare const studiopublishdialogtitle3: (
  inputs: Studiopublishdialogtitle3Inputs
) => LocalizedString;
export declare const studiopublisherrortoast3: (
  inputs: Studiopublisherrortoast3Inputs
) => LocalizedString;
export declare const studiopublishnocampaignwarning4: (
  inputs: Studiopublishnocampaignwarning4Inputs
) => LocalizedString;
export declare const studiopublishsubdomainlabel3: (
  inputs: Studiopublishsubdomainlabel3Inputs
) => LocalizedString;
export declare const studiopublishsubdomaintakentoast4: (
  inputs: Studiopublishsubdomaintakentoast4Inputs
) => LocalizedString;
export declare const studiopublishsuccesstoast3: (
  inputs: Studiopublishsuccesstoast3Inputs
) => LocalizedString;
export declare const studiopublishunverifiedassetstoast4: (
  inputs: Studiopublishunverifiedassetstoast4Inputs
) => LocalizedString;
export declare const studiopublishupdatebutton3: (
  inputs: Studiopublishupdatebutton3Inputs
) => LocalizedString;
export declare const studiorenameerrortoast3: (
  inputs: Studiorenameerrortoast3Inputs
) => LocalizedString;
export declare const studiorollbackbutton2: (
  inputs: Studiorollbackbutton2Inputs
) => LocalizedString;
export declare const studiorollbackerrortoast3: (
  inputs: Studiorollbackerrortoast3Inputs
) => LocalizedString;
export declare const studiorollbacksuccesstoast3: (
  inputs: Studiorollbacksuccesstoast3Inputs
) => LocalizedString;
export declare const studiosavebutton2: (
  inputs: Studiosavebutton2Inputs
) => LocalizedString;
export declare const studiosaveshortcuthint3: (
  inputs: Studiosaveshortcuthint3Inputs
) => LocalizedString;
export declare const studioseocanonicalurlhint4: (
  inputs: Studioseocanonicalurlhint4Inputs
) => LocalizedString;
export declare const studioseocanonicalurllabel4: (
  inputs: Studioseocanonicalurllabel4Inputs
) => LocalizedString;
export declare const studioseodescriptionlabel3: (
  inputs: Studioseodescriptionlabel3Inputs
) => LocalizedString;
export declare const studioseonofollowlabel3: (
  inputs: Studioseonofollowlabel3Inputs
) => LocalizedString;
export declare const studioseonoindexhint3: (
  inputs: Studioseonoindexhint3Inputs
) => LocalizedString;
export declare const studioseonoindexlabel3: (
  inputs: Studioseonoindexlabel3Inputs
) => LocalizedString;
export declare const studioseoogimagelabel4: (
  inputs: Studioseoogimagelabel4Inputs
) => LocalizedString;
export declare const studioseopaneltitle3: (
  inputs: Studioseopaneltitle3Inputs
) => LocalizedString;
export declare const studioseosharepreviewlabel4: (
  inputs: Studioseosharepreviewlabel4Inputs
) => LocalizedString;
export declare const studioseosharepreviewnoimage5: (
  inputs: Studioseosharepreviewnoimage5Inputs
) => LocalizedString;
export declare const studioseostructureddatatypearticle5: (
  inputs: Studioseostructureddatatypearticle5Inputs
) => LocalizedString;
export declare const studioseostructureddatatypeauto5: (
  inputs: Studioseostructureddatatypeauto5Inputs
) => LocalizedString;
export declare const studioseostructureddatatypecourse5: (
  inputs: Studioseostructureddatatypecourse5Inputs
) => LocalizedString;
export declare const studioseostructureddatatypelabel5: (
  inputs: Studioseostructureddatatypelabel5Inputs
) => LocalizedString;
export declare const studioseostructureddatatypelocalbusiness6: (
  inputs: Studioseostructureddatatypelocalbusiness6Inputs
) => LocalizedString;
export declare const studioseostructureddatatypeorganization5: (
  inputs: Studioseostructureddatatypeorganization5Inputs
) => LocalizedString;
export declare const studioseostructureddatatypeproduct5: (
  inputs: Studioseostructureddatatypeproduct5Inputs
) => LocalizedString;
export declare const studioseostructureddatatypewebpage6: (
  inputs: Studioseostructureddatatypewebpage6Inputs
) => LocalizedString;
export declare const studioseotitlehint3: (
  inputs: Studioseotitlehint3Inputs
) => LocalizedString;
export declare const studioseotitlelabel3: (
  inputs: Studioseotitlelabel3Inputs
) => LocalizedString;
export declare const studioseotwittercardlabel4: (
  inputs: Studioseotwittercardlabel4Inputs
) => LocalizedString;
export declare const studioseotwittercardsummary4: (
  inputs: Studioseotwittercardsummary4Inputs
) => LocalizedString;
export declare const studioseotwittercardsummarylargeimage6: (
  inputs: Studioseotwittercardsummarylargeimage6Inputs
) => LocalizedString;
export declare const studioshare1: (
  inputs: Studioshare1Inputs
) => LocalizedString;
export declare const studioshortcutcomment2: (
  inputs: Studioshortcutcomment2Inputs
) => LocalizedString;
export declare const studioshortcutdraw2: (
  inputs: Studioshortcutdraw2Inputs
) => LocalizedString;
export declare const studioshortcutedit2: (
  inputs: Studioshortcutedit2Inputs
) => LocalizedString;
export declare const studioshortcutzoomin3: (
  inputs: Studioshortcutzoomin3Inputs
) => LocalizedString;
export declare const studioshortcutzoomout3: (
  inputs: Studioshortcutzoomout3Inputs
) => LocalizedString;
export declare const studioshowchat2: (
  inputs: Studioshowchat2Inputs
) => LocalizedString;
export declare const studioshowsidebar2: (
  inputs: Studioshowsidebar2Inputs
) => LocalizedString;
export declare const studiosidebarfiles2: (
  inputs: Studiosidebarfiles2Inputs
) => LocalizedString;
export declare const studiosidebarhistory2: (
  inputs: Studiosidebarhistory2Inputs
) => LocalizedString;
export declare const studiosidebarinspect2: (
  inputs: Studiosidebarinspect2Inputs
) => LocalizedString;
export declare const studiosidebarlayers2: (
  inputs: Studiosidebarlayers2Inputs
) => LocalizedString;
export declare const studioskillsbutton2: (
  inputs: Studioskillsbutton2Inputs
) => LocalizedString;
export declare const studioskillsemptytitle3: (
  inputs: Studioskillsemptytitle3Inputs
) => LocalizedString;
export declare const studioskillsloaderrortitle4: (
  inputs: Studioskillsloaderrortitle4Inputs
) => LocalizedString;
export declare const studioskillspopoverdescription3: (
  inputs: Studioskillspopoverdescription3Inputs
) => LocalizedString;
export declare const studioskillspopovertitle3: (
  inputs: Studioskillspopovertitle3Inputs
) => LocalizedString;
export declare const studioskillstoggleerrortoast4: (
  inputs: Studioskillstoggleerrortoast4Inputs
) => LocalizedString;
export declare const studiotabdesignfiles3: (
  inputs: Studiotabdesignfiles3Inputs
) => LocalizedString;
export declare const studiotokenscoloraccentforegroundlabel5: (
  inputs: Studiotokenscoloraccentforegroundlabel5Inputs
) => LocalizedString;
export declare const studiotokenscoloraccentlabel4: (
  inputs: Studiotokenscoloraccentlabel4Inputs
) => LocalizedString;
export declare const studiotokenscolorborderlabel4: (
  inputs: Studiotokenscolorborderlabel4Inputs
) => LocalizedString;
export declare const studiotokenscolorforegroundlabel4: (
  inputs: Studiotokenscolorforegroundlabel4Inputs
) => LocalizedString;
export declare const studiotokenscolormutedlabel4: (
  inputs: Studiotokenscolormutedlabel4Inputs
) => LocalizedString;
export declare const studiotokenscolorprimaryforegroundlabel5: (
  inputs: Studiotokenscolorprimaryforegroundlabel5Inputs
) => LocalizedString;
export declare const studiotokenscolorprimarylabel4: (
  inputs: Studiotokenscolorprimarylabel4Inputs
) => LocalizedString;
export declare const studiotokenscolorsurfacelabel4: (
  inputs: Studiotokenscolorsurfacelabel4Inputs
) => LocalizedString;
export declare const studiotokensfontbodylabel4: (
  inputs: Studiotokensfontbodylabel4Inputs
) => LocalizedString;
export declare const studiotokensfontcustomoption4: (
  inputs: Studiotokensfontcustomoption4Inputs
) => LocalizedString;
export declare const studiotokensfontheadinglabel4: (
  inputs: Studiotokensfontheadinglabel4Inputs
) => LocalizedString;
export declare const studiotokenspaneltitle3: (
  inputs: Studiotokenspaneltitle3Inputs
) => LocalizedString;
export declare const studiotokensradiushint3: (
  inputs: Studiotokensradiushint3Inputs
) => LocalizedString;
export declare const studiotokensradiuslabel3: (
  inputs: Studiotokensradiuslabel3Inputs
) => LocalizedString;
export declare const studiotokensresetbutton3: (
  inputs: Studiotokensresetbutton3Inputs
) => LocalizedString;
export declare const studiotoolbarcomment2: (
  inputs: Studiotoolbarcomment2Inputs
) => LocalizedString;
export declare const studiotoolbardraw2: (
  inputs: Studiotoolbardraw2Inputs
) => LocalizedString;
export declare const studiotoolbaredit2: (
  inputs: Studiotoolbaredit2Inputs
) => LocalizedString;
export declare const studiotoolbarrefresh2: (
  inputs: Studiotoolbarrefresh2Inputs
) => LocalizedString;
export declare const studiotoolbartweaks2: (
  inputs: Studiotoolbartweaks2Inputs
) => LocalizedString;
export declare const studiotoolbarzoomin3: (
  inputs: Studiotoolbarzoomin3Inputs
) => LocalizedString;
export declare const studiotoolbarzoomout3: (
  inputs: Studiotoolbarzoomout3Inputs
) => LocalizedString;
export declare const studiounpublishbutton2: (
  inputs: Studiounpublishbutton2Inputs
) => LocalizedString;
export declare const studiounpublisherrortoast3: (
  inputs: Studiounpublisherrortoast3Inputs
) => LocalizedString;
export declare const studiounpublishsuccesstoast3: (
  inputs: Studiounpublishsuccesstoast3Inputs
) => LocalizedString;
export declare const studioversioncomparebutton3: (
  inputs: Studioversioncomparebutton3Inputs
) => LocalizedString;
export declare const studioversioncomparehint3: (
  inputs: Studioversioncomparehint3Inputs
) => LocalizedString;
export declare const studioversioncurrentbadge3: (
  inputs: Studioversioncurrentbadge3Inputs
) => LocalizedString;
export declare const studioversiondiffcodetab4: (
  inputs: Studioversiondiffcodetab4Inputs
) => LocalizedString;
export declare const studioversiondiffpreviewtab4: (
  inputs: Studioversiondiffpreviewtab4Inputs
) => LocalizedString;
export declare const studioversiondifftitle3: (
  inputs: Studioversiondifftitle3Inputs
) => LocalizedString;
export declare const studioversionhistoryempty3: (
  inputs: Studioversionhistoryempty3Inputs
) => LocalizedString;
export declare const studioversionhistorytitle3: (
  inputs: Studioversionhistorytitle3Inputs
) => LocalizedString;
export declare const studioversionlabelbutton3: (
  inputs: Studioversionlabelbutton3Inputs
) => LocalizedString;
export declare const studioversionlabelerrortoast4: (
  inputs: Studioversionlabelerrortoast4Inputs
) => LocalizedString;
export declare const studioversionlabelplaceholder3: (
  inputs: Studioversionlabelplaceholder3Inputs
) => LocalizedString;
export declare const studioversionlabelsave3: (
  inputs: Studioversionlabelsave3Inputs
) => LocalizedString;
export declare const studioversionoriginaifull4: (
  inputs: Studioversionoriginaifull4Inputs
) => LocalizedString;
export declare const studioversionoriginaipatch4: (
  inputs: Studioversionoriginaipatch4Inputs
) => LocalizedString;
export declare const studioversionoriginimport3: (
  inputs: Studioversionoriginimport3Inputs
) => LocalizedString;
export declare const studioversionoriginmanual3: (
  inputs: Studioversionoriginmanual3Inputs
) => LocalizedString;
export declare const studioversionoriginrestore3: (
  inputs: Studioversionoriginrestore3Inputs
) => LocalizedString;
export declare const studioversionrestorebutton3: (
  inputs: Studioversionrestorebutton3Inputs
) => LocalizedString;
export declare const studioversionrestoreerrortoast4: (
  inputs: Studioversionrestoreerrortoast4Inputs
) => LocalizedString;
export declare const studioversionviewchatbutton4: (
  inputs: Studioversionviewchatbutton4Inputs
) => LocalizedString;
export declare const todaycreatefirstoffer3: (
  inputs: Todaycreatefirstoffer3Inputs
) => LocalizedString;
export declare const todaycreateoffer2: (
  inputs: Todaycreateoffer2Inputs
) => LocalizedString;
export declare const todaydescription1: (
  inputs: Todaydescription1Inputs
) => LocalizedString;
export declare const todayerrortitle2: (
  inputs: Todayerrortitle2Inputs
) => LocalizedString;
export declare const todayeyebrow1: (
  inputs: Todayeyebrow1Inputs
) => LocalizedString;
export declare const todaygettingstarteddescription3: (
  inputs: Todaygettingstarteddescription3Inputs
) => LocalizedString;
export declare const todaygettingstartedtitle3: (
  inputs: Todaygettingstartedtitle3Inputs
) => LocalizedString;
export declare const todayliveoffers2: (
  inputs: Todayliveoffers2Inputs
) => LocalizedString;
export declare const todayloading1: (
  inputs: Todayloading1Inputs
) => LocalizedString;
export declare const todaynextsteptitle3: (
  inputs: Todaynextsteptitle3Inputs
) => LocalizedString;
export declare const todaynextstepwithoffer4: (
  inputs: Todaynextstepwithoffer4Inputs
) => LocalizedString;
export declare const todaynextstepwithoutoffer4: (
  inputs: Todaynextstepwithoutoffer4Inputs
) => LocalizedString;
export declare const todayopenaction2: (
  inputs: Todayopenaction2Inputs
) => LocalizedString;
export declare const todayqueuedescription2: (
  inputs: Todayqueuedescription2Inputs
) => LocalizedString;
export declare const todayqueueempty2: (
  inputs: Todayqueueempty2Inputs
) => LocalizedString;
export declare const todayqueuetitle2: (
  inputs: Todayqueuetitle2Inputs
) => LocalizedString;
export declare const todaysystemready2: (
  inputs: Todaysystemready2Inputs
) => LocalizedString;
export declare const todaysystemstatus2: (
  inputs: Todaysystemstatus2Inputs
) => LocalizedString;
export declare const todaytitle1: (
  inputs: Todaytitle1Inputs
) => LocalizedString;
export declare const usageinsightsbyeventtitle4: (
  inputs: Usageinsightsbyeventtitle4Inputs
) => LocalizedString;
export declare const usageinsightscolumncount3: (
  inputs: Usageinsightscolumncount3Inputs
) => LocalizedString;
export declare const usageinsightscolumnevent3: (
  inputs: Usageinsightscolumnevent3Inputs
) => LocalizedString;
export declare const usageinsightsdescription2: (
  inputs: Usageinsightsdescription2Inputs
) => LocalizedString;
export declare const usageinsightsemptytitle3: (
  inputs: Usageinsightsemptytitle3Inputs
) => LocalizedString;
export declare const usageinsightsloaderrortitle4: (
  inputs: Usageinsightsloaderrortitle4Inputs
) => LocalizedString;
export declare const usageinsightstitle2: (
  inputs: Usageinsightstitle2Inputs
) => LocalizedString;
export declare const usageinsightstotalevents3: (
  inputs: Usageinsightstotalevents3Inputs
) => LocalizedString;
export declare const verifiedbody1: (
  inputs: Verifiedbody1Inputs
) => LocalizedString;
export declare const verifiedloginlink2: (
  inputs: Verifiedloginlink2Inputs
) => LocalizedString;
export declare const verifiedtitle1: (
  inputs: Verifiedtitle1Inputs
) => LocalizedString;
export declare const verifyemailbody2: (
  inputs: Verifyemailbody2Inputs
) => LocalizedString;
export declare const verifyemailtitle2: (
  inputs: Verifyemailtitle2Inputs
) => LocalizedString;
