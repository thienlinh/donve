...
export function StudioPage() {
  ...
  const { prompt, missingLeadForm, missingSeoMeta } = useSearch({
    strict: false
  }) as StudioSearch;
  ...
  if (detailQuery.isPending) {
    return (
      <div className="flex items-center gap-2 p-6 text-sm text-muted-foreground">
        <Spinner /> Đang tải trình chỉnh sửa…
      </div>
    );
  }
  ...
  if (!detailQuery.data) {
    return (
      <Empty>
        <EmptyHeader>
          <EmptyTitle>Không tải được trang bán hàng này</EmptyTitle>
          {detailQuery.error && (
    ...
    if (generateError) {
      ...
      placeholder = (
        <div className="flex h-full flex-col items-center justify-center p-6">
          <Empty>
            <EmptyHeader>
              <EmptyTitle>
                {knownError?.title ?? "Chưa tạo được trang này"}
              </EmptyTitle>
              <EmptyDescription>
      ...
      placeholder = (
        <div className="flex h-full flex-col items-center justify-center p-6">
          <Empty>
            <EmptyHeader>
              <EmptyTitle>Trang này chưa được tạo nội dung</EmptyTitle>
              <EmptyDescription>
                Chưa thể tạo nội dung và không còn yêu cầu nào đang chờ để thử
                lại. Hãy tạo lại từ danh sách trang bán hàng.
              </EmptyDescription>
            </EmptyHeader>
            <EmptyContent>
              <Button
...
function StudioEditor({
  ...
  const [selectedElement, setSelectedElement] =
    React.useState<SelectedElementInfo | null>(null);
  ...
  const undoRedoControlsRef = React.useRef<UndoRedoControls | null>(null);
  ...
  // Inspector's image field (background-image / `<img src>`) — same compress-then-upload
  // pipeline as Design Files' upload button (FR-B-29), just committed as a patch op instead of
  // only landing in the assets list.
  const inspectorImageUploadMutation = useMutation({
    mutationFn: async (file: File) => {
      const { blob, fileName: webpName } = await compressToWebp(file);
      return uploadAsset(landingPage.id, blob, webpName);
    },
    onSuccess: (asset) => {
      queryClient.invalidateQueries({
      ...
      const url = assetFileUrl(landingPage.id, asset.id);
      commitRef.current?.(
        selectedElement?.tag === "img"
          ? { type: "setAttr", srcmapId: selectedId, attr: "src", value: url }
          : {
              type: "setStyle",
              srcmapId: selectedId,
    ...
    onError: () =>
      toast.add({ title: m.studioFilesUploadFailed(), type: "error" })
  });
      ...
      <div className="relative flex h-full min-w-0">
        <div className="relative min-w-0 flex-1">
          <Canvas
            ...
            onUndoRedoControlsReady={(controls) => {
          ...
          <div className="flex h-full w-[320px] flex-col">
            ...
            >
              ...
              <TabsContent value="inspect" className="min-h-0 flex-1">
                {mode === "edit" && selectedId && selectedElement ? (
                  <InspectorPanel
                    key={selectedId}
                    values={selectedElement.values}
                    tag={selectedElement.tag}
                    imageSrc={selectedElement.imageSrc}
                    onCommit={(prop, value) => {
                      commitRef.current?.({
                        type: "setStyle",
                        srcmapId: selectedId,
                    ...
                    onUploadImage={async (file) => {
                      // Failure is already toasted by the mutation's onError above — nothing
                      // left for the field itself to do but stop its "uploading…" state.
                      await inspectorImageUploadMutation
                        .mutateAsync(file)
                        .catch(() => {});
                    }}
