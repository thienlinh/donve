        ...
        {usage && (
          <>
            ...
            {usage.recentUsage.length === 0 ? (
              ...
              // ponytail: plain scroll cap, no virtualization — recentUsage is server-limited to
              // 50 rows (aiUsageRepository.listRecent), a fixed-height scroll box is enough to
              // keep the card from growing to page-length; revisit if that server cap ever grows.
    ...
    <TableRow>
      <TableCell className="flex items-center gap-2 font-medium">
        ...
        {connection.isDefault && (
          <Badge variant="secondary">{m.aiConnectionDefaultBadge()}</Badge>
        )}
      </TableCell>
      <TableCell className="font-mono text-xs">
      ...
      <TableCell>
        ...
        >
          {connection.status === "active"
            ? m.aiConnectionStatusActive()
            : m.aiConnectionStatusInvalid()}
        </Badge>
      </TableCell>
