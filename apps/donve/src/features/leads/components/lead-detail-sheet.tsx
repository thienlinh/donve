...
const ORDER_STATUS_LABELS: Record<Order["status"], string> = {
  ...
  awaiting_confirmation: "Khách báo đã chuyển khoản",
  paid: "Đã thanh toán",
  fulfilled: "Đã giao",
  cancelled: "Đã huỷ",
...
function orderNextAction(status: Order["status"]): string {
  switch (status) {
    case "pending":
      return "Chờ khách chuyển khoản";
    case "awaiting_confirmation":
      return "Kiểm tra giao dịch và xác nhận";
    case "paid":
      return "Giao sản phẩm hoặc đặt lịch";
    case "fulfilled":
      return "Không còn việc";
    case "cancelled":
      return "Không còn việc";
    case "refunded":
      return "Theo dõi hoàn tiền";
  }
}
          ...
          {orders.map((order) => (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground">
                Việc tiếp theo: {orderNextAction(order.status)}
              </p>
