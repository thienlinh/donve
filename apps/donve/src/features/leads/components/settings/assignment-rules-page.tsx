...
// The API only accepts these two exact strings (or null) — see `parseOnSlaBreach` in
// apps/api/src/modules/leads/routes.ts, which 400s on anything else. A free-text input here
// used to let a non-technical user type any phrase and get a silent-looking save failure.
const ON_SLA_BREACH_OPTIONS = [
  "",
  "reassign_next_in_pool",
  "notify_manager"
] as const;
const onSlaBreachLabel: Record<
  (typeof ON_SLA_BREACH_OPTIONS)[number],
  () => string
> = {
  "": m.leadsAssignmentSlaBreachNone,
  reassign_next_in_pool: m.leadsAssignmentSlaBreachReassign,
  notify_manager: m.leadsAssignmentSlaBreachNotify
};
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            >
              {ON_SLA_BREACH_OPTIONS.map((value) => (
                <NativeSelectOption key={value} value={value}>
                  {onSlaBreachLabel[value]()}
                </NativeSelectOption>
              ))}
            </NativeSelect>
          </div>
