      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead>Việc tiếp theo</TableHead>
            <TableHead className="hidden sm:table-cell">
          ...
          {data.leads.map((lead) => (
            <TableRow key={lead.id} className="cursor-pointer">
              ...
              <TableCell
                ...
                onClick={() => onOpenLead(lead)}
              >
                {lead.utm.source ?? leadSourceLabel(lead.source)}
              </TableCell>
              <TableCell>
                <Button
                  aria-label={`Xử lý ${lead.fullName}`}
                  className="gap-1"
                  onClick={() => onOpenLead(lead)}
                  size="sm"
                  variant="ghost"
                >
                  {nextLeadAction(lead.stage)}
...
function nextLeadAction(stage: string): string {
  switch (stage) {
    case "new":
      return "Gọi trong 5 phút";
    case "contacted":
      return "Chờ phản hồi";
    case "interested":
    case "consulting":
