...
export interface LeadSearchParams {
  ...
  /** Deep-link-only, e.g. Today's "Lead mới cần liên hệ" → `/inbox?stage=new` — not a regular
   * filter-bar control, so it's read straight off `search` rather than round-tripped through
   * `LeadFilterState`/`filtersToSearch`. */
  stage?: string;
}
