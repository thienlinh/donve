...
export function StudioNativePage() {
  ...
  // `#frame-root` only exists inside Puck's own iframe (never in this parent document); Puck's
  // own bundled CSS sets `#frame-root{height:1px;min-height:100vh}` (confirmed live 2026-09-05
  // in @puckeditor/core's dist/index.css) — an explicit `height` pins the box at exactly one
  // viewport tall no matter how long the actual page is, so any section past the first screen
  // renders outside `#frame-root`'s own painted background and falls through to Puck's own
  // (dark) `<body>` background instead — a real published page has no such cap, it's just a
  // plain block that grows with its content. `height:auto` overrides that cap so our
  // background/color actually cover the full page, not just its first screen.
  useEffect(() => {
    if (!activeDoc) return;
    ...
    styleEl.textContent = `${designTokensToCss(activeDoc.tokens)}#frame-root{height:auto;background-color:var(--lp-color-surface);color:var(--lp-color-foreground);min-height:100vh;}`;
  ...
  const initialPuckData = useMemo(
    ...
    // — see the `key` on `<Puck>` below, which forces the remount this reads.
