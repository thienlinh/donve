...
**— mới có CI cơ bản (install→lint→fmt:check→typecheck→build→test) trong `.github/workflows/ci.yml`; còn thiếu remote cache, 3 environment, Wrangler secrets, auto-deploy staging — chưa tick.**
